...
AdSelectTypes::Option materializeSelectOption(const QModelIndex& index, const SelectRoleConfig& roles) {
...
QString optionSelectedTextFromMetadata(const AdSelectTypes::Option& option) {
  return option.metadata.value(QString::fromLatin1(kSelectSelectedTextMetadataKey)).toString().trimmed();
}
