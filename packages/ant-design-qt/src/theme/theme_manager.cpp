  ...
  int pixelMetric(PixelMetric metric,
    ...
    switch (metric) {
      ...
      case PM_ButtonIconSize:
        return std::max(QProxyStyle::pixelMetric(metric, option, widget), qRound(theme_.metrics.fontSizeLG));
      case PM_ScrollBarExtent:
        return std::max(QProxyStyle::pixelMetric(metric, option, widget), qRound(theme_.metrics.controlHeightSM));
...
QStyle* makeBaseStyle(const QApplication* app) {
  ...
  if (app && app->style()) {
    const QString objectName = app->style()->objectName();
...
ThemeConfig transitionedDefaults(const ThemeConfig& current,
                                 ThemeScheme nextScheme,
...
ThemeManager::ThemeManager(QObject* parent)
    : QObject(parent), config_(defaultThemeConfig()), resolved_(makeResolvedTheme(config_)), palette_(resolved_.palette) {}
...
void ThemeManager::ensureApplicationStyle() {
  ...
  if (auto* themedStyle = dynamic_cast<AdThemeProxyStyle*>(style_.data())) {
    themedStyle->setTheme(resolved_.theme);
    if (app_->style() != themedStyle) {
      app_->setStyle(themedStyle);
    }
    return;
  }
...
void ThemeManager::setScopeOverride(QObject* scope, const ThemeOverride& overrideValue) {
  ...
  if (it != scopeStates_.end()) {
    ...
    connect(scope, &QObject::destroyed, this, [this](QObject* destroyedScope) { scopeStates_.remove(destroyedScope); });
  }
...
ThemeConfig ThemeManager::resolvedConfigFor(const QWidget* widget, const QWidget* logicalOwner) const {
...
const QStyle* ThemeManager::style() const { return style_ ? style_.data() : (app_ ? app_->style() : nullptr); }
