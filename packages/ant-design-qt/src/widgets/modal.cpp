...
class ModalOverlayWidget final : public QWidget {
 public:
  ...
  void setFocusNavigator(std::function<bool(bool)> navigator) { focusNavigator_ = std::move(navigator); }
...
void AdModal::attachContentWidget(QWidget* widget) {
  ...
  contentWidgetDestroyedConnection_ = connect(contentWidget_, &QObject::destroyed, this, [this, rawWidget]() {
...
void AdModal::attachFooterWidget(QWidget* widget) {
  ...
  footerWidgetDestroyedConnection_ = connect(footerWidget_, &QObject::destroyed, this, [this, rawWidget]() {
...
void AdModal::unregisterOpenModal(AdModal* modal) {
  ...
  QWidget* ownerWindow = modal->ownerWindow_ ? modal->ownerWindow_.data() : modal->resolveOwnerWindow();
  auto& modals = openModals();
  for (int i = modals.size() - 1; i >= 0; --i) {
    if (!modals.at(i) || modals.at(i).data() == modal) {
      modals.removeAt(i);
...
void AdModal::attachOwnerWindowWatcher(QWidget* ownerWindow) {
  ...
  ownerWindowDestroyedConnection_ = connect(ownerWindow, &QObject::destroyed, this, [this]() {
    ownerWindow_.clear();
  });
}
...
void AdModal::ensureOverlay() {
  ...
  if (ownerWindow_ != resolvedOwnerWindow) {
    detachOwnerWindowWatcher();
  ...
  auto* overlay = new ModalOverlayWidget(ownerWindow_);
...
void AdModal::syncOverlayGeometry() {
  if (!overlay_ || !ownerWindow_) {
    return;
  }
  const QRect rect = ownerWindow_->rect();
...
void AdModal::refreshTexts() {
  ...
  if (acceptButtonControl_) {
    acceptButtonControl_->setText(acceptText_.trimmed().isEmpty() ? tr("OK") : acceptText_);
  ...
  if (rejectButtonControl_) {
    rejectButtonControl_->setText(rejectText_.trimmed().isEmpty() ? tr("Cancel") : rejectText_);
  }
...
void AdModal::refreshVisibility() {
  ...
  if (header_) {
    const bool showHeader = confirmMode ? closeButtonVisible_ : (hasTitle || closeButtonVisible_);
  ...
  if (closeButton_) {
    closeButton_->setVisible(closeButtonVisible_);
  }
...
void AdModal::refreshTitleIcon() {
  ...
  iconColors.primary = style.iconColor;
  ...
  titleIconLabel_->setPixmap(adqt::icons::renderIconPixmap(token, QSize(iconSize, iconSize), ratio));
...
void AdModal::updateAccessibility() {
  ...
  const QString resolvedContent =
      (contentLoading_ ? tr("Loading...") : text_).trimmed();

  overlay_->setAccessibleName(tr("Modal overlay"));
...
QWidget* AdModal::resolveInitialFocusTarget() const {
  if (acceptButtonControl_ && acceptButtonControl_->isVisible() && acceptButtonControl_->isEnabled()) {
    return acceptButtonControl_;
  }
  if (rejectButtonControl_ && rejectButtonControl_->isVisible() && rejectButtonControl_->isEnabled()) {
...
void AdModal::applyVisualStyle() {
  ...
  if (closeButton_) {
    adqt::icons::IconColorOverrides iconColors;
    iconColors.primary = style.closeIconColor;
    iconColors.hasPrimary = true;
    ...
    const qreal ratio = panel_ ? std::max(1.0, panel_->devicePixelRatioF()) : 1.0;
    closeButton_->setIcon(QIcon(adqt::icons::renderIconPixmap(outlined_icons::Close(iconColors),
                                                              QSize(closeIconSize, closeIconSize), ratio)));
    closeButton_->setIconSize(QSize(closeIconSize, closeIconSize));
    closeButton_->setEnabled(!acceptButtonBusy_);
    static_cast<ModalIconButton*>(closeButton_.data())->setVisualStyle(style.closeButtonBackground,
                                                                       style.closeButtonHoverBackground,
                                                                       style.closeButtonPressedBackground,
                                                                       style.closeButtonDisabledBackground,
                                                                       style.closeButtonBorderColor,
...
AdModal::VisualStyle AdModal::resolveVisualStyle() const {
  ...
  style.closeButtonPressedBackground = toColor(map.colorFillSecondary, style.closeButtonHoverBackground);
