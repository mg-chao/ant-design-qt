...
AdComboBox::Option makeOption(const QString& value,
                               const QString& label,
                               bool disabled = false,
...
class DemoAddonLabel final : public QLabel {
 public:
  DemoAddonLabel(const QString& text,
                 AdInputNumber::ControlSize controlSize,
 ...
 private:
  void applyTheme() {
    ...
    const QColor contentColor =
        demo::themeColorOr(map.colorTextSecondary, demo::themeColorOr(map.colorText, QColor("#595959")));

    setStyleSheet(QStringLiteral(
                      "QLabel { border: %1px solid %2; border-radius: 0; border-top-left-radius: %3px; "
                      "border-bottom-left-radius: %3px; border-top-right-radius: %4px; "
                      "border-bottom-right-radius: %4px; background: %5; color: %6; padding: 0 10px; }")
                      .arg(borderWidth)
                      .arg(borderColor.name(QColor::HexArgb))
                      .arg(leftRadius)
                      .arg(rightRadius)
                      .arg(background.name(QColor::HexArgb))
                      .arg(contentColor.name(QColor::HexArgb)));
    setFixedHeight(height);

    if (adqt::icons::isValid(iconRef_)) {
      adqt::icons::IconRef tinted = iconRef_;
      tinted.colors.primary = contentColor;
...
QLabel* makeAddonLabel(const QString& text,
                       AdInputNumber::ControlSize controlSize,
                       bool joinedLeft = false,
                       bool joinedRight = false,
                       QWidget* parent = nullptr) {
...
QLabel* makeAddonIcon(const adqt::icons::IconRef& token,
                      AdInputNumber::ControlSize controlSize,
                      bool joinedLeft = false,
                      bool joinedRight = false,
                      QWidget* parent = nullptr) {
...
QWidget* InputNumberDocsPage::buildAddonDemo() {
...

  {
    ...
    auto* disabledAfter = makeAddonIcon(outlined_icons::Setting(), AdInputNumber::ControlSize::Medium);
...
QWidget* InputNumberDocsPage::buildDisabledDemo() {
  ...
  connect(toggle, &QAbstractButton::clicked, input, [input]() { input->setEnabled(!input->isEnabled()); });
...
QWidget* InputNumberDocsPage::buildWheelDemo() {
  ...
  connect(input, &AdInputNumber::stepped, hint,
          [hint, input](int offset, AdInputNumber::StepType type, AdInputNumber::StepEmitter emitter) {
            ...
            const QString valueText = input->hasValue() ? valueToText(input->value()) : QStringLiteral("null");
