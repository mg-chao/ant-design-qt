...
QVector<QColor> generateMappedPreset(const QColor& base, bool darkTheme, const QColor& backgroundColor) {
  ...
  const QVector<QColor> mapped = adqt::theme::generateMappedPalette(base, darkTheme, backgroundColor);
...
ColorFamily makePresetFamily(AdButton::AccentRole accentRole, const ThemeMapToken& map, const ThemeSeedToken& seed) {
  ...
  const QColor containerBg = toColor(map.colorBgContainer, QColor("#ffffff"));
  // Ant Design exposes preset hover/active steps differently in dark mode than the base accent tokens.
  ...
  family.shadow = resolveAlphaColor(toColor(mapped.value(1), family.base.lighter(170)), containerBg);
...
ButtonMetrics resolveMetrics(const ButtonStyleInput& input, const ThemeMapToken& map) {
  ...
  const int inlinePadding =
      (input.sizeClass == AdButton::SizeClass::Small) ? qRound(8.0 - map.lineWidth)
