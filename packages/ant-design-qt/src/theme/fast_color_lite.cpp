...
bool FastColorLite::parseHex(const QString& input) {
  ...
  auto readSingle = [&hex](int index) {
    return QString(hex.at(index)) + hex.at(index);
  };
...
bool FastColorLite::parseRgb(const QString& input) {
  const QRegularExpression prefixRe("^\\s*rgba?\\((.*)\\)\\s*$", QRegularExpression::CaseInsensitiveOption);
