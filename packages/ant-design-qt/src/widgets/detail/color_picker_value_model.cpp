...
ColorPickerValueModel::State ColorPickerValueModel::normalizedState(const State& state) {
  ...
  if (out.selection.isGradient()) {
    out.selection.gradientStops = normalizeStops(out.selection.gradientStops, out.selection.solidColor);
    if (!modeListContains(out.modeOptions, Mode::Gradient)) {
      const int index =
          std::clamp(out.activeStopIndex, 0, static_cast<int>(out.selection.gradientStops.size()) - 1);
  ...
  if (out.mode == Mode::Gradient && modeListContains(out.modeOptions, Mode::Gradient)) {
    ...
    out.mode = modeListContains(out.modeOptions, Mode::Solid) ? Mode::Solid : out.modeOptions.constFirst();
    out.activeStopIndex = 0;
  }
  return out;
}

ColorPickerValueModel::State ColorPickerValueModel::stateFromSelection(const AdColorSelection& selection,
...
ColorPickerValueModel::State ColorPickerValueModel::stateFromCssValue(const QString& cssValue,
                                                                      const QVector<Mode>& modeOptions,
                                                                      Mode preferredMode,
                                                                      int activeStopIndex,
                                                                      bool* ok) {
...
ColorPickerValueModel::State ColorPickerValueModel::withModeOptions(const State& state,
                                                                    const QVector<Mode>& modeOptions) {
  State next = state;
...
ColorPickerValueModel::State ColorPickerValueModel::withGradientStopPositions(const State& state,
  ...
  const QVector<AdColorGradientStop> current = normalizeStops(next.selection.gradientStops,
                                                              editableColor(next));
  ...
  next.activeStopIndex = std::clamp(nextActiveIndex, 0,
                                    static_cast<int>(next.selection.gradientStops.size()) - 1);
...
QString ColorPickerValueModel::cssValue(const State& state) {
  return cssValue(state.selection);
}
...
QString ColorPickerValueModel::cssValue(const AdColorSelection& selection) {
  ...
  if (selection.isGradient()) {
    ...
    for (const AdColorGradientStop& stop : normalizeStops(selection.gradientStops, selection.solidColor)) {
...
QString ColorPickerValueModel::formattedValue(const AdColorSelection& selection,
                                              Format format,
                                              int activeStopIndex) {
  ...
  if (selection.isGradient()) {
    QStringList parts;
    const QVector<AdColorGradientStop> stops = normalizeStops(selection.gradientStops, selection.solidColor);
    parts.reserve(stops.size());
    for (const AdColorGradientStop& stop : stops) {
      parts.append(QStringLiteral("%1 %2%").arg(formattedColorString(stop.color, format)).arg(stop.percent));
    }
    return QStringLiteral("linear-gradient(90deg, %1)").arg(parts.join(QStringLiteral(", ")));
  }
