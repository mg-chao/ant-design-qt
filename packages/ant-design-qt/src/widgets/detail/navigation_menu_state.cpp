...
QString navigationMenuStableIdForIndex(const QModelIndex& sourceIndex) {
  ...
  const QString explicitId = sourceIndex.data(AdNavigationMenu::StableIdRole).toString().trimmed();
...
void NavigationMenuState::rebuildStableIdIndexCache() {
    ...
    for (int row = 0; row < rowCount; ++row) {
      ...
      if (!stableId.isEmpty()) {
        if (duplicateStableIds.contains(stableId)) {
          // Leave duplicate stable ids unresolved so state restoration never targets an ambiguous row.
  ...
  if (!duplicateStableIds.isEmpty()) {
    ...
    qWarning().noquote() << "AdNavigationMenu: duplicate stable ids detected:" << duplicates.join(", ");
  }
}
...
bool NavigationMenuState::containsExpanded(const QModelIndex& index,
