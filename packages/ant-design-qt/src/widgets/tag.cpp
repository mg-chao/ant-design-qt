...
bool colorTokensEqual(const AdTag::ColorTokens& lhs, const AdTag::ColorTokens& rhs) {
  return std::tie(lhs.defaultBg,
                  lhs.defaultColor,
                  lhs.defaultBorderColor,
                  lhs.solidTextColor,
                  lhs.closeColor,
                  lhs.closeHoverColor,
                  lhs.closeHoverBackground,
                  lhs.colorTextDisabled,
                  lhs.colorBgContainerDisabled,
                  lhs.colorBorderDisabled,
                  lhs.checkableHoverColor,
                  lhs.checkableHoverBg,
                  lhs.checkableCheckedColor,
                  lhs.checkableCheckedBg,
                  lhs.checkableCheckedHoverBg,
                  lhs.checkableActiveBg,
                  lhs.focusOutlineColor,
                  lhs.waveColor) ==
         std::tie(rhs.defaultBg,
                  rhs.defaultColor,
                  rhs.defaultBorderColor,
                  rhs.solidTextColor,
                  rhs.closeColor,
                  rhs.closeHoverColor,
                  rhs.closeHoverBackground,
                  rhs.colorTextDisabled,
                  rhs.colorBgContainerDisabled,
                  rhs.colorBorderDisabled,
                  rhs.checkableHoverColor,
                  rhs.checkableHoverBg,
                  rhs.checkableCheckedColor,
                  rhs.checkableCheckedBg,
                  rhs.checkableCheckedHoverBg,
                  rhs.checkableActiveBg,
                  rhs.focusOutlineColor,
                  rhs.waveColor);
}

bool metricTokensEqual(const AdTag::MetricTokens& lhs, const AdTag::MetricTokens& rhs) {
  return std::tie(lhs.height,
                  lhs.borderRadius,
                  lhs.borderWidth,
                  lhs.paddingHorizontal,
                  lhs.iconSize,
                  lhs.contentGap,
                  lhs.closeGap,
...
void mergeSemanticSlotStyle(AdTag::SemanticSlotStyle* target, const AdTag::SemanticSlotStyle& source) {
  if (!target) {
    return;
  }
...
QSize AdTag::sizeHint() const {
  ...
  const int height = std::max(style.metrics.height, contentHeight + style.metrics.borderWidth * 2 + 4);
...
void AdTag::paintEvent(QPaintEvent* event) {
  ...
  const qreal focusInset = (hasFocus() && focusVisible_ && visualStyle.focusOutlineColor.alpha() > 0 &&
                            visualStyle.metrics.focusOutlineWidth > 0.0)
                               ? visualStyle.metrics.focusOutlineWidth / 2.0 +
                                     visualStyle.metrics.focusOutlineOffset
                               : 0.0;
  ...
  if (hasFocus() && focusVisible_ && visualStyle.focusOutlineColor.alpha() > 0 &&
      visualStyle.metrics.focusOutlineWidth > 0.0) {
    painter.setPen(QPen(visualStyle.focusOutlineColor,
                        visualStyle.metrics.focusOutlineWidth,
                        Qt::SolidLine,
                        Qt::RoundCap,
                        Qt::RoundJoin));
  ...
  const qreal bodyInset =
      focusInset + (visualStyle.metrics.borderWidth > 0 ? visualStyle.metrics.borderWidth / 2.0 : 0.0) +
      0.5;
  ...
  if (visualStyle.borderColor.alpha() > 0 && visualStyle.metrics.borderWidth > 0) {
    painter.setPen(QPen(visualStyle.borderColor,
                        visualStyle.metrics.borderWidth,
                        visualStyle.borderPenStyle,
                        Qt::RoundCap,
                        Qt::RoundJoin));
    painter.setBrush(Qt::NoBrush);
    painter.drawPath(bodyPath);
  ...
  const int closeGap = closeButtonVisible() &&
                               (textWidthValue > 0 || adqt::icons::isValid(iconRef_))
  ...
  if (closeButtonVisible()) {
    ...
    if (adqt::icons::isValid(closeIcon)) {
      adqt::icons::IconRef coloredClose = closeIcon;
      coloredClose.colors.primary =
          closeHovered_ || closePressed_ ? visualStyle.closeHoverColor : visualStyle.closeColor;
      coloredClose.colors.hasPrimary = true;
  ...
  QRectF iconRect;
  if (adqt::icons::isValid(iconRef_)) {
    const qreal iconY = contentRect.y() + std::max<qreal>(0.0, (contentRect.height() - iconSize) / 2.0);
    if (rtl) {
    ...
    adqt::icons::IconRef coloredIcon = iconRef_;
    coloredIcon.colors.primary = visualStyle.iconColor;
    coloredIcon.colors.hasPrimary = true;
  ...
  if (!text().isEmpty()) {
    ...
    const qreal textY = contentRect.y() + std::max<qreal>(0.0, (contentRect.height() - textHeight) / 2.0);
    if (rtl) {
      textRect = QRectF(contentRect.left(), textY, std::max<qreal>(0.0, cursor - contentRect.left()), textHeight);
    } else {
      textRect = QRectF(cursor, textY, std::max<qreal>(0.0, contentRect.right() - cursor + 1.0), textHeight);
    }

    QPalette palette = this->palette();
    palette.setColor(QPalette::ButtonText, visualStyle.contentColor);
    QWidget::style()->drawItemText(&painter,
                                   textRect.toAlignedRect(),
...
void AdTag::mouseReleaseEvent(QMouseEvent* event) {
  ...
  if (closePressed_) {
    const bool triggerClose = event->button() == Qt::LeftButton && closeButtonHit(mouseEventPos(event));
    closePressed_ = false;
    updateHoverState(mouseEventPos(event));
    update();
    if (triggerClose) {
...
QRect AdTag::closeButtonRect() const {
  ...
  const QRectF bodyRect = QRectF(rect()).adjusted(borderInset, borderInset, -borderInset, -borderInset);
