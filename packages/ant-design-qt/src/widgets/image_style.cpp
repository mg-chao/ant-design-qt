...
void applySemanticSlot(const AdImageViewer::SemanticSlotStyle& slot,
                       QColor* textColor,
                       QColor* backgroundColor,
                       QColor* borderColor) {
  if (textColor && slot.textColor.has_value()) {
...
ImageVisualStyle baseImageVisualStyle(const adqt::theme::ResolvedTheme& resolved) {
  ...
  style.focusRing = toColor(semantic.accentBorder, toColor(map.colorPrimaryBorder, QColor("#91caff")));
...
void applyViewerComponentTokens(const AdImageViewer::ComponentTokens& tokens, ImageVisualStyle* style) {
  if (!style) {
    return;
  }
