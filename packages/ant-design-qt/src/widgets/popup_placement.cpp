...
QPoint applyOverlayCrossAxisShift(OverlayPopupPlacement placement, QPoint point, const QSize& popupSize,
                                  const QRect& bounds) {
  if (!bounds.isValid()) {
    return point;
  }
  ...
  } else if (placement == OverlayPopupPlacement::Left || placement == OverlayPopupPlacement::Right) {
...
OverlayPopupPlacementOutput resolveOverlayPopupPlacement(const OverlayPopupPlacementInput& input) {
  ...
  if (input.allowFallback) {
    const OverlayPopupPlacement fallbackPlacement = oppositeOverlayPopupPlacement(input.preferredPlacement);
    const QPoint fallbackTopLeft = computeForPlacement(fallbackPlacement);
    if (overflowCost(fallbackTopLeft, popupSize, bounds) < overflowCost(preferredTopLeft, popupSize, bounds)) {
      selectedTopLeft = fallbackTopLeft;
      selectedPlacement = fallbackPlacement;
    }
  }

  out.topLeft = selectedTopLeft;
  out.placement = selectedPlacement;
  const QRect popupRect(selectedTopLeft, popupSize);
