...
void SelectCompositeModel::reconnectSourceModel(QPointer<QAbstractItemModel>* slot, QAbstractItemModel* model) {
  if (*slot == model) {
    return;
  }
  if (*slot) {
  ...
  *slot = model;
  if (*slot) {
    ...
    connect(*slot, &QAbstractItemModel::rowsRemoved, this,
            [this](const QModelIndex&, int, int) { resetFromSourceChange(); });
    connect(*slot, &QAbstractItemModel::rowsMoved, this,
    ...
    connect(*slot,
            ...
            this,
            [this](const QModelIndex&, const QModelIndex&, const QList<int>&) { resetFromSourceChange(); });
  }
  resetFromSourceChange();
}
...
bool SelectFilterProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const {
  if (!sourceModel()) {
    return false;
  }
  ...
  if (!searchFilterFields_.isEmpty()) {
    for (const QString& field : searchFilterFields_) {
      if (optionFieldValue(option, field).contains(term, Qt::CaseInsensitive)) {
        return true;
      }
    }
    return false;
  ...
  const QList<int> roles = searchRoles_.isEmpty() ? QList<int>{roles_.labelRole, roles_.valueRole}
                                                  : searchRoles_;
  for (int role : roles) {
    const QString candidate = sourceModel()->data(index, role).toString();
    if (candidate.contains(term, Qt::CaseInsensitive)) {
      return true;
    }
  }
  return false;
