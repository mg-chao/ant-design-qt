...
int SelectSelectionController::indexOf(const QVariant& value) const {
  ...
  return key.isEmpty() ? -1 : keys_.indexOf(key);
}
...
bool SelectSelectionController::setCurrentValue(const QVariant& value) {
  return setCurrentValues(value.isValid() && !value.isNull() ? QVariantList{value} : QVariantList());
}
...
bool SelectSelectionController::clear() {
