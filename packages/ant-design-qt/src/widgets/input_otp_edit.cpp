...
QColor resolvedBackgroundColor(const InputVisualStyle& style, bool enabled, bool focused, bool hovered) {
  if (!enabled) {
...
QColor resolvedBorderColor(const InputVisualStyle& style, bool enabled, bool focused, bool hovered) {
  if (!enabled) {
...
void AdOtpEdit::applyVisualStyle() {
  ...
  for (QLineEdit* cell : cells_) {
    ...
    paintStyle.border = resolvedBorderColor(style, isEnabled(), cell->hasFocus(), cell->underMouse());
    ...
    palette.setColor(QPalette::Text, isEnabled() ? style.selectorTextColor : style.disabledTextColor);
...
void AdOtpEdit::emitInputState() {
  ...
  for (QLineEdit* cell : cells_) {
    const QString piece = (cell && !cell->text().isEmpty()) ? QString(cell->text().at(0)) : QString();
