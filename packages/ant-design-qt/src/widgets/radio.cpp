...
bool colorTokensEqual(const AdRadio::ColorTokens& lhs, const AdRadio::ColorTokens& rhs) {
  return std::tie(lhs.textColor,
                  lhs.indicatorBorderColor,
                  lhs.indicatorFillColor,
                  lhs.indicatorDotColor,
                  lhs.buttonTextColor,
                  lhs.buttonFillColor,
                  lhs.buttonBorderColor,
                  lhs.focusRingColor,
                  lhs.waveColor) ==
         std::tie(rhs.textColor,
                  rhs.indicatorBorderColor,
                  rhs.indicatorFillColor,
                  rhs.indicatorDotColor,
                  rhs.buttonTextColor,
                  rhs.buttonFillColor,
                  rhs.buttonBorderColor,
                  rhs.focusRingColor,
                  rhs.waveColor);
}

bool metricTokensEqual(const AdRadio::MetricTokens& lhs, const AdRadio::MetricTokens& rhs) {
  return std::tie(lhs.radioSize,
                  lhs.dotSize,
                  lhs.borderWidth,
                  lhs.labelPaddingInlineStart,
                  lhs.labelPaddingInlineEnd,
                  lhs.textLineHeight,
                  lhs.wrapperMarginInlineEnd,
                  lhs.buttonHeight,
                  lhs.buttonPaddingInline,
                  lhs.buttonBorderRadius,
                  lhs.focusOutlineWidth,
                  lhs.focusOutlineOffset) ==
         std::tie(rhs.radioSize,
                  rhs.dotSize,
                  rhs.borderWidth,
                  rhs.labelPaddingInlineStart,
                  rhs.labelPaddingInlineEnd,
                  rhs.textLineHeight,
                  rhs.wrapperMarginInlineEnd,
                  rhs.buttonHeight,
                  rhs.buttonPaddingInline,
                  rhs.buttonBorderRadius,
                  rhs.focusOutlineWidth,
                  rhs.focusOutlineOffset);
}

bool componentTokensEqual(const AdRadio::ComponentTokens& lhs, const AdRadio::ComponentTokens& rhs) {
...
void mergeComponentTokens(AdRadio::ComponentTokens* target, const AdRadio::ComponentTokens& source) {
  if (!target) {
    return;
  }
...
qreal centeredStrokeInset(qreal strokeWidth) { return std::max<qreal>(0.0, strokeWidth / 2.0) + 0.5; }
...
RadioLabelContent measureRadioLabelContent(const AdRadio* radio, const detail::RadioMetrics& metrics) {
...
InlineContentLayout layoutInlineContent(const QRectF& rect,
  ...
  const qreal startY = rect.y() + std::max<qreal>(0.0, (rect.height() - layout.contentHeight) / 2.0);

  if (direction == Qt::RightToLeft) {
    qreal cursor = startX + layout.contentWidth;
    if (hasIcon) {
      cursor -= iconWidth;
      layout.iconRect = QRectF(cursor,
                               startY + std::max<qreal>(0.0, (layout.contentHeight - iconHeight) / 2.0),
                               iconWidth,
                               iconHeight);
    }
    if (hasText) {
      ...
      cursor -= textWidth;
      layout.textRect = QRectF(cursor,
                               startY + std::max<qreal>(0.0, (layout.contentHeight - textHeight) / 2.0),
                               textWidth,
                               textHeight);
    }
  } else {
...
QRectF defaultRadioIndicatorRect(const QRectF& contentRect,
                                 const detail::RadioVisualStyle& style,
                                 qreal labelContentWidth,
...
QRectF defaultRadioIndicatorRectForRadio(const AdRadio* radio,
                                         const detail::RadioVisualStyle& style,
                                         const RadioLabelContent& labelContent,
                                         bool fill) {
  if (!radio) {
    return QRectF();
  }

  return defaultRadioIndicatorRect(QRectF(radio->rect()),
                                   style,
                                   static_cast<qreal>(labelContent.width),
                                   labelContent.hasContent(),
                                   radio->layoutDirection(),
                                   fill);
}

QRectF defaultRadioLabelRect(const QRectF& contentRect,
                             const QRectF& indicatorRect,
                             const detail::RadioVisualStyle& style,
                             Qt::LayoutDirection direction) {
  if (direction == Qt::RightToLeft) {
    const qreal textLeft = contentRect.x() + static_cast<qreal>(style.metrics.labelPaddingInlineEnd);
    const qreal textRight = indicatorRect.left() - static_cast<qreal>(style.metrics.labelPaddingInlineStart);
    return QRectF(textLeft,
                  contentRect.y(),
                  std::max<qreal>(0.0, textRight - textLeft),
                  contentRect.height());
  }

  const qreal textX = indicatorRect.right() + static_cast<qreal>(style.metrics.labelPaddingInlineStart);
  const qreal textRight = contentRect.right() + 1.0 - static_cast<qreal>(style.metrics.labelPaddingInlineEnd);
  return QRectF(textX,
                contentRect.y(),
...
void drawStyledText(const QWidget* widget,
                    QPainter* painter,
                    const QRectF& rect,
                    const QString& text,
                    const QColor& color,
  ...
  widget->style()->drawItemText(painter,
                                rect.toAlignedRect(),
                                alignment | Qt::TextShowMnemonic | Qt::TextSingleLine,
                                palette,
                                widget->isEnabled(),
                                text,
                                role);
...
void drawButtonIcon(const AdRadio* radio,
                    QPainter* painter,
                    const QRectF& rect,
                    const QSize& iconSize) {
  ...
  const QIcon::Mode mode = radio->isEnabled() ? QIcon::Normal : QIcon::Disabled;
...
detail::RadioDotStateStyle resolveDotStateStyle(const detail::RadioVisualStyle& style,
                                                bool enabled,
                                                bool checked,
                                                bool hovered,
                                                bool pressed) {
...
void AdRadio::applyEffectiveStateChange(const EffectiveStateSnapshot& before,
...
QSize AdRadio::sizeHint() const {
  ...
  if (effectiveVariant() == Variant::Button) {
    const detail::RadioButtonVisualStyle style =
        detail::resolveRadioButtonVisualStyle(buildButtonStyleInput(), resolvedTheme);
    ...
    const int height =
        std::max(style.metrics.buttonHeight, iconLogicalSize.height() + style.metrics.borderWidth * 2 + 8);
    return QSize(width, height);
  }

  const detail::RadioVisualStyle style = detail::resolveRadioVisualStyle(buildStyleInput(), resolvedTheme);
  ...
  if (labelContent.hasContent()) {
    width += style.metrics.labelPaddingInlineStart + labelContent.width + style.metrics.labelPaddingInlineEnd;
  }
  const int height =
      std::max({style.metrics.radioSize, style.metrics.textLineHeight, labelContent.iconSize.height()});
  return QSize(width, height);
}
...
void AdRadio::paintButtonVariant(QPainter* painter) const {
  ...
  if (hasVisibleBorder) {
    const QPainterPath borderPath = roundedRectPath(borderRect, topLeft, topRight, bottomRight, bottomLeft);
    QPen borderPen(state.borderColor,
  ...
  const InlineContentLayout contentLayout =
      layoutInlineContent(contentRect.adjusted(style.metrics.buttonPaddingInline,
                                               0,
                                               -style.metrics.buttonPaddingInline,
                                               0),
                          iconLogicalSize,
                          textBounds,
                          contentGapForMetrics(style.metrics),
                          layoutDirection(),
                          true);
...
void AdRadio::paintDefaultVariant(QPainter* painter) const {
  ...
  const QRectF indicatorBorderRect = snapRectToDevicePixels(indicatorRect.adjusted(iconStrokeInset,
                                                                                    iconStrokeInset,
                                                                                    -iconStrokeInset,
                                                                                    -iconStrokeInset),
                                                            dpr,
  ...
  if (isChecked()) {
    ...
    const QRectF dotRect =
        snapRectToDevicePixels(centeredSquare(indicatorBackgroundRect.center(), alignedDotSize),
                               dpr,
                               paintOrigin);
    painter->setPen(Qt::NoPen);
  ...
  if (labelContent.hasContent()) {
    const QRectF labelRect = defaultRadioLabelRect(contentRect, indicatorRect, style, layoutDirection());
    const InlineContentLayout contentLayout =
        layoutInlineContent(labelRect,
                            labelContent.iconSize,
                            labelContent.textBounds,
                            labelContent.gap,
                            layoutDirection(),
                            false);
...
void AdRadio::mouseReleaseEvent(QMouseEvent* event) {
  if (forwardedPressTarget_) {
    ...
    if (target) {
      ...
      const bool triggerTarget =
          event->button() == Qt::LeftButton && target->rect().contains(targetPos) &&
  ...
  const bool triggerWave = event->button() == Qt::LeftButton && pressed_ && rect().contains(mouseEventPos(event)) &&
                           !interactionBlocked();
  pressed_ = false;
  if (triggerWave) {
...
void AdRadio::keyPressEvent(QKeyEvent* event) {
  if (event) {
    if (AdRadioButtonGroup* const group = group_; group && group->tryHandleNavigation(this, event->key())) {
      event->accept();
      return;
    }
  ...
  const bool interactiveKey =
      event && (event->key() == Qt::Key_Space || event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter);
...
int AdRadio::horizontalSpacingHint() const {
  const adqt::theme::ResolvedTheme resolvedTheme = adqt::theme::ThemeManager::instance().resolve(this);
  if (effectiveVariant() == Variant::Button) {
    return std::max(0, detail::resolveRadioButtonVisualStyle(buildButtonStyleInput(), resolvedTheme)
                           .metrics.wrapperMarginInlineEnd);
  }
  return std::max(0, detail::resolveRadioVisualStyle(buildStyleInput(), resolvedTheme)
                         .metrics.wrapperMarginInlineEnd);
}
...
void AdRadio::syncManagedSizePolicy() {
  const QSizePolicy::Policy horizontal = effectiveFill() ? QSizePolicy::Expanding : QSizePolicy::Preferred;
  if (sizePolicy().horizontalPolicy() == horizontal && sizePolicy().verticalPolicy() == QSizePolicy::Fixed) {
    return;
  }
...
void AdRadio::applyAutomaticCursor(std::optional<Qt::CursorShape> cursorShape) {
  ...
  if (cursorShape.has_value()) {
    if (!autoCursorManaged_ || !testAttribute(Qt::WA_SetCursor) || cursor().shape() != cursorShape.value()) {
...
void AdRadio::bumpGroupZOrder() {
  if (effectiveVariant() != Variant::Button || groupPosition_ == GroupPosition::None || !isVisible()) {
    return;
  }
