...
QColor bestTextColorForBackground(const QColor& background,
                                  const QColor& lightCandidate,
...
TagMetrics baseMetrics(const QFont& baseFont, const adqt::theme::ResolvedTheme& resolvedTheme) {
  ...
  metrics.font.setPixelSize(std::max(12, qRound(map.fontSizeSM > 0.0 ? map.fontSizeSM : map.fontSize)));
...
SchemePalette resolveDefaultPalette(const AdTag::ColorTokens& tokens,
  ...
  palette.lightBg = tokens.defaultBg.value_or(toColor(map.colorFillTertiary, QColor(QStringLiteral("#f0f0f0"))));
  palette.lightBorder =
      tokens.defaultBorderColor.value_or(toColor(map.colorBorder, QColor(QStringLiteral("#d9d9d9"))));
  palette.text = tokens.defaultColor.value_or(toColor(map.colorText, QColor(QStringLiteral("#141414"))));
  palette.solidBg = toColor(map.colorBgSolid, QColor(QStringLiteral("#595959")));
  palette.solidBorder = Qt::transparent;
  palette.solidText = tokens.solidTextColor.value_or(bestTextColorForBackground(
      palette.solidBg,
      toColor(map.colorWhite, QColor(Qt::white)),
      toColor(map.colorText, QColor(Qt::black))));
  return palette;
}

SchemePalette resolveAccentPalette(AdTag::ColorScheme scheme,
                                   const QColor& customColor,
                                   const adqt::theme::ResolvedTheme& resolvedTheme) {
  const adqt::theme::ThemeMapToken& map = resolvedTheme.values;
  const QColor base = scheme == AdTag::ColorScheme::Custom ? customColor
                                                           : accentBaseColor(scheme, resolvedTheme);
...
SchemePalette resolveStatusPalette(AdTag::ColorScheme scheme,
  ...
  if (!palette.solidText.isValid()) {
    palette.solidText = bestTextColorForBackground(
        palette.solidBg,
        toColor(map.colorWhite, QColor(Qt::white)),
        toColor(map.colorText, QColor(Qt::black)));
  }
...
TagVisualStyle resolveTagVisualStyle(const TagStyleInput& input,
  ...
  const QColor colorTextDisabled =
      tokens.colorTextDisabled.value_or(toColor(map.colorTextDisabled, QColor(QStringLiteral("#bfbfbf"))));
  const QColor colorBorderDisabled =
      tokens.colorBorderDisabled.value_or(toColor(map.colorBorderDisabled, QColor(QStringLiteral("#d9d9d9"))));
  ...
  const QColor colorFillSecondary = toColor(map.colorFillSecondary, QColor(QStringLiteral("#f5f5f5")));
  const QColor colorPrimary = toColor(map.colorPrimary, QColor(QStringLiteral("#1677ff")));
  const QColor colorPrimaryHover = toColor(map.colorPrimaryHover, QColor(QStringLiteral("#4096ff")));
  const QColor colorPrimaryActive = toColor(map.colorPrimaryActive, QColor(QStringLiteral("#0958d9")));
  const QColor colorTextLightSolid =
      toColor(map.colorTextLightSolid, QColor(QStringLiteral("#ffffff")));
  ...
  style.borderPenStyle = input.borderStyle == AdTag::BorderStyle::Dashed ? Qt::DashLine : Qt::SolidLine;
  ...
  if (input.checkable) {
    ...
    style.backgroundColor = colorBgDisabled;
    style.borderColor = input.variant == AdTag::Variant::Outlined ? colorBorderDisabled : Qt::transparent;
    style.contentColor = colorTextDisabled;
    style.iconColor = colorTextDisabled;
  } else {
  ...
  style.closeColor = input.disabled
                         ? colorTextDisabled
                         : tokens.closeColor.value_or(toColor(map.colorTextTertiary,
                                                              withAlpha(style.contentColor, 180)));
  ...
  style.closeHoverBackground = input.disabled
                                   ? Qt::transparent
                                   : toColor(tokens.closeHoverBackground.value_or(Qt::transparent),
                                             Qt::transparent);
