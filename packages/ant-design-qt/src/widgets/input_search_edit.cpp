...
class SearchSeparatorOverlay final : public QWidget {
 ...
 protected:
  void paintEvent(QPaintEvent* event) override {
    ...
    const qreal x =
        detail::input_internal::snapToDevicePixelCoord(static_cast<qreal>(padding_) + penWidth / 2.0, dpr);
...
QColor resolveSearchSeparatorColor(const adqt::theme::ThemeMapToken& map,
                                   AdLineEdit::Status status,
                                   bool enabled,
                                   bool inputActiveBorder) {
  if (!enabled) {
...
bool AdSearchEdit::joinedRight() const {
  ...
  return position == detail::SegmentPosition::Leading || position == detail::SegmentPosition::Middle;
}
...
void AdSearchEdit::updateButtonVisual() {
  ...
  button_->setEnabled(isEnabled());
  detail::setButtonSegmentPosition(button_,
                                   joinedRight() ? detail::SegmentPosition::Middle
                                                 : detail::SegmentPosition::Trailing);
...
void AdSearchEdit::updateSeparatorVisual() {
  ...
  const int separatorX = std::clamp(buttonRect.left() - overlayPadding, 0,
                                    std::max(0, width() - overlayWidth));
