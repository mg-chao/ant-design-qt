...
struct IconPixmapCacheKey {
  adqt::icons::IconId id;
  QSize size;
  int dprMilli = 1000;
  int mode = 0;
  adqt::icons::IconColorOverrides colors;

  bool operator==(const IconPixmapCacheKey& other) const {
    return id == other.id && size == other.size && dprMilli == other.dprMilli &&
           mode == other.mode && colors == other.colors;
  }
};
...
QModelIndex findIndexByStableId(const QAbstractItemModel* model,
...
QRect horizontalIndicatorContentRect(const QRect& contentRect,
  ...
  if (textWidth > 0 && textRect.isValid()) {
    ...
    if (clampedTextWidth > 0) {
      const QRect paintedTextRect(textRect.left(), textRect.top(), clampedTextWidth, textRect.height());
      occupiedRect = occupiedRect.isValid() ? occupiedRect.united(paintedTextRect) : paintedTextRect;
    }
  }
...
QRect horizontalActiveBarRect(const QRect& rowRect,
  ...
  if (!occupiedRect.isValid() || occupiedRect.isEmpty()) {
    return QRect(contentRect.left(),
                 rowRect.bottom() - barHeight + 1,
                 std::max(0, contentRect.width()),
                 barHeight);
  }

  return QRect(occupiedRect.left(),
               rowRect.bottom() - barHeight + 1,
               std::max(0, occupiedRect.width()),
               barHeight);
}
...
int collapsedInlineRootWidth(const MenuVisualStyle& style) {
  ...
  const int contentDrivenWidth = collapsedIconSize + std::max(0, style.metrics.itemPaddingInline) * 2 + insetWidth;
...
void paintMenuIcon(QPainter& painter,
  ...
  if ((!icon.colors.hasPrimary || !icon.colors.primary.isValid()) && color.isValid()) {
...
bool isSeparatorIndex(const QModelIndex& sourceIndex) {
  return sourceIndex.isValid() && nodeTypeForIndex(sourceIndex) == AdNavigationMenu::NodeKind::Separator;
}

bool isGroupIndex(const QModelIndex& sourceIndex) {
  return sourceIndex.isValid() && nodeTypeForIndex(sourceIndex) == AdNavigationMenu::NodeKind::Group;
}
...
bool isInteractiveIndex(const QModelIndex& sourceIndex) {
  return sourceIndex.isValid() && !isSeparatorIndex(sourceIndex) &&
         (sourceIndex.flags() & Qt::ItemIsEnabled) && (isActionIndex(sourceIndex) || isSubmenuIndex(sourceIndex));
}
...
int menuRowBoundarySpacing(const MenuVisualStyle& style,
                           const QModelIndex& sourceIndex,
                           const QModelIndex& previousSourceIndex) {
  if (!sourceIndex.isValid() || !previousSourceIndex.isValid() || !isMenuRowIndex(sourceIndex) ||
      !isMenuRowIndex(previousSourceIndex)) {
    return 0;
  }
  return sourceIndex.parent() != previousSourceIndex.parent() ? menuRowBaseSpacing(style, sourceIndex) : 0;
}

int menuRowLeadingSpacing(const MenuVisualStyle& style,
                          const QModelIndex& sourceIndex,
                          const QModelIndex& previousSourceIndex) {
...
bool shouldShowSubMenuArrow(AdNavigationMenu::Mode mode, bool collapsed, const QModelIndex& sourceIndex) {
...
QString tooltipTextForIndex(const QModelIndex& sourceIndex) {
  const QString explicitText = sourceIndex.data(Qt::ToolTipRole).toString().trimmed();
...
AdNavigationMenu::ColorScheme popupColorSchemeForIndex(const QModelIndex& sourceIndex, AdNavigationMenu::ColorScheme fallback) {
...
int rootBorderWidthForStyle(AdNavigationMenu::Mode mode,
                            AdNavigationMenu::ColorScheme colorScheme,
                            adqt::theme::ThemeScheme themeScheme,
...
void paintMenuRootBorder(const QWidget* view, QWidget* surface, const QRect& dirtyRect) {
  ...
  if (!borderColor.isValid() || borderColor.alpha() <= 0 || borderWidth <= 0 || borderEdge == kMenuRootBorderNone) {
    return;
  }
  ...
  if (borderEdge == kMenuRootBorderBottom) {
    painter.fillRect(QRect(0, std::max(0, surface->height() - borderWidth), surface->width(), borderWidth),
                     borderColor);
  } else if (borderEdge == kMenuRootBorderRight) {
    painter.fillRect(QRect(std::max(0, surface->width() - borderWidth), 0, borderWidth, surface->height()),
                     borderColor);
  }
}
...
class AdMenuTreeView final : public QTreeView {
 public:
  explicit AdMenuTreeView(AdNavigationMenu* owner, QWidget* parent = nullptr) : QTreeView(parent), owner_(owner) {}
  ...
  QRect visualRect(const QModelIndex& index) const override {
    ...
    if (!rawRect.isValid() || !owner_ || !property("AdNavigationMenu.popupLevel").toBool()) {
      return rawRect;
    }

    const AdNavigationMenu::ColorScheme popupColorScheme =
        static_cast<AdNavigationMenu::ColorScheme>(property("AdNavigationMenu.popupColorScheme").toInt());
    const MenuVisualStyle style = owner_->resolvedVisualStyle(AdNavigationMenu::Mode::Vertical, popupColorScheme, false);
    ...
    const int topSpacing = menuRowLeadingSpacing(style, sourceIndex, previousSourceIndex);
  ...
  void paintEvent(QPaintEvent* event) override {
    ...
    if (owner_) {
      const AdNavigationMenu::Mode mode = static_cast<AdNavigationMenu::Mode>(property("AdNavigationMenu.mode").toInt());
      const bool collapsed = property("AdNavigationMenu.collapsed").toBool();
      popupLevel = property("AdNavigationMenu.popupLevel").toBool();
      const AdNavigationMenu::ColorScheme popupColorScheme =
          static_cast<AdNavigationMenu::ColorScheme>(property("AdNavigationMenu.popupColorScheme").toInt());
      if (popupLevel && viewport()) {
        popupStyle = owner_->resolvedVisualStyle(AdNavigationMenu::Mode::Vertical, popupColorScheme, false);
        popupRadius = std::max<qreal>(0.0, static_cast<qreal>(popupStyle.metrics.popupBorderRadius));
        ...
        if (mode == AdNavigationMenu::Mode::Inline && !collapsed) {
          const MenuVisualStyle style = owner_->resolvedVisualStyle(AdNavigationMenu::Mode::Inline, popupColorScheme, false);
          if (style.subMenuBackground.alpha() > 0 && model() && viewport()) {
            QPainter painter(viewport());
            const QRect dirtyRect = event->rect();
            painter.setClipRect(dirtyRect);
            auto isDescendantOf = [](QModelIndex index, const QModelIndex& ancestor) {
              while (index.isValid()) {
                if (index == ancestor) {
                  return true;
                }
            ...
            auto sourceAndPreviousForIndex = [this](const QModelIndex& index) {
              const QModelIndex sourceIndex = mapToSourceIndex(index);
              const QModelIndex previousIndex = indexAbove(index);
              const QModelIndex previousSourceIndex =
                  previousIndex.isValid() ? mapToSourceIndex(previousIndex) : QModelIndex();
              return qMakePair(sourceIndex, previousSourceIndex);
            };
            auto blockTopInsetForIndex = [&sourceAndPreviousForIndex, &style](const QModelIndex& index) {
              if (!index.isValid()) {
                return 0;
              }
              const auto [sourceIndex, previousSourceIndex] = sourceAndPreviousForIndex(index);
              return menuRowBoundarySpacing(style, sourceIndex, previousSourceIndex);
            };
            auto blockBottomOutsetForIndex = [&sourceAndPreviousForIndex, &style](const QModelIndex& index) {
              if (!index.isValid()) {
                return 0;
              }
              const auto [sourceIndex, _] = sourceAndPreviousForIndex(index);
              return menuRowBaseSpacing(style, sourceIndex);
            };
            std::function<void(const QModelIndex&)> paintExpandedBlocks;
            paintExpandedBlocks = [this,
                                   &painter,
                                   &style,
                                   &isDescendantOf,
                                   &blockTopInsetForIndex,
                                   &blockBottomOutsetForIndex,
                                   &paintExpandedBlocks,
                                   dirtyRect](const QModelIndex& parent) {
              if (!model()) {
                return;
              }
              const int rowCount = model()->rowCount(parent);
              for (int row = 0; row < rowCount; ++row) {
                ...
                if (isGroupIndex(child)) {
                  paintExpandedBlocks(child);
                  continue;
                }
                if (!isSubmenuIndex(child) || !isExpanded(child)) {
                  continue;
                }
                const QModelIndex firstVisible = indexBelow(child);
                if (!firstVisible.isValid() || !isDescendantOf(firstVisible, child)) {
                  continue;
                }
                QModelIndex lastVisible = firstVisible;
                QModelIndex next = indexBelow(firstVisible);
                while (next.isValid() && isDescendantOf(next, child)) {
                  lastVisible = next;
                  next = indexBelow(next);
                }
                const QRect firstRect = visualRect(firstVisible);
                const QRect lastRect = visualRect(lastVisible);
                if (firstRect.isValid() && lastRect.isValid()) {
                  const int blockTop = firstRect.top() + blockTopInsetForIndex(firstVisible);
                  const int blockBottom = lastRect.bottom() + blockBottomOutsetForIndex(lastVisible);
                  const QRect blockRect = QRect(0,
                                                blockTop,
                                                viewport()->width(),
                                                blockBottom - blockTop + 1)
                                              .intersected(viewport()->rect())
                                              .intersected(dirtyRect);
                  if (!blockRect.isEmpty()) {
                    painter.fillRect(blockRect, style.subMenuBackground);
                  }
                }
                paintExpandedBlocks(child);
              }
            };
            paintExpandedBlocks(rootIndex());
          }
        }
      }
    }
    ...
    if (popupLevel && viewport() && popupStyle.metrics.borderWidth > 0 && popupStyle.popupBorderColor.alpha() > 0) {
 ...
 public:
  ...
  void setSourceModel(QAbstractItemModel* sourceModel) override {
    ...
    if (sourceModel) {
      ...
      connections_.append(QObject::connect(sourceModel, &QAbstractItemModel::modelReset, this, reset));
      connections_.append(QObject::connect(sourceModel, &QAbstractItemModel::layoutChanged, this, reset));
      connections_.append(QObject::connect(sourceModel, &QAbstractItemModel::rowsInserted, this, reset));
      connections_.append(QObject::connect(sourceModel, &QAbstractItemModel::rowsRemoved, this, reset));
      connections_.append(QObject::connect(sourceModel, &QAbstractItemModel::rowsMoved, this, reset));
      connections_.append(QObject::connect(sourceModel, &QAbstractItemModel::dataChanged, this,
                                           [this](const QModelIndex& topLeft,
                                             ...
                                             emit dataChanged(index(topLeft.row(), 0, QModelIndex()),
                                                              index(bottomRight.row(), 0, QModelIndex()),
                                                              roles);
                                           }));
    }
    endResetModel();
  ...
  QModelIndex mapFromSource(const QModelIndex& sourceIndex) const override {
    if (!sourceModel() || !sourceIndex.isValid() || sourceIndex.parent().isValid() || sourceIndex.column() != 0) {
      return QModelIndex();
    }
  ...
  QModelIndex index(int row, int column, const QModelIndex& parent = QModelIndex()) const override {
    if (parent.isValid() || !sourceModel() || column != 0 || row < 0 || row >= sourceModel()->rowCount()) {
      return QModelIndex();
    }
...
class AdMenuBarView final : public QListView {
 public:
  explicit AdMenuBarView(QWidget* parent = nullptr) : QListView(parent) {}
 ...
 protected:
  ...
  void paintEvent(QPaintEvent* event) override {
    if (viewport()) {
      paintMenuRootBackground(this, viewport(), event->rect());
      // Match Ant Design stacking: horizontal item indicators should paint over the root bottom border.
      paintMenuRootBorder(this, viewport(), event->rect());
    }
...
class AdMenuPopupShell final : public QWidget {
 ...
 protected:
  void paintEvent(QPaintEvent* event) override {
    ...
    const MenuVisualStyle style = owner_ ? owner_->resolvedVisualStyle(AdNavigationMenu::Mode::Vertical, popupColorScheme_, false)
...
class AdNavigationMenu::Private {
 public:
  ...
  QStringList filterKnownStableIds(const QStringList& keys, bool submenuOnly, const char* context) const;
  ...
  void syncDelegates();
  void updateViewProperties(QWidget* view,
                            Mode viewMode,
                            bool collapsedValue,
  ...
  QSize popupContentSize(const QModelIndex& submenuIndex,
                         ColorScheme popupColorScheme,
                         const QWidget* measuringWidget = nullptr) const;
  QSize rootContentSize() const;
  void appendVisibleRows(const QModelIndex& parentIndex,
                         bool inlineMode,
  ...
  void activateSourceIndex(const QModelIndex& sourceIndex,
                           bool fromKeyboard,
  ...
  QModelIndex nextNavigableIndex(detail::AdMenuTreeView* view, const QModelIndex& start, int delta) const;
  ...
  static void applyExpandedStateChange(QStringList& indexes,
...
bool AdNavigationMenu::Private::popupLikeMode() const {
  return mode != Mode::Inline || collapsed;
}
...
void AdNavigationMenu::Private::rebuildStableIdIndexCache() {
  state.rebuildStableIdIndexCache();
}
...
void AdNavigationMenu::Private::syncModelConnections() {
  ...
  const auto refreshData = [this, refreshStructure](const QModelIndex&,
                                                    const QModelIndex&,
                                                    const QList<int>& roles) {
    const bool unknownRoles = roles.isEmpty();
    const bool requiresStructureRefresh =
        unknownRoles || roles.contains(AdNavigationMenu::StableIdRole) ||
    ...
    const bool affectsPopup =
        roles.contains(AdNavigationMenu::PopupColorSchemeRole) ||
  ...
  modelConnections.append(QObject::connect(model, &QAbstractItemModel::modelReset, q, refreshStructure));
  modelConnections.append(QObject::connect(model, &QAbstractItemModel::layoutChanged, q, refreshStructure));
  modelConnections.append(QObject::connect(model, &QAbstractItemModel::rowsInserted, q,
                                           [refreshStructure](const QModelIndex&, int, int) {
                                             refreshStructure();
                                           }));
  modelConnections.append(QObject::connect(model, &QAbstractItemModel::rowsRemoved, q,
                                           [refreshStructure](const QModelIndex&, int, int) {
                                             refreshStructure();
                                           }));
  modelConnections.append(QObject::connect(model, &QAbstractItemModel::rowsMoved, q,
                                           [refreshStructure](const QModelIndex&,
                                                              int,
                                                              int,
                                                              const QModelIndex&,
                                                              int) {
...
void AdNavigationMenu::Private::syncSelectionConnections() {
  ...
  selectionConnections.append(QObject::connect(selectionModel, &QItemSelectionModel::selectionChanged, q,
                                               [this](const QItemSelection&, const QItemSelection&) {
                                                 syncBarSelectionState();
                                                 updateAllViewports();
                                               }));
  selectionConnections.append(QObject::connect(selectionModel, &QItemSelectionModel::currentChanged, q,
...
void AdNavigationMenu::Private::rebuildRootViews() {
  ...
  if (!inlineView) {
    ...
    QObject::connect(inlineView, &QTreeView::clicked, q, [this](const QModelIndex& index) {
  ...
  if (!verticalView) {
    ...
    verticalView->keyHandler = [this](QKeyEvent* event) { return handleTreeKey(verticalView, event); };
    ...
    QObject::connect(verticalView, &QTreeView::clicked, q, [this](const QModelIndex& index) {
      activateSourceIndex(index, false, true);
    });
...
void AdNavigationMenu::Private::updateViewProperties(QWidget* view,
                                   Mode viewMode,
                                   bool collapsedValue,
                                   ColorScheme popupColorScheme,
                                   int rootDepth,
  ...
  const MenuVisualStyle style =
      q->resolvedVisualStyle(popupLevel ? Mode::Vertical : viewMode, popupColorScheme, collapsedValue);
...
void AdNavigationMenu::Private::syncTreeView(detail::AdMenuTreeView* view, const QModelIndex& rootIndex, bool inlineMode) {
  if (!view) {
    return;
  }
  if (view->model() != model) {
  ...
  const Mode rootViewMode = inlineMode ? Mode::Inline
                                       : ((mode == Mode::Inline && collapsed && !popupLevel)
                                              ? Mode::Inline
                                              : Mode::Vertical);
  updateViewProperties(view,
                       rootViewMode,
                       inlineMode ? false : collapsed,
                       colorScheme,
                       popupLevel ? sourceDepth(rootIndex) + 1 : 0,
                       popupLevel);
...
void AdNavigationMenu::Private::syncViewSelectionModels() {
  ...
  if (inlineView && inlineView->model() == model && inlineView->selectionModel() != sourceSelection) {
    inlineView->setSelectionModel(sourceSelection);
  }
  if (verticalView && verticalView->model() == model && verticalView->selectionModel() != sourceSelection) {
...
void AdNavigationMenu::Private::syncBarSelectionState() {
  ...
  const QModelIndex currentProxy =
      currentSource.isValid() ? mapFromSourceIndex(barProxyModel, topLevelAncestor(currentSource)) : QModelIndex();
...
void AdNavigationMenu::Private::normalizeExpandedStableIds(QStringList& keys) const {
  state.normalizeExpandedStableIds(keys, [](const QModelIndex& index) { return isSubmenuIndex(index); });
}
...
void AdNavigationMenu::Private::applyExpandedStateChange(QStringList& targetExpanded,
  ...
  auto removeIndex = [&targetExpanded](const QString& targetKey) {
    for (int i = targetExpanded.size() - 1; i >= 0; --i) {
  ...
  auto removeWithDescendants = [&targetExpanded, self](const QModelIndex& parentIndex) {
    for (int i = targetExpanded.size() - 1; i >= 0; --i) {
      const QModelIndex candidate = self ? self->indexForStableId(targetExpanded.at(i)) : QModelIndex();
...
void AdNavigationMenu::Private::setExpandedInternal(const QModelIndex& index, bool expanded, bool emitSignal) {
  if (!isSubmenuIndex(index)) {
    return;
  }
...
void AdNavigationMenu::Private::ensurePopupLevel(int levelIndex) {
  while (static_cast<int>(popupLevels.size()) <= levelIndex) {
    ...
    popup->shell->setObjectName(levelIndex == 0 ? QStringLiteral("AdNavigationMenu-popup-window")
    ...
    popup->view->keyHandler = [this, view = popup->view.data()](QKeyEvent* event) { return handleTreeKey(view, event); };
    ...
    QObject::connect(popup->view, &QTreeView::clicked, q, [this](const QModelIndex& index) {
      activateSourceIndex(index, false, true);
    });
    ...
    QObject::connect(popup->view, &QTreeView::entered, q, [this, popupView = popup->view.data()](const QModelIndex& index) {
...
void AdNavigationMenu::Private::syncPopupLevel(PopupLevel& level, const QModelIndex& submenuIndex) {
  ...
  updateViewProperties(level.view,
                       Mode::Vertical,
                       false,
                       level.popupColorScheme,
                       sourceDepth(submenuIndex) + 1,
                       true);
  ...
  if (wantsCustomPopup) {
    ...
    QWidget* custom = popupFactory ? popupFactory->createPopup(ctx, defaultPopup, level.shell) : nullptr;
    if (custom) {
...
QSize AdNavigationMenu::Private::popupContentSize(const QModelIndex& submenuIndex,
  ...
  QAbstractItemDelegate* activeDelegate = effectiveDelegate();
  if (!activeDelegate) {
    return QSize(width, std::max(style.metrics.itemHeight * 2, style.metrics.itemHeight));
  }
  ...
  return QSize(width + std::max(0, style.metrics.borderWidth) * 2,
               std::max(height, style.metrics.itemHeight * 2));
}
...
QSize AdNavigationMenu::Private::rootContentSize() const {
  ...
  if (!model) {
    if (mode == Mode::Horizontal) {
      return QSize(160,
                   std::max(1, style.metrics.horizontalLineHeight) +
                       rootBorderWidthForStyle(mode, colorScheme, themeScheme, style));
    }
    return QSize(collapsed ? collapsedWidth : 256, style.metrics.itemHeight + style.metrics.itemMarginBlock * 2);
  }
  QVector<QModelIndex> rows;
  ...
  for (const QModelIndex& row : rows) {
    ...
    if (mode == Mode::Horizontal) {
      ...
      height = std::max(height,
                        hint.height() + rootBorderWidthForStyle(mode, colorScheme, themeScheme, style));
    } else {
      width = std::max(width, hint.width());
      height += hint.height();
      lastRow = row;
  ...
  if (mode == Mode::Horizontal) {
    return QSize(std::max(160, width),
                 std::max(height,
                          std::max(1, style.metrics.horizontalLineHeight) +
                              rootBorderWidthForStyle(mode, colorScheme, themeScheme, style)));
  }
...
void AdNavigationMenu::Private::appendVisibleRows(const QModelIndex& parentIndex,
                                bool inlineMode,
                                QVector<QModelIndex>& out) const {
  if (!model) {
    return;
  ...
  for (int row = 0; row < rowCount; ++row) {
    ...
    } else if (inlineMode && isSubmenuIndex(child) && containsExpanded(child)) {
      appendVisibleRows(child, inlineMode, out);
    }
  }
}
...
bool AdNavigationMenu::Private::isSourceIndexSelected(const QModelIndex& sourceIndex) const {
  if (!sourceIndex.isValid() || isSeparatorIndex(sourceIndex) || isGroupIndex(sourceIndex) || !selectionModel) {
    return false;
  }
  return selectionModel->isSelected(sourceIndex) || (isSubmenuIndex(sourceIndex) && hasSelectedDescendant(sourceIndex));
}

bool AdNavigationMenu::Private::hasSelectedDescendantForStyle(const QModelIndex& sourceIndex) const {
...
bool AdNavigationMenu::Private::hasFocusWithin() const {
  ...
  for (const auto& level : popupLevels) {
    ...
    if (focusWidget == level->shell || level->shell->isAncestorOf(focusWidget)) {
      return true;
    }
  }
  return false;
...
bool AdNavigationMenu::Private::applySelectionTrigger(const QModelIndex& sourceIndex) {
  ...
  sm->select(sourceIndex, (alreadySelected ? QItemSelectionModel::Deselect : QItemSelectionModel::Select) |
                              QItemSelectionModel::Rows);
  return true;
}
...
void AdNavigationMenu::Private::setCurrentIndexInternal(const QModelIndex& sourceIndex, bool emitSignal) {
  QItemSelectionModel* sm = ensureSelectionModel();
  if (!sm) {
    return;
  }
...
void AdNavigationMenu::Private::activateSourceIndex(const QModelIndex& sourceIndex,
  ...
  setCurrentIndexInternal(sourceIndex, false);
  if (isSubmenuIndex(sourceIndex)) {
    if (mode == Mode::Inline && !collapsed) {
...
void AdNavigationMenu::Private::cancelHoverClose() {
  hoverCloseTimer.stop();
}
...
void AdNavigationMenu::Private::closeDanglingPopups() {
  ...
  for (const auto& level : popupLevels) {
    if (level && level->shell && level->shell->isVisible() && widgetContainsGlobalPos(level->shell, cursorPos)) {
      return;
    }
  }
  clearPopupExpandedSubmenus();
...
void AdNavigationMenu::Private::handleHoveredIndex(const QModelIndex& sourceIndex) {
  ...
  const bool hoveredCollapsedInlineLeaf =
      mode == Mode::Inline && collapsed && sourceIndex.isValid() && !sourceIndex.parent().isValid() &&
      !isSubmenuIndex(sourceIndex);
  if (hoveredCollapsedInlineLeaf && popupLikeMode() && submenuTrigger == TriggerSubMenuAction::Hover) {
    pendingHoverIndex = QModelIndex();
    hoverOpenTimer.stop();
    hideTooltip();
...
void AdNavigationMenu::Private::updateCursorForView(QWidget* target, const QModelIndex& sourceIndex) {
  ...
  const bool cursorEligible = sourceIndex.isValid() && (isActionIndex(sourceIndex) || isSubmenuIndex(sourceIndex));
  if (!cursorEligible) {
...
QModelIndex AdNavigationMenu::Private::barAdjacentIndex(const QModelIndex& start, int delta) const {
  ...
  int row = start.isValid() ? start.row() + delta : (delta >= 0 ? 0 : barView->model()->rowCount() - 1);
...
void AdNavigationMenu::Private::syncPopupVisibility() {
  if (!popupLikeMode()) {
    hidePopupLevelsFrom(0);
    detail::setInWindowPopupHostOpen(q, false);
    return;
  }
  ...
  if (mode == Mode::Inline && collapsed) {
    const auto popupChainAnchoredByInteraction = [this, &chain]() {
      const QModelIndex currentSource = selectionModel ? selectionModel->currentIndex() : QModelIndex();
      ...
      if (currentAnchorView && currentAnchorView->viewport()) {
        ...
        const QModelIndex hoveredSource = currentAnchorView == barView ? mapToSourceIndex(viewHoveredIndex)
                                                                      : viewHoveredIndex;
      ...
      for (const auto& level : popupLevels) {
        ...
        if (level->shell->rect().contains(popupCursor)) {
          return true;
        }
      }
      return false;
  ...
  for (int i = 0; i < chain.size(); ++i) {
    ...
    if (popupLayout) {
      // Keep the shell sized to the visible popup panel. Horizontal root gaps are applied after
      // placement so geometry and the rendered panel edge move together.
    ...
    const int sidePopupGap = horizontalRootPopup
                                 ? 0
                                 : std::max(0,
                                            placementStyle.metrics.popupPlacementGap -
    ...
    QSize popupSize = measurePopupShell();

    detail::PopupPlacementInput placementInput;
    placementInput.anchorTopLeft = currentAnchorView->viewport()->mapTo(scopeWindow, anchorRect.topLeft());
    if (horizontalRootPopup) {
    ...
    const QPoint baseOffset = q->popupOffset() + chain.at(i).data(AdNavigationMenu::PopupOffsetRole).toPoint();
    ...
    if (level.shell) {
      level.shell->move(placementOutput.topLeft);
      level.shell->show();
      level.shell->raise();
    }
  }
  hidePopupLevelsFrom(chain.size());
  detail::setInWindowPopupHostOpen(q, true);
}

int measureMenuItemWidth(const QModelIndex& sourceIndex,
                         const MenuVisualStyle& style,
                         AdNavigationMenu::Mode mode,
                         bool collapsed,
                         int indentStep,
                         int relativeDepth,
                         bool popupLevel) {
  ...
  if (mode == AdNavigationMenu::Mode::Inline && !collapsed) {
    ...
    width += std::max(0, relativeDepth * (style.metrics.itemPaddingInline - style.metrics.itemMarginInline));
  }
  ...
  if (shouldShowSubMenuArrow(mode, collapsed, sourceIndex) || (popupLevel && isSubmenuIndex(sourceIndex))) {
    width += kSubMenuArrowBoxWidth + kSubMenuArrowTextGap;
  }
  return width;
}

int menuItemBaseHeight(const QModelIndex& sourceIndex,
                       const MenuVisualStyle& style,
                       AdNavigationMenu::Mode mode,
                       bool popupLevel) {
  if (isSeparatorIndex(sourceIndex)) {
...
int menuItemLeadingSpacing(const QWidget* widget,
                           const QModelIndex& index,
                           const QModelIndex& sourceIndex,
                           const MenuVisualStyle& style,
                           AdNavigationMenu::Mode mode,
...
QSize AdNavigationMenuItemDelegate::sizeHint(const QStyleOptionViewItem& option,
  ...
  const AdNavigationMenu::Mode mode = widget ? static_cast<AdNavigationMenu::Mode>(widget->property("AdNavigationMenu.mode").toInt())
                                   : owner_->mode();
  const bool collapsed = widget ? widget->property("AdNavigationMenu.collapsed").toBool() : owner_->collapsed();
  ...
  const AdNavigationMenu::ColorScheme colorScheme =
      widget ? static_cast<AdNavigationMenu::ColorScheme>(widget->property("AdNavigationMenu.popupColorScheme").toInt())
             : owner_->colorScheme();
  const MenuVisualStyle style =
      owner_->resolvedVisualStyle(popupLevel ? AdNavigationMenu::Mode::Vertical : mode, colorScheme, collapsed);
  const int relativeDepth = std::max(0, sourceDepth(sourceIndex) - rootDepth);
  const int height = menuItemBaseHeight(sourceIndex, style, mode, popupLevel) +
                     menuItemLeadingSpacing(widget, index, sourceIndex, style, mode, popupLevel);
  return QSize(measureMenuItemWidth(sourceIndex,
                                    style,
                                    mode,
                                    collapsed,
                                    owner_->effectiveIndentation(style),
                                    relativeDepth,
                                    popupLevel),
               height);
}

void AdNavigationMenuItemDelegate::paint(QPainter* painter,
  ...
  const int relativeDepth = std::max(0, sourceDepth(sourceIndex) - rootDepth);
  const int topSpacing = menuItemLeadingSpacing(widget, index, sourceIndex, style, mode, popupLevel);
  const QRect contentRowRect = rowRect.adjusted(0, topSpacing, 0, 0);
  ...
  const bool itemSelected = option.state.testFlag(QStyle::State_Selected) && isActionIndex(sourceIndex);
  ...
  QRect fillRect = contentRowRect.adjusted(style.metrics.itemMarginInline, 0, -style.metrics.itemMarginInline, 0);
  qreal radius = horizontal ? style.metrics.horizontalItemBorderRadius
  ...
  if (!horizontal && style.metrics.activeBarWidth > 0 && itemSelected && !collapsed) {
    ...
    const int activeBarLeft = rtl ? fillRect.left() : fillRect.right() - style.metrics.activeBarWidth + 1;
    painter->drawRoundedRect(QRect(activeBarLeft, fillRect.top(), style.metrics.activeBarWidth, fillRect.height()),
  ...
  if (mode == AdNavigationMenu::Mode::Inline && !collapsed) {
    ...
    indent = relativeDepth * std::max(0, style.metrics.itemPaddingInline - style.metrics.itemMarginInline);
  }
  QRect logicalContentRect = fillRect.adjusted(style.metrics.itemPaddingInline + indent, 0,
                                               -style.metrics.itemPaddingInline, 0);
  if (horizontal) {
    logicalContentRect = fillRect.adjusted(style.metrics.itemPaddingInline, 0,
                                           -style.metrics.itemPaddingInline, 0);
  }
  const bool collapsedInlineRoot =
      !horizontal && mode == AdNavigationMenu::Mode::Inline && collapsed && !popupLevel && relativeDepth == 0;
  ...
  const int iconSide =
      collapsedInlineRoot ? std::max(10, style.metrics.collapsedIconSize) : std::max(10, style.metrics.iconSize);
  const adqt::icons::IconRef icon = iconRefForIndex(sourceIndex);
  const bool hasIcon = adqt::icons::isValid(icon);
  QRect logicalIconRect(logicalContentRect.left(), logicalContentRect.center().y() - iconSide / 2, iconSide, iconSide);
  if (collapsedInlineRoot) {
  ...
  const bool showArrow =
      shouldShowSubMenuArrow(mode, collapsed, sourceIndex) || (popupLevel && isSubmenuIndex(sourceIndex));
  QRect logicalArrowRect(logicalContentRect.right() - kSubMenuArrowBoxWidth,
                         logicalContentRect.center().y() - kSubMenuArrowBoxHeight / 2,
                         kSubMenuArrowBoxWidth,
                         kSubMenuArrowBoxHeight);
  const QRect arrowRect = QStyle::visualRect(direction, fillRect, logicalArrowRect);
  int textRight = showArrow ? logicalArrowRect.left() - kSubMenuArrowTextGap : logicalContentRect.right();

  const QString extra = sourceIndex.data(AdNavigationMenu::ExtraTextRole).toString();
  if (!extra.isEmpty() && !horizontal && !(mode == AdNavigationMenu::Mode::Inline && collapsed)) {
    painter->setPen(textColor);
    const int extraWidth = painter->fontMetrics().horizontalAdvance(extra) + 8;
    QRect logicalExtraRect(std::max(textLeft, textRight - extraWidth),
                           logicalContentRect.top(),
                           extraWidth,
    ...
    painter->drawText(extraRect,
                      QStyle::visualAlignment(direction, Qt::AlignVCenter | Qt::AlignRight),
                      extra);
  ...
  const QRect logicalTextRect(textLeft,
                              logicalContentRect.top(),
                              std::max(0, textRight - textLeft),
                              logicalContentRect.height());
  const QRect textRect = QStyle::visualRect(direction, fillRect, logicalTextRect);
  const int textFlags = collapsedInlineRoot && !hasIcon ? Qt::AlignCenter
                                                        : QStyle::visualAlignment(direction,
                                                                                  Qt::AlignVCenter | Qt::AlignLeft);
  const QFontMetrics textMetrics(textFont);
  const int paintedTextWidth =
      label.isEmpty() ? 0 : std::max(textMetrics.horizontalAdvance(label), textMetrics.boundingRect(label).width());

  if (horizontal && style.metrics.activeBarHeight > 0 && (selected || hovered || pressed || opened) && !disabled) {
    const QRect indicatorRect = horizontalActiveBarRect(
        rowRect, contentRect, iconRect, hasIcon, textRect, paintedTextWidth, style.metrics.activeBarHeight);
    if (indicatorRect.isValid() && !indicatorRect.isEmpty()) {
      painter->setPen(Qt::NoPen);
      painter->setBrush(style.horizontalSelected.text.isValid() ? style.horizontalSelected.text : textColor);
...
AdNavigationMenu::SelectionMode AdNavigationMenu::selectionMode() const { return d_->selectionMode; }
...
QAbstractItemDelegate* AdNavigationMenu::itemDelegate() const {
  return d_->effectiveDelegate();
}
...
AdNavigationMenuPopupFactory* AdNavigationMenu::popupFactory() const {
  return d_->popupFactory;
}
...
void AdNavigationMenu::setSelectionModel(QItemSelectionModel* model) {
  ...
  if (model && d_->model != model->model()) {
    setModel(const_cast<QAbstractItemModel*>(model->model()));
  }
...
QObject* AdNavigationMenu::popupOwnerObject() const {
  return const_cast<AdNavigationMenu*>(this);
}

QWidget* AdNavigationMenu::popupAnchorWidget() const {
  return const_cast<AdNavigationMenu*>(this);
}
...
detail::MenuVisualStyle AdNavigationMenu::resolvedVisualStyle(Mode mode,
                                                              ColorScheme colorScheme,
                                                              bool collapsed) const {
  const adqt::theme::ResolvedTheme resolvedTheme = adqt::theme::ThemeManager::instance().resolve(this);
