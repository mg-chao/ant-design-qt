...
MenuNode actionNode(const QString& id,
                    const QString& label,
...
MenuNode submenuNode(const QString& id,
                     const QString& label,
                     const QVector<MenuNode>& children,
                     const adqt::icons::IconRef& icon = {}) {
...
QStandardItem* buildMenuItem(const MenuNode& node) {
  ...
  switch (node.kind) {
    ...
    case MenuNode::Kind::Group:
      item->setData(static_cast<int>(AdNavigationMenu::NodeKind::Group), AdNavigationMenu::NodeKindRole);
      item->setSelectable(false);
      item->setEnabled(true);
      break;
    case MenuNode::Kind::Separator:
      item->setData(static_cast<int>(AdNavigationMenu::NodeKind::Separator), AdNavigationMenu::NodeKindRole);
...
QItemSelectionModel* attachMenuModel(
    AdNavigationMenu* menu,
    QAbstractItemModel* model,
    AdNavigationMenu::SelectionMode selectionMode = AdNavigationMenu::SelectionMode::SingleSelection) {
...
void selectMenuId(AdNavigationMenu* menu,
...
QVector<MenuNode> horizontalNodes() {
  ...
  return {
      ...
      submenuNode(
          ...
          QStringLiteral("Navigation Three - Submenu"),
          {groupNode(QStringLiteral("g1"),
...
QVector<MenuNode> collapsedInlineNodes() {
  ...
  return {
      ...
      submenuNode(QStringLiteral("sub1"),
                  QStringLiteral("Navigation One"),
                  {actionNode(QStringLiteral("5"), QStringLiteral("Option 5")),
                   actionNode(QStringLiteral("6"), QStringLiteral("Option 6")),
                   actionNode(QStringLiteral("7"), QStringLiteral("Option 7")),
                   actionNode(QStringLiteral("8"), QStringLiteral("Option 8"))},
                  mail),
      submenuNode(QStringLiteral("sub2"),
                  QStringLiteral("Navigation Two"),
                  {actionNode(QStringLiteral("9"), QStringLiteral("Option 9")),
                   actionNode(QStringLiteral("10"), QStringLiteral("Option 10")),
                   submenuNode(QStringLiteral("sub3"),
  ...
  return {
      ...
      submenuNode(QStringLiteral("2"),
                  ...
                  {actionNode(QStringLiteral("21"), QStringLiteral("Option 1")),
                   ...
                   submenuNode(QStringLiteral("24"),
                               QStringLiteral("Submenu 2"),
...
QVector<MenuNode> switchModeNodes() {
  ...
  return {
      ...
      submenuNode(QStringLiteral("sub1"),
                  ...
                  {actionNode(QStringLiteral("3"), QStringLiteral("Option 3")),
                   actionNode(QStringLiteral("4"), QStringLiteral("Option 4")),
                   submenuNode(QStringLiteral("sub1-2"),
                               QStringLiteral("Submenu"),
                               {actionNode(QStringLiteral("5"), QStringLiteral("Option 5")),
                                actionNode(QStringLiteral("6"), QStringLiteral("Option 6"))})},
                  app),
...
class DebugMenuItemDelegate final : public AdNavigationMenuItemDelegate {
 public:
  ...
  void paint(QPainter* painter,
    ...
    const auto nodeKind = static_cast<AdNavigationMenu::NodeKind>(
        index.data(AdNavigationMenu::NodeKindRole).toInt());
    ...
    if (submenu) {
      ...
      painter->drawLine(option.rect.left() + 10,
                        option.rect.bottom() - 2,
                        option.rect.right() - 10,
                        option.rect.bottom() - 2);
    }
    painter->restore();
  }
...
MenuDocsPage::MenuDocsPage(QWidget* parent) : QWidget(parent) {
  ...
  auto* title = new QLabel("Navigation Menu");
  QFont titleFont = title->font();
  ...
  auto* subtitle = new QLabel(
      "This page documents the Ant Design-style navigation menu implementation and its Qt-first state ownership model.");
  ...
  addSection(root,
             "API Overview",
             "Qt-first entry points are preferred and the menu now focuses on model/selection ownership, delegates, and popup factories.",
             buildApiOverview());
...
QWidget* MenuDocsPage::buildPopupColorSchemeDemo() {
  ...
  const auto rebuildModel = [menu, popupSchemeBox]() {
    ...
    auto* model = createMenuModel(menu,
                                  {sub1,
                                   actionNode(QStringLiteral("5"), QStringLiteral("Option 5")),
                                   actionNode(QStringLiteral("6"), QStringLiteral("Option 6"))});
  ...
  layout->addLayout(controls);
  layout->addWidget(makeHintLabel("Inherit keeps submenu popups on the parent menu's effective scheme."));
  layout->addWidget(menu);
  return box;
}
...
QWidget* MenuDocsPage::buildStyleClassDemo() {
  ...
  const QVector<MenuNode> nodes = {
      submenuNode(QStringLiteral("SubMenu"),
                  QStringLiteral("Navigation One"),
                  {groupNode(QStringLiteral("g1"),
                             QStringLiteral("Item 1"),
                             {actionNode(QStringLiteral("1"), QStringLiteral("Option 1")),
  ...
  demo::bindThemeRefresh(menu1, [menu1]() {
    ...
    semantic1.root.backgroundColor = demo::themeColorOr(map.colorBgContainer, QColor(255, 255, 255));
    semantic1.root.borderColor = demo::themeColorOr(map.colorBorder, QColor(QStringLiteral("#d9d9d9")));
    semantic1.item.textColor = demo::themeColorOr(map.colorPrimary, QColor(QStringLiteral("#1677ff")));
  ...
  menu2->setSemanticStyleResolver([menu2](const AdNavigationMenu::StyleContext& ctx) {
    ...
    const bool hasSub = ctx.model && ctx.model->rowCount() > 0 &&
                        ctx.model->rowCount(ctx.model->index(0, 0)) > 0;
    QColor highlighted = demo::themeColorOr(map.colorPrimaryBg, QColor(240, 249, 255));
    highlighted.setAlpha(160);
    styles.root.backgroundColor = hasSub ? highlighted
                                         : demo::themeColorOr(map.colorBgContainer, QColor(255, 255, 255));
    styles.root.borderColor = demo::themeColorOr(map.colorBorder, QColor(QStringLiteral("#d9d9d9")));
...
QWidget* MenuDocsPage::buildStyleDebugDemo() {
  ...
  auto* selectionModel = attachMenuModel(
      ...
      createMenuModel(
          menu,
          {submenuNode(QStringLiteral("sub1"),
                       QStringLiteral("Navigation One Long Long Long Long"),
                       {actionNode(QStringLiteral("1"), QStringLiteral("Option 1")),
                        actionNode(QStringLiteral("2"), QStringLiteral("Option 2")),
                        actionNode(QStringLiteral("3"), QStringLiteral("Option 3")),
                        actionNode(QStringLiteral("4"), QStringLiteral("Option 4"))},
...
QWidget* MenuDocsPage::buildMenuV4Demo() {
  ...
  connect(modeCheck, &QCheckBox::toggled, menu, [menu](bool checked) {
    menu->setMode(checked ? AdNavigationMenu::Mode::Vertical
                          : AdNavigationMenu::Mode::Inline);
  });
...
QWidget* MenuDocsPage::buildComponentTokenDemo() {
  ...
  auto* collapsedSelection =
      attachMenuModel(menu1InlineCollapsed, createMenuModel(menu1InlineCollapsed, collapsedInlineNodes()));
...
QWidget* MenuDocsPage::buildExtraStyleDemo() {
  ...
  MenuNode sub1 = submenuNode(QStringLiteral("sub1"),
                              QStringLiteral("Navigation One"),
...
QWidget* MenuDocsPage::buildCustomPopupRenderDemo() {
  ...
  QVector<MenuNode> nodes = {
      ...
      submenuNode(QStringLiteral("features"),
      ...
      submenuNode(QStringLiteral("resources"),
...
QWidget* MenuDocsPage::buildSemanticDemo() {
  ...
  menu->setSemanticStyleResolver([menu](const AdNavigationMenu::StyleContext& ctx) {
    ...
    if (ctx.mode == AdNavigationMenu::Mode::Horizontal) {
      styles.root.backgroundColor = demo::themeColorOr(map.colorSuccessBg, QColor(QStringLiteral("#f6ffed")));
      styles.item.textColor = demo::themeColorOr(map.colorSuccess, QColor(QStringLiteral("#389e0d")));
    } else if (ctx.mode == AdNavigationMenu::Mode::Vertical) {
      styles.root.backgroundColor = demo::themeColorOr(map.colorWarningBg, QColor(QStringLiteral("#fff7e6")));
      styles.item.textColor = demo::themeColorOr(map.colorWarning, QColor(QStringLiteral("#d46b08")));
    } else {
      styles.root.backgroundColor = demo::themeColorOr(map.colorPrimaryBg, QColor(QStringLiteral("#e6f4ff")));
      styles.item.textColor = demo::themeColorOr(map.colorPrimary, QColor(QStringLiteral("#1677ff")));
    }
    styles.root.borderColor = demo::themeColorOr(map.colorBorder, QColor(QStringLiteral("#d9d9d9")));
    return styles;
  ...
  const auto rebuildForMode = [menu, applySemanticDemoWidth](AdNavigationMenu::Mode mode) {
    ...
    if (mode != AdNavigationMenu::Mode::Horizontal) {
      nodes.append(groupNode(QStringLiteral("grp"),
...
QWidget* MenuDocsPage::buildApiOverview() {
  ...
  auto* hint = makeHintLabel(
      ...
      "Breaking changes removed the old Ant-style item arrays and popup/paint hook shims in favor of delegates and popup factories.");
  ...
  const QVector<QPair<QString, QString>> rows = {
      {QStringLiteral("AdNavigationMenu"), QStringLiteral("Qt-first navigation menu widget with Ant Design visual parity")},
      ...
      {QStringLiteral("model / selectionModel / currentIndex"), QStringLiteral("primary state ownership and synchronization path")},
      {QStringLiteral("selectionMode"), QStringLiteral("no selection | single | multi")},
      {QStringLiteral("collapsed / indentation"), QStringLiteral("inline collapse behavior and nested indentation in pixels")},
      {QStringLiteral("submenuTrigger"), QStringLiteral("hover | click")},
      {QStringLiteral("submenuOpenDelayMs / submenuCloseDelayMs"), QStringLiteral("popup open and close delay in milliseconds")},
      {QStringLiteral("tooltipEnabled"), QStringLiteral("inline-collapsed tooltip behavior")},
      {QStringLiteral("componentTokens"), QStringLiteral("component token overrides: metrics + colors.shared/light/dark")},
      {QStringLiteral("semanticStyles / semanticStyleResolver"), QStringLiteral("semantic slot style overrides")},
      {QStringLiteral("itemDelegate"), QStringLiteral("custom row rendering through a Qt item delegate")},
      {QStringLiteral("popupFactory"), QStringLiteral("custom popup container factory for submenu popups")},
      {QStringLiteral("expandIcon / popupOffset"), QStringLiteral("submenu indicator icon and popup placement adjustments")},
      {QStringLiteral("signals: activated / expanded / collapsed / currentIndexChanged"), QStringLiteral("Qt-style interaction and state change notifications")},
      {QStringLiteral("removed APIs"), QStringLiteral("setItems, key-based selection/open state, popupRender, and paint hooks were removed")},
  };
