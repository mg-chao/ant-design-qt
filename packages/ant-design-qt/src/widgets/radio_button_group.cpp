...
AdRadioButtonGroup::ComponentTokens AdRadioButtonGroup::componentTokens() const { return componentTokens_; }
...
AdRadio* AdRadioButtonGroup::checkedRadio() const { return qobject_cast<AdRadio*>(checkedButton()); }
...
QList<AdRadioButtonGroup::RadioSnapshot> AdRadioButtonGroup::snapshotRadios(bool includeTokens) const {
...
void AdRadioButtonGroup::applySnapshots(const QList<RadioSnapshot>& snapshots, bool tokensMayChange) {
...
QList<AdRadio*> AdRadioButtonGroup::orderedRadios(bool visibleOnly) const {
  ...
  if (allVisible) {
    std::sort(result.begin(), result.end(), [vertical, reverse](const AdRadio* lhs, const AdRadio* rhs) {
      if (!lhs || !rhs || lhs == rhs) {
        return false;
      }
...
void AdRadioButtonGroup::refreshManagedLayoutSpacing() {
  ...
  const QWidget* spacingSource =
      managedLayout_->parentWidget() ? static_cast<const QWidget*>(managedLayout_->parentWidget())
                                     : static_cast<const QWidget*>(currentRadios.isEmpty() ? nullptr
                                                                                          : currentRadios.front());

  if (variant_ == AdRadio::Variant::Button) {
    spacing = 0;
  } else if (orientationForLayout(managedLayout_) == Qt::Vertical) {
    spacing = std::max(0, qRound(adqt::theme::ThemeManager::instance().resolveTheme(spacingSource).sizeXS));
  } else if (!currentRadios.isEmpty() && currentRadios.front()) {
...
bool AdRadioButtonGroup::tryHandleNavigation(AdRadio* radio, int key) const {
  ...
  const int currentIndex = candidates.indexOf(radio);
...
AdRadio* AdRadioButtonGroup::visibleNeighbor(const AdRadio* radio, int delta) const {
  ...
  const QList<AdRadio*> visibleRadios = orderedRadios(true);
  const int index = visibleRadios.indexOf(const_cast<AdRadio*>(radio));
  if (index < 0) {
    return nullptr;
  }
