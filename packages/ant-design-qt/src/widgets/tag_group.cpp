...
void AdTagGroup::paintEvent(QPaintEvent* event) {
  ...
  const int radius = tokens.borderRadius.value_or(std::max(0, qRound(resolvedTheme.values.borderRadiusSM)));
...
void AdTagGroup::rebuildTags() {
  ...
  for (const Option& option : std::as_const(options_)) {
    ...
    connect(tag, &QAbstractButton::toggled, this, [this, tag](bool checked) {
      updateSelectionFromTag(tag, checked);
    });
...
void AdTagGroup::applyContainerStyle() {
  ...
  const int resolvedSpacing = spacing_ >= 0 ? spacing_
