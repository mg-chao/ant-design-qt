...
QtColorValue toQtColorValue(const ColorValue& value) {
  return toColorValue(value);
}

ColorValue toLegacyColorValue(const QtColorValue& value) {
  return toColorSelection(value);
}
 ...
 private:
  void refreshAccessibleDescription() {
    setAccessibleDescription(
        QStringLiteral("Hue %1, saturation %2 percent, brightness %3 percent")
            .arg(hue_)
...
void clampInlineUnitCursor(QLineEdit* editor, const QString& unitText) {
  ...
  const int maxCursorPosition =
      std::max(0, static_cast<int>(text.size() - unitText.size()));
 ...
 protected:
  bool eventFilter(QObject* watched, QEvent* event) override {
    if (watched == editor_ && event &&
      ...
      QTimer::singleShot(0, editor_.data(), [editor, unitText]() {
        clampInlineUnitCursor(editor, unitText);
      });
    }
...
class InlineUnitInputNumberTextPolicy final : public AdInputNumberTextPolicy {
 public:
  ...
  QString formatText(const QString& canonicalText,
                     bool editing,
...
AdInputNumber* createColorPickerStepperInput(QWidget* parent,
...
void bindColorPickerStepperInput(QObject* context,
                                 AdInputNumber* input,
...
void applyColorPickerStepperStyle(AdInputNumber* input,
                                  int controlHeight,
...
QString colorToTriggerHexText(const QColor& color) {
  ...
  const QString hex = colorToHexRgbLower(color).toUpper();
  if (color.alpha() >= 255) {
    return hex;
  }
...
class ColorPickerSegmentedFrame final : public QWidget {
 public:
  ...
  void setVisualStyle(const QColor& background, qreal radius) {
    const qreal normalizedRadius = std::max<qreal>(0.0, radius);
    const bool changed = background_ != background ||
                         !qFuzzyCompare(radius_ + 1.0, normalizedRadius + 1.0);
    background_ = background;
    radius_ = normalizedRadius;
    if (changed) {
...
class ColorPickerPresetsPanel final : public QWidget {
 ...
 protected:
  void paintEvent(QPaintEvent* event) override {
    ...
    painter.setPen(QPen(topBorderColor_, topBorderWidth_, Qt::SolidLine, Qt::SquareCap, Qt::MiterJoin));
...
class PresetCollapseHeaderButton final : public QAbstractButton {
 public:
  ...
  void setVisualStyle(const QFont& font,
  ...
  void paintEvent(QPaintEvent* event) override {
    ...
    if (expanded_) {
      painter.drawPolyline(
          QPolygonF({QPointF(arrowRect.left() + inset, arrowRect.top() + arrowRect.height() * 0.35),
      ...
      painter.drawPolyline(
          QPolygonF({QPointF(arrowRect.left() + arrowRect.width() * 0.35, arrowRect.top() + inset),
                     QPointF(arrowRect.right() - arrowRect.width() * 0.28, arrowRect.center().y()),
                     QPointF(arrowRect.left() + arrowRect.width() * 0.35,
                             arrowRect.bottom() - inset)}));
    }

    painter.setFont(font_);
    painter.setPen(isEnabled() ? textColor_ : arrowColor_);
    const QRectF textRect =
        contentRect.adjusted(arrowExtent + gap_, 0.0, 0.0, 0.0);
...
class PresetColorButton final : public QAbstractButton {
 public:
  ...
  void setColorValue(const ColorValue& value) {
    value_ = value;
    setAccessibleName(QStringLiteral("Preset color %1").arg(detail::ColorPickerValueModel::cssValue(value_)));
    setAccessibleDescription(QStringLiteral("Apply preset color"));
  }
  ...
  void setCheckerColors(const QColor& light, const QColor& dark, int cellSize) {
    const int normalizedCell = std::max(2, cellSize);
    const bool changed = checkerLight_ != light || checkerDark_ != dark ||
                         checkerCellSize_ != normalizedCell;
    checkerLight_ = light;
    checkerDark_ = dark;
    checkerCellSize_ = normalizedCell;
 ...
 protected:
  ...
  void paintEvent(QPaintEvent* event) override {
    ...
    const QRectF fillRect = outerRect.adjusted(outerPadding_, outerPadding_, -outerPadding_, -outerPadding_);
    if (!fillRect.isValid() || fillRect.width() <= 0.0 || fillRect.height() <= 0.0) {
      return;
    }
    ...
    if (hovered_ && isEnabled()) {
      ...
      const QRectF rawOutlineRect =
          outerRect.adjusted(outlineHalf + 0.5, outlineHalf + 0.5, -outlineHalf - 0.5, -outlineHalf - 0.5);
      ...
      painter.setPen(QPen(hoverOutlineColor_, outlineWidth, Qt::SolidLine, Qt::SquareCap,
                          Qt::MiterJoin));
      painter.setBrush(Qt::NoBrush);
      painter.drawPath(
...
void reflowPresetItems(QWidget* itemsHost, int columnCount, int spacing) {
  ...
  for (QAbstractButton* child :
       itemsHost->findChildren<QAbstractButton*>(QString::fromLatin1(kPresetSwatchObjectName),
  ...
  std::sort(buttons.begin(), buttons.end(), [](PresetColorButton* lhs, PresetColorButton* rhs) {
    const int lhsOrder = lhs ? lhs->property("ad-color-picker-order").toInt() : std::numeric_limits<int>::max();
    const int rhsOrder = rhs ? rhs->property("ad-color-picker-order").toInt() : std::numeric_limits<int>::max();
...
class ColorPickerClearButton final : public QAbstractButton {
 public:
  explicit ColorPickerClearButton(QWidget* parent = nullptr) : QAbstractButton(parent) {
    ...
    setFocusPolicy(Qt::TabFocus);
    setAttribute(Qt::WA_Hover, true);
    setAccessibleName(QStringLiteral("Clear color"));
    setAccessibleDescription(QStringLiteral("Reset the current color selection"));
  }
...
detail::ColorPickerValueModel::State createValueModelState(
    const AdColorSelection& selection,
    AdColorPicker::Mode mode,
...
QVector<AdColorPickerState::Mode> AdColorPickerState::modeOptions() const {
  return modeOptions_;
}
...
void AdColorPickerState::setAlphaChannelEnabled(bool value) {
  applyState(colorValue_, mode_, modeOptions_, format_, allowClear_, value,
             formatSelectorEnabled_, activeStopIndex_, presets_);
}
...
QString AdColorPickerState::cssText() const {
  return detail::ColorPickerValueModel::cssValue(colorValue_);
}

void AdColorPickerState::setCssText(const QString& value) {
  bool ok = false;
  const detail::ColorPickerValueModel::State nextState =
      detail::ColorPickerValueModel::stateFromCssValue(value, modeOptions_, mode_, activeStopIndex_, &ok);
  if (!ok) {
    return;
  }
...
QString AdColorPickerState::displayText() const {
  return detail::ColorPickerValueModel::formattedValue(
      createValueModelState(colorValue_, mode_, modeOptions_, activeStopIndex_), format_);
}
...
void AdColorPickerState::setValue(const AdColorValue& value) {
  const detail::ColorPickerValueModel::State nextState =
      detail::ColorPickerValueModel::stateFromSelection(toLegacyColorValue(value), modeOptions_, mode_,
                                                        activeStopIndex_);
  applyState(nextState.selection, nextState.mode, nextState.modeOptions, format_, allowClear_,
             alphaChannelEnabled_, formatSelectorEnabled_, nextState.activeStopIndex, presets_);
}
...
void AdColorPickerState::applyState(const AdColorSelection& selection,
                                       Mode mode,
  ...
  const QString previousCss = detail::ColorPickerValueModel::cssValue(previousState);
  ...
  const QString nextFormatted =
      detail::ColorPickerValueModel::formattedValue(nextState, format);
  ...
  const bool colorValueChangedFlag = colorValue_ != nextState.selection;
  const bool presetsChangedFlag = !presetItemsEqual(presets_, presets);
  const bool cssChangedFlag = previousCss != nextCss;
  const bool formattedChangedFlag = previousFormatted != nextFormatted;
  ...
  mode_ = nextState.mode;
  modeOptions_ = nextState.modeOptions;
  format_ = format;
  allowClear_ = allowClear;
  alphaChannelEnabled_ = alphaChannelEnabled;
  formatSelectorEnabled_ = formatSelectorEnabled;
  activeStopIndex_ = nextState.activeStopIndex;
  colorValue_ = nextState.selection;
  presets_ = presets;
...
void AdColorPicker::setMode(Mode value) {
  ...
  const detail::ColorPickerValueModel::State valueModelState = createValueModelState(
      currentState.selection, currentState.mode, currentState.modeOptions, currentState.activeStopIndex);
  const detail::ColorPickerValueModel::State nextState =
...
QString AdColorPicker::displayText() const {
  return detail::ColorPickerValueModel::formattedValue(exportColorValue(), format_, activeStopIndex_);
}
...
void AdColorPicker::setTriggerContent(QWidget* widget) {
  ...
  ensureTriggerUi();
  QWidget* previousTriggerWidget = currentTriggerWidget();
  if (previousTriggerWidget && previousTriggerWidget != this && previousTriggerWidget != defaultTrigger_) {
    previousTriggerWidget->removeEventFilter(this);
  }
  triggerContent_ = widget;
  if (QWidget* nextTriggerWidget = currentTriggerWidget()) {
    if (nextTriggerWidget != this && nextTriggerWidget != defaultTrigger_) {
      nextTriggerWidget->installEventFilter(this);
    }
  }
  if (popover_) {
  ...
  if (QWidget* triggerWidget = currentTriggerWidget()) {
    setHostedRootWidget(triggerWidget);
  }
...
bool AdColorPicker::eventFilter(QObject* watched, QEvent* event) {
  ...
  const bool watchedThemeOwner = watched == currentTriggerWidget() && watched != this && watched != defaultTrigger_;
...
detail::ColorPickerVisualStyle AdColorPicker::visualStyle() const {
  ...
  return detail::resolveColorPickerVisualStyle(input, resolvedTheme);
}
...
void AdColorPicker::invalidateStyleCache() const {
  styleCache_.reset();
}
...
void AdColorPicker::ensureTriggerUi() {
  ...
  triggerFrame_ = new ColorPickerTriggerFrame(this);
  ...
  setHostedRootWidget(defaultTrigger_);
}
...
void AdColorPicker::ensurePopover() {
  ...
  setHostedRootWidget(currentTriggerWidget());
  popover_->setSourceWidget(currentTriggerWidget());
  if (panelHost_) {
    attachPanelHostToPopover();
  } else {
...
void AdColorPicker::ensureClearButtonUi() {
  ...
  connect(clearButton_, &QAbstractButton::clicked, this, [this]() {
    ...
    auto state = detail::ColorPickerValueModel::State{exportColorValue(), mode_, modeOptions_,
                                                       activeStopIndex_};
    importColorValue(detail::ColorPickerValueModel::clearedState(state).selection, true, true, true);
  });
}
...
void AdColorPicker::ensureRgbInputUi() {
  ...
  bindColorPickerStepperInput(this, rgbInputR_, [this]() { previewFormatInputs(); },
                              [this]() { commitFormatInputs(); });
  bindColorPickerStepperInput(this, rgbInputG_, [this]() { previewFormatInputs(); },
                              [this]() { commitFormatInputs(); });
  bindColorPickerStepperInput(this, rgbInputB_, [this]() { previewFormatInputs(); },
                              [this]() { commitFormatInputs(); });
}
...
void AdColorPicker::ensureHsbInputUi() {
  ...
  hsbInputS_ = createColorPickerStepperInput(hsbInputHost_,
                                             QStringLiteral("ad-color-picker-hsb-s-input"), 0, 100, 1,
                                             0, QStringLiteral("%"));
  hsbInputB_ = createColorPickerStepperInput(hsbInputHost_,
                                             QStringLiteral("ad-color-picker-hsb-b-input"), 0, 100, 1,
                                             0, QStringLiteral("%"));
  ...
  bindColorPickerStepperInput(this, hsbInputH_, [this]() { previewFormatInputs(); },
                              [this]() { commitFormatInputs(); });
  bindColorPickerStepperInput(this, hsbInputS_, [this]() { previewFormatInputs(); },
                              [this]() { commitFormatInputs(); });
  bindColorPickerStepperInput(this, hsbInputB_, [this]() { previewFormatInputs(); },
                              [this]() { commitFormatInputs(); });
}
...
void AdColorPicker::ensureGradientUi() {
  ...
  int insertIndex = saturationPanel_ ? pickerLayout->indexOf(saturationPanel_) : pickerLayout->count();
  if (insertIndex < 0) {
    insertIndex = 0;
  }
...
void AdColorPicker::ensureEditorUi() {
  ...
  alphaInput_ = createColorPickerStepperInput(formatRowWidget,
                                              QStringLiteral("ad-color-picker-alpha-input"), 0, 100,
                                              1, 0, QStringLiteral("%"),
                                              QStringLiteral("100"), currentMetrics.alphaInputWidth);
  ...
  auto bindChannel = [this](AdSlider* slider) {
    ...
    connect(slider, &AdSlider::editingFinished, this, [this]() {
      if (syncingControls_) {
        return;
      }
...
connect(formatCombo_, &AdComboBox::currentDataChanged, this, [this](const QVariant& value) {
  ...
  bindColorPickerStepperInput(this, alphaInput_, [this]() { previewAlphaInput(); }, [this]() {
    if (syncingControls_ || !alphaInput_) {
      return;
    }
    int percent = 0;
...
void AdColorPicker::refreshStyle(bool preserveCurrentTriggerWidth) {
  ...
  if (triggerFrame_) {
    ...
    if (auto* triggerLayout = qobject_cast<QHBoxLayout*>(triggerFrame_->layout())) {
      ...
      const int startPad = triggerTextVisible_
      ...
      triggerLayout->setSpacing(triggerTextVisible_ ? std::max(0, style.metrics.triggerTextGap) : 0);
  ...
  if (presetsPanel_) {
    ...
    const int presetHeaderHeight =
        std::max(16, std::max(qRound(mapToken.fontHeightSM), QFontMetrics(presetLabelFont).height()));

    for (QWidget* group :
         presetsPanel_->findChildren<QWidget*>(QString::fromLatin1(kPresetGroupObjectName),
                                               Qt::FindDirectChildrenOnly)) {
      if (!group) {
        continue;
      }
    ...
    for (QAbstractButton* headerButton :
         presetsPanel_->findChildren<QAbstractButton*>(QString::fromLatin1(kPresetHeaderObjectName),
    ...
    for (QWidget* body :
         presetsPanel_->findChildren<QWidget*>(QString::fromLatin1(kPresetBodyObjectName),
                                               Qt::FindChildrenRecursively)) {
      if (!body) {
        continue;
      }
    ...
    for (QLabel* empty :
         presetsPanel_->findChildren<QLabel*>(QString::fromLatin1(kPresetEmptyObjectName),
                                              Qt::FindChildrenRecursively)) {
      if (!empty) {
        continue;
      }
  ...
  if (modeSegmented_) {
    ...
    const QColor segmentedBg = style.segmentedBackground;
    const QColor segmentedItemBg = style.segmentedItemBackground;
    const QColor segmentedHoverBg = style.segmentedItemHoverBackground;
    const QColor segmentedBorder = style.panelBorder;
    const QColor segmentedText = style.segmentedText;
    const QColor segmentedTextDisabled = style.segmentedTextDisabled;

    if (auto* segmentedFrame = dynamic_cast<ColorPickerSegmentedFrame*>(modeSegmented_.data())) {
      segmentedFrame->setVisualStyle(segmentedBg, segmentedRadius);
    }
    ...
    modeSegmented_->setStyleSheet(
        QStringLiteral(
            ...
            .arg(segmentedBorderWidth)
            .arg(segmentedBorder.name(QColor::HexArgb))
            .arg(style.segmentedTextChecked.name(QColor::HexArgb))
  ...
  if (formatCombo_) {
    ...
    const bool formatIconSynced =
        formatTokens.metrics.iconSize.has_value() &&
  ...
  if (auto* formatRowWidget =
    ...
    QWidget* formatSelectGap =
        formatRowWidget->findChild<QWidget*>(QString::fromLatin1(kFormatSelectGapObjectName),
                                             Qt::FindDirectChildrenOnly);
    if (formatSelectGap) {
      formatSelectGap->setFixedWidth(formatSelectGapWidth);
      formatSelectGap->setVisible(formatCombo_ && formatCombo_->isVisible() && formatSelectGapWidth > 0);
    }
    QWidget* formatAlphaGap =
        formatRowWidget->findChild<QWidget*>(QString::fromLatin1(kFormatAlphaGapObjectName),
                                             Qt::FindDirectChildrenOnly);
    if (formatAlphaGap) {
      formatAlphaGap->setFixedWidth(formatAlphaGapWidth);
      formatAlphaGap->setVisible(alphaInput_ && alphaInput_->isVisible() && formatAlphaGapWidth > 0);
    }
    if (formatInputHost_) {
      const QMargins rowMargins = formatRowLayout ? formatRowLayout->contentsMargins() : QMargins();
      int availableWidth =
          std::max(0, formatRowWidget->width() - rowMargins.left() - rowMargins.right());
      if (availableWidth <= 0) {
        availableWidth = std::max(0, style.metrics.panelWidth - sliderInlineStartInset);
      }
      if (formatCombo_ && formatCombo_->isVisible()) {
        const int formatWidth =
            formatCombo_->maximumWidth() > 0 ? formatCombo_->maximumWidth() : formatCombo_->sizeHint().width();
        availableWidth = std::max(0, availableWidth - formatWidth);
      }
      if (formatSelectGap && formatSelectGap->isVisible()) {
        availableWidth = std::max(0, availableWidth - formatSelectGap->width());
      }
      if (alphaInput_ && alphaInput_->isVisible()) {
        const int alphaWidth =
            alphaInput_->maximumWidth() > 0 ? alphaInput_->maximumWidth() : style.metrics.alphaInputWidth;
        availableWidth = std::max(0, availableWidth - alphaWidth);
      }
      if (formatAlphaGap && formatAlphaGap->isVisible()) {
  ...
  auto applyFormatNumberInputStyle = [&](AdInputNumber* input, bool fixedAlphaWidth) {
    applyColorPickerStepperStyle(input, style.metrics.inputHeight, compactHorizontalPadding,
                                 colorInputFontSize, 48, inputNumberHandleWidth,
                                 disabled() || (fixedAlphaWidth && disabledAlpha_),
                                 fixedAlphaWidth ? style.metrics.alphaInputWidth : 0,
                                 !fixedAlphaWidth || !disabledAlpha_);
  };
  ...
  if (clearButton_) {
    ...
    if (auto* clear = dynamic_cast<ColorPickerClearButton*>(clearButton_.data())) {
      clear->setVisualStyle(style.panelBackground,
                            style.panelBorder,
                            style.triggerBorder,
                            style.clearButtonSlash,
  ...
  auto compositeOn = [](const QColor& foreground, const QColor& background) {
    ...
    QColor composite;
    composite.setRedF(foreground.redF() * alpha + background.redF() * (1.0 - alpha));
    composite.setGreenF(foreground.greenF() * alpha + background.greenF() * (1.0 - alpha));
    composite.setBlueF(foreground.blueF() * alpha + background.blueF() * (1.0 - alpha));
    ...
    if (value.isGradient() && !value.gradientStops.isEmpty()) {
      ...
      if (!normalized.isEmpty()) {
        ...
        for (int index = 0; index < normalized.size() - 1; ++index) {
          ...
          QColor mixed;
          mixed.setRedF(lhs.color.redF() + (rhs.color.redF() - lhs.color.redF()) * t);
          mixed.setGreenF(lhs.color.greenF() + (rhs.color.greenF() - lhs.color.greenF()) * t);
          mixed.setBlueF(lhs.color.blueF() + (rhs.color.blueF() - lhs.color.blueF()) * t);
          mixed.setAlphaF(lhs.color.alphaF() + (rhs.color.alphaF() - lhs.color.alphaF()) * t);
  ...
  for (QWidget* itemsHost :
       presetsPanel_ ? presetsPanel_->findChildren<QWidget*>(QString::fromLatin1(kPresetItemsObjectName),
                                                              Qt::FindChildrenRecursively)
                     : QList<QWidget*>()) {
  ...
  if (presetsPanel_) {
    for (QAbstractButton* button :
         presetsPanel_->findChildren<QAbstractButton*>(QString::fromLatin1(kPresetSwatchObjectName),
                                                       Qt::FindChildrenRecursively)) {
  ...
  for (PresetColorButton* swatch : swatches) {
    ...
    swatch->setFrameStyle(style.presetBorder, style.metrics.borderWidth, style.metrics.swatchRadius);
    ...
    if (value.isGradient() && !value.gradientStops.isEmpty()) {
      ...
      if (stops.isEmpty()) {
        ...
        std::sort(stops.begin(), stops.end(), [](const QPair<qreal, QColor>& lhs,
                                                 const QPair<qreal, QColor>& rhs) {
...
void AdColorPicker::syncTriggerWidthLock(bool preserveCurrentWidth) {
  const bool shouldLock = popupVisible() && triggerTextVisible_ && defaultTrigger_ && !triggerContent_;
  if (!shouldLock) {
    ...
    if (text.trimmed().isEmpty()) {
      if (displayValue.isEmpty()) {
        ...
        if (markInactive) {
          ...
          for (const InternalGradientStop& stop : normalized) {
            cells.append(QStringLiteral("%1 %2%").arg(formattedColorString(stop.color)).arg(stop.percent));
          }
        }
...
if (formatCombo_) {
  ...
  auto syncGapVisibility = [](QWidget* gap, QWidget* section) {
    if (!gap) {
      return;
    }
    const int gapHeight = std::max(gap->minimumHeight(), gap->maximumHeight());
    gap->setVisible(section && section->isVisible() && gapHeight > 0);
  };
  syncGapVisibility(operationGap_, operationRow_);
  syncGapVisibility(gradientGap_, gradientSection_);
...
void AdColorPicker::setCurrentFromControls(bool emitCompleted) {
  ...
  QScopedValueRollback<LivePanelSyncSource> sourceGuard(livePanelSyncSource_,
                                                        LivePanelSyncSource::ChannelControls);
...
void AdColorPicker::importColorValue(const ColorValue& value,
  ...
  const auto nextState = detail::ColorPickerValueModel::stateFromSelection(value, modeOptions_, mode_,
                                                                           activeStopIndex_);
  ...
  if (!selectionChanged && !modeChangedByModel && wasCleared == cleared_ &&
      !emitCssTextSignal && !emitCompleted) {
    return;
  }
...
void AdColorPicker::applyStateObject() {
  ...
  detail::ColorPickerValueModel::State nextState = createValueModelState(
      toLegacyColorValue(state_->value()), state_->mode(), state_->modeOptions(),
      state_->activeStopIndex());
  nextState = detail::ColorPickerValueModel::normalizedState(nextState);
  ...
  if (!modeChangedFlag && !modeOptionsChangedFlag && !activeStopIndexChangedFlag &&
      !colorValueChangedFlag && !formatChangedFlag && !allowClearChangedFlag &&
      !alphaChangedFlag && !formatSelectorChangedFlag && !presetsChangedFlag) {
    return;
  }
...
void AdColorPicker::syncStateObject() {
  ...
  state_->applyState(exportColorValue(), mode_, modeOptions_, format_, allowClear_,
...
AdColorPickerPanel::AdColorPickerPanel(QWidget* parent) : QWidget(parent) {
  ...
  connect(core_, &AdColorPicker::displayTextChanged, this,
          &AdColorPickerPanel::displayTextChanged);
...
bool AdColorPickerPanel::allowClear() const {
  return core_ && core_->allowClear();
}
...
bool AdColorPickerPanel::disabled() const {
  return core_ && core_->disabled();
}
...
QString AdColorPickerPanel::cssText() const {
  return core_ ? core_->cssText() : QString();
}
...
QString AdColorPickerPanel::displayText() const {
  return core_ ? core_->displayText() : QString();
}

AdColorValue AdColorPickerPanel::value() const {
  return core_ ? core_->value() : AdColorValue();
}
...
AdColorPickerState* AdColorPickerPanel::state() const {
  return core_ ? core_->state() : nullptr;
}
