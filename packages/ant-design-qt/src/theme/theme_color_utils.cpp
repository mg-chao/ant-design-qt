...
QColor deriveSolidHoverColor(const QColor& background, const QColor& text, qreal minimumContrast) {
  ...
  return colorContrastRatio(hover, validOr(text, QColor(Qt::white))) >= minimumContrast ? hover : background;
}
...
QVector<QColor> generateMappedPalette(const QColor& base, bool darkTheme, const QColor& background) {
  const QColor resolvedBase = validOr(base, QColor(QStringLiteral("#1677ff")));
  const QString backgroundValue = background.isValid() ? background.name(QColor::HexRgb) : QString();
