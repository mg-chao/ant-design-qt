...
QPixmap renderTintedIcon(const adqt::icons::IconRef& token, const QColor& color, int side, qreal dpr) {
  if (!adqt::icons::isValid(token) || side <= 0) {
    return QPixmap();
  }
  adqt::icons::IconRef tinted = token;
  tinted.colors.primary = color;
  tinted.colors.hasPrimary = true;
  return adqt::icons::renderIconPixmap(tinted, QSize(side, side), dpr, QIcon::Normal, QIcon::Off);
}

bool inputNumberValueChanged(const detail::InputNumberValueModel* model,
...
QString inputNumberAccessibleValueText(const AdInputNumber* input) {
  ...
  const QString display = input->displayText().trimmed();
  if (!display.isEmpty()) {
    return display;
  }
...
QString inputNumberAccessibleDescription(const AdInputNumber* input) {
  ...
  const QString explicitDescription = input->accessibleDescription().trimmed();
...
class InputActionsBorderOverlay final : public QWidget {
 ...
 protected:
  void paintEvent(QPaintEvent* event) override {
    ...
    if (cornerRadius > 0.0) {
      painter.drawArc(QRectF(right - cornerRadius * 2.0, top, cornerRadius * 2.0, cornerRadius * 2.0),
                      0,
                      90 * 16);
      painter.drawArc(QRectF(right - cornerRadius * 2.0,
                             bottom - cornerRadius * 2.0,
                             cornerRadius * 2.0,
                             cornerRadius * 2.0),
                      270 * 16,
                      90 * 16);
...
class StepControlButton final : public QToolButton {
 ...
 protected:
  ...
  void paintEvent(QPaintEvent* event) override {
    ...
    if (background.isValid() && background.alpha() > 0 && fillRect.isValid() && fillRect.width() > 0.0 &&
        fillRect.height() > 0.0) {
      const QPainterPath fillPath = roundedRectPath(fillRect,
                                                    paintStyle_.topLeftRadius,
                                                    paintStyle_.topRightRadius,
                                                    paintStyle_.bottomRightRadius,
                                                    paintStyle_.bottomLeftRadius);
    ...
    if (!currentIcon.isNull()) {
      ...
      if (!pixmap.isNull()) {
        ...
        const QPoint topLeft((width() - pixmapSize.width()) / 2, (height() - pixmapSize.height()) / 2);
...
double AdInputNumber::value() const {
  return value_.isValid() ? value_.toDouble() : 0.0;
}
...
double AdInputNumber::minimum() const {
  return minimum_.isValid() ? minimum_.toDouble() : 0.0;
}
...
double AdInputNumber::maximum() const {
  return maximum_.isValid() ? maximum_.toDouble() : 0.0;
}
...
void AdInputNumber::focusEditor(FocusSelection selection) {
  ...
  switch (selection) {
    ...
    case FocusSelection::End:
      editor_->setCursorPosition(text.size());
      break;
...
AdInputNumber::ResolvedVisualState AdInputNumber::resolvedVisualState() const {
  ...
  state.lineEditVerticalInset = stepButtonLayout_ == StepButtonLayout::Split ? state.verticalInset : state.borderInset;
  ...
  state.splitActionWidth =
      std::max(state.handleWidth, std::max(8, state.style.metrics.inputFontSize) + state.horizontalPadding * 2);
...
void AdInputNumber::updateEditorTextFromValue(bool userTyping) {
  ...
  if (userTyping) {
    ...
    editor_->setCursorPosition(nextText.size());
  }
  emitTextChangedIfNeeded(nextText);
}
...
void AdInputNumber::setCommittedValueInternal(const QVariant& value,
                                              bool emitSignal,
                                              bool preserveEditorText,
                                              bool clampToRange) {
  const bool previousHasValue = hasValue();
  const QVariant previousValue = value_;
...
void AdInputNumber::emitCommittedValueSignals(const QVariant& previousValue, bool previousHasValue) {
...
QString AdInputNumber::displayTextForValue(const QVariant& value,
                                           bool editing,
...
void AdInputNumber::refreshVisualState(bool geometryChanged) {
  ...
  const int lineEditPaddingInline = stepButtonLayout_ == StepButtonLayout::Split ? state.horizontalPadding : 0;
  ...
  if (prefixIconLabel_) {
    ...
    prefixIconLabel_->setPixmap(stepButtonLayout_ == StepButtonLayout::Compact && adqt::icons::isValid(prefixIconRef_)
                                    ? renderTintedIcon(prefixIconRef_, state.style.prefixColor, iconSide, dpr)
                                    : QPixmap());
  }
  if (suffixIconLabel_) {
    suffixIconLabel_->setFixedSize(iconSide, iconSide);
    suffixIconLabel_->setPixmap(stepButtonLayout_ == StepButtonLayout::Compact && adqt::icons::isValid(suffixIconRef_)
  ...
  if (inputActionsWidget_) {
    const QColor actionsBackground =
        stepButtonLayout_ == StepButtonLayout::Compact ? state.style.handleBg : QColor(Qt::transparent);
    ...
    if (inputActionsLayout_) {
      inputActionsLayout_->setContentsMargins(0,
  ...
  if (inputActionsBorderOverlay_) {
    const bool showBorderOverlay = stepButtonLayout_ == StepButtonLayout::Compact && state.visibleHandleWidth > 0 &&
                                   state.inputShellBorderCanBeVisible && state.borderInset > 0 &&
    ...
    static_cast<InputActionsBorderOverlay*>(inputActionsBorderOverlay_)
        ->setBorderStyle(state.border,
                         state.style.metrics.borderWidth,
...
void AdInputNumber::layoutChildren(const ResolvedVisualState& state) {
  ...
  if (stepButtonLayout_ == StepButtonLayout::Split) {
    const int downWidth = (splitDownButton_ && splitDownButton_->isVisible()) ? splitDownButton_->width() : 0;
    const int upWidth = (splitUpButton_ && splitUpButton_->isVisible()) ? splitUpButton_->width() : 0;
  ...
  const auto centeredRect = [contentTop, contentHeight](int left, const QSize& size) {
    return QRect(left, contentTop + (contentHeight - size.height()) / 2, size.width(), size.height());
  };

  bool hasLeadingAffix = false;
  const auto layoutLeft = [&](QWidget* widget, const QSize& size) {
    if (!widget || !widget->isVisible() || !size.isValid() || size.width() <= 0 || size.height() <= 0) {
      return;
    }
    if (hasLeadingAffix) {
      x += itemGap;
  ...
  const bool hasTrailingAffixWidgets =
      (suffixIconLabel_ && suffixIconLabel_->isVisible()) || (suffixLabel_ && suffixLabel_->isVisible());
  ...
  if (prefixIconLabel_ && prefixIconLabel_->isVisible()) {
    layoutLeft(prefixIconLabel_, prefixIconLabel_->sizeHint().expandedTo(prefixIconLabel_->minimumSize()));
  }
  ...
  if (suffixIconLabel_ && suffixIconLabel_->isVisible()) {
    layoutRight(suffixIconLabel_, suffixIconLabel_->sizeHint().expandedTo(suffixIconLabel_->minimumSize()));
  }
...
void AdInputNumber::updateInteractionFocusOverlay() {
  ...
  if (!state.context.focused || !isEnabled() || !isVisible() || state.style.underlined ||
      state.style.selectorFocusOutlineColor.alpha() <= 0 || state.style.metrics.focusOutlineWidth <= 0.0) {
    stopInteractionFocusForOwner(this);
    return;
  }
...
void AdInputNumber::updateActionIcons(const ResolvedVisualState& state) {
  const adqt::icons::IconRef upToken =
      adqt::icons::isValid(upIconRef_)
          ? upIconRef_
          : (stepButtonLayout_ == StepButtonLayout::Split ? outlined_icons::Plus() : outlined_icons::Up());
  const adqt::icons::IconRef downToken =
      adqt::icons::isValid(downIconRef_)
          ? downIconRef_
          : (stepButtonLayout_ == StepButtonLayout::Split ? outlined_icons::Minus() : outlined_icons::Down());
  ...
  if (state.interactiveControls) {
    ...
    const bool upHovered =
        !upDisabled &&
        ((stepButtonLayout_ == StepButtonLayout::Split && splitUpButton_ && splitUpButton_->isEnabled() &&
          splitUpButton_->underMouse()) ||
         (stepButtonLayout_ == StepButtonLayout::Compact && inputUpButton_ && inputUpButton_->isEnabled() &&
          inputUpButton_->underMouse()));
    const bool downHovered =
        !downDisabled &&
        ((stepButtonLayout_ == StepButtonLayout::Split && splitDownButton_ && splitDownButton_->isEnabled() &&
          splitDownButton_->underMouse()) ||
         (stepButtonLayout_ == StepButtonLayout::Compact && inputDownButton_ && inputDownButton_->isEnabled() &&
          inputDownButton_->underMouse()));
    if (upHovered) {
...
bool AdInputNumber::shouldShowInputActions(const ResolvedVisualState& state) const {
  return stepButtonLayout_ == StepButtonLayout::Compact && state.interactiveControls && state.visibleHandleWidth > 0;
}
...
void AdInputNumber::updateReadOnlyState() {
  ...
  if (inputUpButton_) {
    inputUpButton_->setEnabled(buttonEnabled && (!valueModel_ || !valueModel_->isStepDisabled(value_, true)));
  }
  if (inputDownButton_) {
    inputDownButton_->setEnabled(buttonEnabled && (!valueModel_ || !valueModel_->isStepDisabled(value_, false)));
  }
  if (splitUpButton_) {
    splitUpButton_->setEnabled(buttonEnabled && (!valueModel_ || !valueModel_->isStepDisabled(value_, true)));
  }
  if (splitDownButton_) {
    splitDownButton_->setEnabled(buttonEnabled && (!valueModel_ || !valueModel_->isStepDisabled(value_, false)));
  }
}
...
void AdInputNumber::updateInteractiveCursor() {
  ...
  if (editor_) {
    editor_->setCursor(disabledNow ? Qt::ForbiddenCursor : (readOnly_ ? Qt::ArrowCursor : Qt::IBeamCursor));
  }
...
void AdInputNumber::updateInputActionsGeometry(const ResolvedVisualState& state) {
  ...
  if (inputActionsBorderOverlay_ && inputActionsBorderOverlay_->isVisible()) {
    ...
    inputActionsBorderOverlay_->setGeometry(overlayShellX,
                                            0,
                                            std::max(0, width() - overlayShellX),
                                            height());
  }
...
bool AdInputNumber::isHoverTrackedChild(const QObject* watched) const {
  return watched == editor_ || watched == prefixLabel_ || watched == suffixLabel_ ||
         watched == prefixIconLabel_ || watched == suffixIconLabel_ || watched == inputActionsWidget_ ||
         watched == inputUpButton_ || watched == inputDownButton_ || watched == splitUpButton_ ||
         watched == splitDownButton_;
}
...
void AdInputNumber::notifyAccessibleDescriptionChange() const {
  detail::notifyAccessibilityEvent(const_cast<AdInputNumber*>(this), QAccessible::DescriptionChanged);
}
...
bool AdInputNumber::eventFilter(QObject* watched, QEvent* event) {
  ...
  } else if (watched == inputUpButton_ || watched == inputDownButton_ || watched == splitUpButton_ ||
             watched == splitDownButton_) {
...
void AdInputNumber::paintEvent(QPaintEvent* event) {
  ...
  const qreal cornerRadius = state.style.underlined ? 0.0 : std::max<qreal>(0.0, state.style.metrics.borderRadius);
...
void AdInputNumber::changeEvent(QEvent* event) {
  ...
  if (event->type() == QEvent::EnabledChange || event->type() == QEvent::FontChange ||
    ...
    const bool geometryChanged = event->type() == QEvent::EnabledChange ||
...
QValidator::State AdInputNumber::validate(QString& text, int& pos) const {
  ...
  const QVariant parsed = valueModel_ ? valueModel_->parseInputText(text, parser, true) : QVariant();
  if (parsed.isValid()) {
