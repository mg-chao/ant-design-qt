...
class TimingHub final : public QObject {
 public:
  ...
  void scheduleTask(QObject* context, const QString& key, int delayMs, DeadlineCallback callback) {
    ...
    if (const auto existing = keyedTaskIds_.constFind(taskKey); existing != keyedTaskIds_.constEnd()) {
    ...
    taskPayloads_.insert(taskId, std::move(payload));
  ...
  void setFrameSubscription(QObject* context,
                            const QString& key,
                            bool enabled,
    ...
    registerContext(context);
    frameCallbacks_.insert(frameKey, std::move(callback));
    updateFrameTimerState();
  }
 ...
 private:
  ...
  void registerContext(QObject* context) {
    ...
    contextConnections_.insert(
        context, connect(context, &QObject::destroyed, this, [this](QObject* destroyed) {
          clearContext(destroyed);
        }));
  }
  ...
  void refreshTimingConfigFromTheme() {
    const theme::ThemeMapToken& map = theme::ThemeManager::instance().currentMapToken();
    TimingConfig next;
    next.frameIntervalMs = std::max(0, map.timingFrameIntervalMs);
    next.spinnerCycleMs = std::max(0, map.timingSpinnerCycleMs);
    next.waveDurationMs = std::max(0, map.timingWaveDurationMs);
    next.menuOpenDelayMs = std::max(0, map.timingMenuOpenDelayMs);
    next.menuCloseDelayMs = std::max(0, map.timingMenuCloseDelayMs);
    next.loadingDelayMs = std::max(0, map.timingLoadingDelayMs);
...
void scheduleTimingTask(QObject* context, const QString& key, int delayMs, DeadlineCallback callback) {
...
void setFrameSubscription(QObject* context, const QString& key, bool enabled, FrameCallback callback) {
