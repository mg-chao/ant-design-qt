...
bool tryMapHostRectToOwnerRect(const QWidget* owner, const QWidget* hostWindow, const QRectF& hostRect,
                               QRectF* ownerRect) {
...
class SharedInteractionOverlay final : public QWidget {
 public:
  ...
  void triggerInteractionFocus(const InteractionFocusRequest& request, const QRectF& localBaseRect) {
    ...
    track.request = request;
    track.request.baseRectInWindow = localBaseRect;
    track.ownerBaseRectValid =
        tryMapHostRectToOwnerRect(request.owner, hostWindow_, request.baseRectInWindow, &track.ownerBaseRect);
  ...
  void stopInteractionFocusIfOwner(const QWidget* owner) {
    ...
    if (focusTracks_.remove(owner) > 0) {
      refreshOverlayState();
    }
  }
 ...
 protected:
  bool eventFilter(QObject* watched, QEvent* event) override {
    ...
    switch (event->type()) {
      ...
      case QEvent::DynamicPropertyChange: {
        ...
        if (propertyEvent && propertyEvent->propertyName() == QByteArray(kInteractionSurfaceProperty)) {
          markVisibleSurfaceCacheDirty();
        }
        break;
      }
  ...
  void paintEvent(QPaintEvent* event) override {
    ...
    for (auto it = waveTracks_.cbegin(); it != waveTracks_.cend(); ++it) {
      ...
      const qreal startStroke = std::max<qreal>(0.6, (kInteractionWaveStrokeWidth * 0.32) * strokeScale);
      ...
      const QRectF waveRect = track.request.baseRectInWindow.adjusted(-outwardOffset, -outwardOffset,
    ...
    for (auto it = focusTracks_.cbegin(); it != focusTracks_.cend(); ++it) {
      ...
      const QRectF focusRect = focusBaseRect.adjusted(-outwardOffset, -outwardOffset, outwardOffset,
                                                      outwardOffset);
 ...
 private:
  ...
  bool refreshDynamicFocusTrackRects() {
    ...
    bool changed = false;
    for (auto it = focusTracks_.begin(); it != focusTracks_.end(); ++it) {
      ...
      if (!track.resolvedBaseRectValid || !rectApproximatelyEqual(track.resolvedBaseRect, nextRect)) {
  ...
  void advanceInteractionWaveFrame() {
    ...
    const bool needsRepaint =
        changed || !waveTracks_.isEmpty() || focusMoved || (clipAgainstSurfaces_ && visibleSurfaceCacheDirty_);
...
SharedInteractionOverlay* resolveOverlayForOwner(const QWidget* owner, const QRectF& rectInHostWindow,
  ...
  if (surfaceRoot && surfaceRoot != hostWindow) {
    if (outRectInOverlay) {
      *outRectInOverlay =
          mapRectFromHostToOverlay(rectInHostWindow, hostWindow, surfaceRoot);
    }
