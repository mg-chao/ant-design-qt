...
QStandardItemModel* makeSelectItemModel(
    const QVector<AdComboBox::Option>& options,
...
QWidget* SelectDocsPage::buildSearchMultiFieldDemo() {
  ...
  select->setModel(makeSelectItemModel(
      ...
      select,
      [](QStandardItem* item, const AdComboBox::Option& option) {
...
QWidget* SelectDocsPage::buildSelectUsersDemo() {
  ...
  auto requestId = std::make_shared<int>(0);
  connect(select, &AdMultiSelect::searchTextChanged, select,
            ...
            adqt::widgets::detail::scheduleTimingTask(
                ...
                [select, hint, requestId, current, value]() {
                  ...
                  select->setModel(makeSelectItemModel(
                      options,
                      select,
                      [](QStandardItem* item, const Option& option) {
                        const QString avatar = option.metadata.value("avatar").toString();
                        item->setData(QStringLiteral("%1 %2").arg(avatar, option.label), Qt::DisplayRole);
                      }));
                  select->setLoading(false);
...
QWidget* SelectDocsPage::buildHideSelectedDemo() {
  ...
  connect(select, &AdMultiSelect::selectedValuesChanged, select, [rebuild](const QVariantList& values) {
...
QWidget* SelectDocsPage::buildCustomTagRenderDemo() {
  ...
  auto* select = new AdMultiSelect();
  select->setModel(makeSelectItemModel(
      {makeOption("gold", "gold"), makeOption("lime", "lime"), makeOption("green", "green"),
       makeOption("cyan", "cyan")},
      select,
      [](QStandardItem* item, const Option& option) {
        item->setData(QStringLiteral("[%1]").arg(option.label.toUpper()), AdComboBox::DefaultTagTextRole);
      }));
