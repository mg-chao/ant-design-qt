...
QString sourceToString(const QUrl& source) {
...
bool decodeDataUrl(const QUrl& source, QByteArray* bytes) {
  ...
  const int commaIndex = encoded.indexOf(',');
  if (commaIndex <= 0) {
    return false;
  }
...
QImage loadImageSynchronously(const QUrl& source) {
  if (source.isEmpty()) {
    return QImage();
  }

  QByteArray bytes;
  if (decodeDataUrl(source, &bytes)) {
    QImage image;
  ...
  QImage image;
  if (source.isLocalFile() && image.load(source.toLocalFile())) {
    return image;
  }

  const QString raw = sourceToString(source);
  if (image.load(raw)) {
    return image;
  }

  if (source.scheme().compare(QStringLiteral("qrc"), Qt::CaseInsensitive) == 0) {
    if (image.load(source.path())) {
      return image;
    }
  }

  if (source.isRelative()) {
    const QString localPath = QFileInfo(raw).filePath();
    if (!localPath.isEmpty() && image.load(localPath)) {
      return image;
    }
  }
...
int imageCostKiB(const QImage& image) {
  ...
  return static_cast<int>(std::max<qint64>(1, (bytes + 1023) / 1024));
}
...
AdImageViewer::SemanticStyles mergedViewerSemanticStyles(
    const AdImageViewer::SemanticStyles& base,
    const AdImageViewer::SemanticStyles& extra) {
...
class DefaultImageReply final : public AdImageReply {
 public:
  ...
  void setNetworkReply(QNetworkReply* reply) { networkReply_ = reply; }

  void completeSuccess(const QImage& image) { succeed(image); }

  void completeFailure(const QString& errorString = QString()) { fail(errorString); }
 ...
 public slots:
  void abort() override {
    ...
    if (networkReply_) {
      networkReply_->disconnect(this);
      if (networkReply_->isRunning()) {
        networkReply_->abort();
      }
 ...
 private:
  QPointer<QNetworkReply> networkReply_;
};

class DefaultImageLoader final : public AdImageLoader {
 public:
  explicit DefaultImageLoader(QObject* parent = nullptr) : AdImageLoader(parent), cache_(64 * 1024) {}
  ...
  AdImageReply* load(const QUrl& source, QObject* parent = nullptr) override {
    ...
    if (key.isEmpty()) {
      QTimer::singleShot(0, reply, [reply]() { reply->completeFailure(QStringLiteral("Empty image source")); });
      return reply;
    }

    if (const QImage* cached = cache_.object(key)) {
      const QImage image = *cached;
      QTimer::singleShot(0, reply, [reply, image]() { reply->completeSuccess(image); });
      return reply;
    }

    const QImage localImage = loadImageSynchronously(source);
    if (!localImage.isNull()) {
      cache_.insert(key, new QImage(localImage), imageCostKiB(localImage));
      QTimer::singleShot(0, reply, [reply, localImage]() { reply->completeSuccess(localImage); });
      return reply;
    }

    if (!isRemoteSource(source)) {
      QTimer::singleShot(0, reply, [reply]() {
        reply->completeFailure(QStringLiteral("Image source is not readable"));
      });
      return reply;
    }
    ...
    const QPointer<DefaultImageReply> guardedReply(reply);
    QObject::connect(networkReply, &QNetworkReply::finished, reply, [this, guardedReply, networkReply, key]() {
      if (!guardedReply) {
        if (networkReply) {
      ...
      if (networkReply->error() != QNetworkReply::NoError) {
        guardedReply->completeFailure(networkReply->errorString());
        return;
      }

      QImage image;
      if (!image.loadFromData(networkReply->readAll())) {
        guardedReply->completeFailure(QStringLiteral("Failed to decode image data"));
        return;
      }

      cache_.insert(key, new QImage(image), imageCostKiB(image));
      guardedReply->completeSuccess(image);
    });
...
class ImagePreviewDialog final : public QWidget {
 public:
  ...
  class OverlayIconButton final : public QToolButton {
   public:
    ...
    void setVisualStyle(const QColor& normalBackground,
                        const QColor& hoverBackground,
                        const QColor& pressedBackground,
                        const QColor& disabledBackground,
                        qreal radius) {
      const qreal normalizedRadius = std::max<qreal>(0.0, radius);
      const bool changed = normalBackground_ != normalBackground ||
  ...
  class ImagePreviewCanvas final : public QWidget {
   public:
    ...
    void setBottomReservedInset(qreal value) {
      const qreal clamped = std::max<qreal>(0.0, value);
      if (qFuzzyCompare(bottomReservedInset_ + 1.0, clamped + 1.0)) {
        return;
      }
      bottomReservedInset_ = clamped;
      clampPanOffset();
      updateCursor();
      update();
    }
    ...
    void clearImage() {
      if (image_.isNull() && panOffset_.isNull() && !dragging_ && rotation_ == 0 && !flipX_ && !flipY_ &&
          qFuzzyCompare(userScale_ + 1.0, 2.0)) {
        return;
      }
    ...
    void resetTransform() {
      const bool hadState = dragging_ || rotation_ != 0 || flipX_ || flipY_ || !panOffset_.isNull() ||
                            !qFuzzyCompare(userScale_ + 1.0, 2.0);
      dragging_ = false;
      rotation_ = 0;
      flipX_ = false;
    ...
    void setScale(qreal value, std::optional<QPointF> anchor = std::nullopt) {
      ...
      if (!image_.isNull() && previousRect.isValid() && !qFuzzyCompare(previousScale + 1.0, nextScale + 1.0)) {
   ...
   protected:
    void paintEvent(QPaintEvent* event) override {
      ...
      const QRectF imageRect(-image_.width() * 0.5,
                             -image_.height() * 0.5,
   ...
   private:
    ...
    QRectF contentRect() const {
      constexpr qreal kInset = 8.0;
      QRectF inner = rect();
      inner.adjust(kInset, kInset, -kInset, -(kInset + bottomReservedInset_));
      if (inner.height() <= 0.5) {
        inner = rect();
        inner.adjust(kInset, kInset, -kInset, -kInset);
      }
      return inner;
    }
    ...
    QSizeF transformedImageSize() const {
      ...
      return rotated ? QSizeF(image_.height(), image_.width()) : QSizeF(image_.width(), image_.height());
    }
    ...
    QSizeF displaySize() const {
      ...
      const qreal fitScale =
          std::min(fitBounds.width() / logicalSize.width(), fitBounds.height() / logicalSize.height());
    ...
    qreal bottomReservedInset_ = 0.0;
    int rotation_ = 0;
    bool flipX_ = false;
    bool flipY_ = false;
  };
  ...
  explicit ImagePreviewDialog(QWidget* parent = nullptr) : QWidget(parent) {
    ...
    auto createAction = [this](const adqt::icons::IconRef& token,
    ...
    flipYButton_ = createAction(outlined_icons::Swap(), 90.0, tr("Flip vertical"), [this]() { flipY(); });
  ...
  std::function<void(int currentRow, int totalCount, const AdImageItem& item, const QSize& actualSize)>
      currentItemChanged;
  ...
  int totalCount() const { return entries_.size(); }
 ...
 private:
  ...
  void applyButtonIconColorOverrides(const ButtonIconSpec& spec,
                            const QSize& logicalSize,
  ...
  int resolveOpenableEntryRow(int preferredRow) const {
    ...
    if (preferredRow >= 0 && preferredRow < entries_.size() && entries_.at(preferredRow).item.isValid()) {
      return preferredRow;
    }
  ...
  void loadCurrentItem() {
    ...
    if (!entry.item.isValid()) {
      ...
      if (currentItemChanged) {
        currentItemChanged(currentRow_, entries_.size(), entry.item, QSize());
      }
      updateControls();
      return;
    }
    ...
    AdImageReply* reply = resolvedImageLoader(entry.loader.data())->load(item.source, this);
    loadReply_ = reply;
    ...
    QObject::connect(reply, &AdImageReply::finished, this, [this, token, item, guardedReply]() {
      ...
      if (!ok) {
        ...
        imageStack_->setCurrentWidget(loadingLabel_);
        if (currentItemChanged) {
          currentItemChanged(currentRow_, entries_.size(), item, QSize());
        }
        updateControls();
        return;
      }
      ...
      if (currentItemChanged) {
        currentItemChanged(currentRow_, entries_.size(), item, image.size());
      }
      updateControls();
    });
  }
  ...
  void applyVisualStyle() {
    ...
    applyLabelStyle(loadingLabel_, visualStyle_.popupFooterText, visualStyle_.operationDisabledColor);
    ...
    const int footerBottomOffset =
        std::max(controlOffset, visualStyle_.metrics.footerBottomOffset);
    ...
    auto applyFloatingButtonStyle = [&](OverlayIconButton* button, int sizePx) {
      ...
      button->setVisualStyle(operationBg,
                             operationBgHover,
                             operationBg,
    ...
    auto applyActionStyle = [&](OverlayIconButton* button) {
      ...
      button->setVisualStyle(QColor(Qt::transparent),
                             QColor(Qt::transparent),
                             QColor(Qt::transparent),
                             QColor(Qt::transparent),
                             actionSize / 2.0);
    ...
    applyButtonIconColorOverrides(closeButtonSpec_,
                         iconLogicalSize,
                         floatingColor,
                         floatingColor,
                         visualStyle_.operationDisabledColor);
    applyButtonIconColorOverrides(prevButtonSpec_,
                         iconLogicalSize,
                         floatingColor,
                         floatingColor,
                         visualStyle_.operationDisabledColor);
    applyButtonIconColorOverrides(nextButtonSpec_,
                         iconLogicalSize,
                         floatingColor,
                         floatingColor,
                         visualStyle_.operationDisabledColor);
    ...
    for (const ButtonIconSpec& spec : actionIconSpecs_) {
      applyButtonIconColorOverrides(spec,
                           iconLogicalSize,
  ...
  void updateOverlayLayout() {
    ...
    if (imageCanvas_) {
      qreal reservedBottomInset = 0.0;
      if (footer_ && footer_->isVisible()) {
        reservedBottomInset = std::max<qreal>(0.0, bounds.height() - footer_->y());
      }
      imageCanvas_->setBottomReservedInset(reservedBottomInset);
    }
...
void AdImageReply::succeed(const QImage& image) {
  if (finished_) {
    return;
  }
...
void AdImageViewer::setModel(QAbstractItemModel* value) {
  ...
  if (model_) {
    modelConnections_.append(
        QObject::connect(model_, &QAbstractItemModel::modelReset, this, [this]() { handleModelReset(); }));
    modelConnections_.append(QObject::connect(model_,
    ...
    modelConnections_.append(
        QObject::connect(model_, &QAbstractItemModel::layoutChanged, this, [this]() { handleModelReset(); }));
...
int AdImageViewer::rowCount() const {
  return model_ ? model_->rowCount() : 0;
}

AdImageItem AdImageViewer::itemAt(int row) const {
  return readItemFromModel(model_, row);
}
...
AdImageViewer::ComponentTokens AdImageViewer::componentTokens() const {
...
AdImageViewer::SemanticStyles AdImageViewer::semanticStyles() const {
...
void AdImageViewer::setSemanticStyleResolver(SemanticStyleResolver resolver) {
  semanticStyleResolver_ = resolver;
  emit semanticStylesChanged();
  applyPreviewSurfaceVisuals();
}

void AdImageViewer::openAt(int row) {
  openAtFromWidget(row, contextWidget_);
}
...
void AdImageViewer::openAtFromWidget(int row, QWidget* widget) {
  ...
  if (ImagePreviewDialog* dialog = asPreviewDialog(previewSurface_)) {
    ...
    applyPreviewSurfaceVisuals();
    QWidget* owner = widget ? static_cast<QWidget*>(widget) : resolvedOwnerWindow();
...
void AdImageViewer::handlePreviewCurrentItemChanged(int currentRow,
                                                    int totalCount,
...
void AdImageViewer::ensurePreviewSurface() {
  ...
  dialog->currentItemChanged =
      [this](int currentRow, int totalCount, const AdImageItem& item, const QSize& actualSize) {
...
void AdImage::setFallbackSource(const QUrl& value) {
  ...
  if (loadFailed_ || imageData_.isNull()) {
    reloadMainImage();
  }
}
...
QSize AdImage::sizeHint() const {
  ...
  if (!sourcePixmap.isNull()) {
    const QSize natural = pixmapActualSize(sourcePixmap);
    if (targetWidth > 0 && natural.width() > 0) {
      const int h =
          std::max(1, static_cast<int>(std::round(targetWidth * natural.height() / std::max(1, natural.width()))));
      return QSize(targetWidth, h);
    }
    if (targetHeight > 0 && natural.height() > 0) {
      const int w =
          std::max(1, static_cast<int>(std::round(targetHeight * natural.width() / std::max(1, natural.height()))));
...
void AdImage::paintEvent(QPaintEvent* event) {
  ...
  const detail::ImageVisualStyle visual = detail::resolveImageVisualStyle(styleInput, resolvedTheme);
  ...
  if (!displayPixmap.isNull()) {
    const QSize scaled = pixmapActualSize(displayPixmap).scaled(drawRect.size(), Qt::KeepAspectRatio);
    const QPoint topLeft(drawRect.center().x() - scaled.width() / 2,
                         drawRect.center().y() - scaled.height() / 2);
    painter.drawPixmap(QRect(topLeft, scaled),
                       displayPixmap,
    ...
    adqt::icons::IconColorOverrides iconColors;
    iconColors.hasPrimary = true;
    iconColors.primary = visual.placeholderIcon;
    ...
    const QString text = loadFailed_ ? tr("Failed to load image")
                                     : (loading_ ? tr("Loading...") : tr("No image"));
    const QRect textRect(drawRect.left() + 8,
                         drawRect.center().y() + iconSize / 2,
                         drawRect.width() - 16,
                         30);
  ...
  if (context.previewable && (hovered_ || context.previewVisible)) {
    ...
    adqt::icons::IconColorOverrides iconColors;
    iconColors.hasPrimary = true;
    iconColors.primary = visual.coverText;
    const QPixmap iconPixmap = adqt::icons::renderIconPixmap(
...
int AdImage::effectivePreviewRow() const {
  return previewRow_ >= 0 ? previewRow_ : 0;
}
...
void AdImage::ensureInternalViewerModel() {
  ...
  AdImageViewer* targetViewer = internalViewer_ ? internalViewer_.data() : new AdImageViewer(const_cast<AdImage*>(this));
  if (!internalViewer_) {
...
void AdImage::reloadMainImage() {
  ...
  AdImageReply* reply = resolvedImageLoader(imageLoader_)->load(source_, this);
  mainReply_ = reply;
  const QPointer<AdImageReply> guardedReply(reply);
  QObject::connect(reply, &AdImageReply::finished, this, [this, token, guardedReply]() {
    ...
    if (!fallbackSource_.isEmpty() && normalizeSourceKey(fallbackSource_) != normalizeSourceKey(source_)) {
      AdImageReply* fallbackReply = resolvedImageLoader(imageLoader_)->load(fallbackSource_, this);
      fallbackReply_ = fallbackReply;
      const QPointer<AdImageReply> guardedFallbackReply(fallbackReply);
      QObject::connect(fallbackReply, &AdImageReply::finished, this, [this, token, guardedFallbackReply]() {
        if (token != mainLoadToken_) {
          return;
        }

        const QImage fallbackImage = guardedFallbackReply ? guardedFallbackReply->image() : QImage();
        const bool fallbackOk =
            guardedFallbackReply && guardedFallbackReply->isSuccessful() && !fallbackImage.isNull();
        fallbackReply_.clear();
        if (fallbackOk) {
          imageData_ = fallbackImage;
...
void AdImage::reloadPlaceholderImage() {
  ...
  QObject::connect(reply, &AdImageReply::finished, this, [this, token, guardedReply]() {
    ...
    if (guardedReply && guardedReply->isSuccessful() && !guardedReply->image().isNull()) {
      placeholderImageData_ = guardedReply->image();
      placeholderPixmap_ = imageToPixmap(placeholderImageData_);
    } else {
      placeholderImageData_ = QImage();
      placeholderPixmap_ = QPixmap();
    }
