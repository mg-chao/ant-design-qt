...
AdTag* makeTag(const QString& text,
               const QString& colorName = QString(),
...
class EditableTagDemo final : public QWidget {
 public:
  explicit EditableTagDemo(QWidget* parent = nullptr) : QWidget(parent) {
    ...
    root->addWidget(makeHintLabel(
        "Double-click a removable tag to edit it. The dashed New Tag button is a Qt-native recreation of control.tsx."));
  ...
  void rebuild() {
    ...
    for (int i = 0; i < tags_.size(); ++i) {
      if (editIndex_ == i) {
        ...
        connect(input, &QLineEdit::textChanged, this, [this](const QString& value) {
          editValue_ = value;
        });
...
TagDocsPage::TagDocsPage(QWidget* parent) : QWidget(parent) {
  ...
  auto* subtitle = new QLabel(
      "A compact label for categorization, status, and inline actions. This page cross-checks the Qt "
      "implementation against Ant Design Tag demos while keeping the widget Qt-native.");
  subtitle->setWordWrap(true);
  root->addWidget(subtitle);

  addSection(root, "Basic and closable",
             ...
             buildBasicDemo());
  addSection(root, "Colorful variants",
  ...
  addSection(root, "Checkable and group",
             ...
             buildCheckableDemo());
  addSection(root, "Dynamic add / remove / edit",
  ...
  addSection(root, "Semantic styling",
...
void TagDocsPage::addSection(QVBoxLayout* root,
                             const QString& title,
...
QWidget* TagDocsPage::buildBasicDemo() {
  ...
  deleteTag->setCloseIconRef(outlined_icons::Delete());
...
QWidget* TagDocsPage::buildColorfulDemo() {
  static const QStringList presets = {QStringLiteral("magenta"), QStringLiteral("red"),
                                      QStringLiteral("volcano"), QStringLiteral("orange"),
                                      QStringLiteral("gold"), QStringLiteral("lime"),
                                      QStringLiteral("green"), QStringLiteral("cyan"),
                                      QStringLiteral("blue"), QStringLiteral("geekblue"),
...
QWidget* TagDocsPage::buildCheckableDemo() {
  ...
  connect(singleGroup, &AdTagGroup::selectedValueChanged, singleGroupOutput,
          [singleGroupOutput](const QVariant& value) {
            singleGroupOutput->setText(
                QStringLiteral("Single: %1").arg(value.isValid() ? value.toString() : QStringLiteral("none")));
          });
  ...
  connect(multipleGroup, &AdTagGroup::selectedValuesChanged, multipleGroupOutput,
          [multipleGroupOutput](const QVariantList& values) {
            ...
            multipleGroupOutput->setText(QStringLiteral("Multiple: %1")
...
QWidget* TagDocsPage::buildDisabledDemo() {
  ...
  auto* redOutlined = makeTag(QStringLiteral("Preset red"), QStringLiteral("red"), AdTag::Variant::Outlined);
  redOutlined->setDisabled(true);
  auto* customOutlined = makeTag(QStringLiteral("Custom #f50"), QStringLiteral("#f50"), AdTag::Variant::Outlined);
  customOutlined->setDisabled(true);
  auto* customSolid = makeTag(QStringLiteral("Custom #f50 solid"), QStringLiteral("#f50"), AdTag::Variant::Solid);
  customSolid->setDisabled(true);
  auto* customFilled = makeTag(QStringLiteral("Custom #f50 filled"), QStringLiteral("#f50"), AdTag::Variant::Filled);
  customFilled->setDisabled(true);
  auto* success = makeTag(QStringLiteral("Success"), QStringLiteral("success"), AdTag::Variant::Filled);
  success->setDisabled(true);
  layout->addWidget(makeWrapPanel({redOutlined, customOutlined, customSolid, customFilled, success}));
  ...
  layout->addWidget(makeHintLabel("Disabled closable tags keep the affordance visible, but the close action is blocked."));
  return box;
}
...
QWidget* TagDocsPage::buildComponentTokenDemo() {
  ...
  auto* magentaClose = makeTag(QStringLiteral("magenta"), QStringLiteral("magenta"), AdTag::Variant::Filled);
  ...
  appendTag(redSolid);
  auto* processing = makeTag(QStringLiteral("processing"), QStringLiteral("processing"), AdTag::Variant::Filled);
  processing->setIconRef(outlined_icons::Sync());
  appendTag(processing);
  auto* disabled = makeTag(QStringLiteral("disabled"), QStringLiteral("success"), AdTag::Variant::Filled);
  ...
  layout->addWidget(makeHintLabel(
      "Use the Light / Dark mode switch in this demo window to compare the solid default tag against Ant Design's token example."));
  return box;
}
...
QWidget* TagDocsPage::buildStyleClassDemo() {
  ...
  QVector<AdTagGroup::Option> frontEndOptions = {{QStringLiteral("React"), QStringLiteral("React"), {}, false},
  ...
  resolverGroup->setOptions({{QStringLiteral("meet-student"), QStringLiteral("meet-student"), {}, false},
