...
AdTagSelect::AdTagSelect(QWidget* parent) : AdAbstractSelectWidget(parent) {
  ...
  connect(internalSelect(), &AdSelect::currentValuesChanged, this, &AdTagSelect::selectedValuesChanged);
  connect(internalSelect(), &AdSelect::currentItemsChanged, this, &AdTagSelect::selectedItemsChanged);
  connect(internalSelect(), &AdSelect::selectionChanged, this, &AdTagSelect::selectionChanged);
  connect(internalSelect(), &AdSelect::maxCountChanged, this, &AdTagSelect::maxSelectionCountChanged);
  connect(internalSelect(), &AdSelect::maxTagCountChanged, this, &AdTagSelect::maxVisibleTagsChanged);
...
bool AdTagSelect::responsiveMaxTagCount() const { return internalSelect()->responsiveMaxTagCount(); }
