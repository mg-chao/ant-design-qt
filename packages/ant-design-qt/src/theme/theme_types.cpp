...
QColor compositeOn(const QColor& foreground, const QColor& background, qreal opacityScale = 1.0) {
  ...
  const qreal alpha =
      std::clamp(static_cast<qreal>(foreground.alphaF()) * opacityScale, qreal(0.0), qreal(1.0));
  if (alpha >= 0.999) {
    QColor opaque = foreground;
    opaque.setAlpha(255);
    return opaque;
...
QColor alphaColor(const QColor& base, qreal alpha) {
  ...
  tinted.setAlphaF(std::clamp(alpha, qreal(0.0), qreal(1.0)));
  return tinted;
}
...
ThemeSemanticPalette makeSemanticPalette(const ThemeColors& colors) {
  ...
  const QColor surfaceSubtle = compositeOn(validOr(colors.colorFillAlter, colors.colorFillQuaternary), surface);
  ...
  semantic.accentDisabled = compositeOn(validOr(colors.colorPrimary, colors.colorPrimarySolid), surfaceDisabled, 0.38);
...
bool isEmptyThemeOverrideImpl(const ThemeOverride& overrideValue) { return overrideValue == ThemeOverride{}; }

void applyLightSemanticColors(ThemeColors* colors, const ThemeConfig& config) {
  const QColor primaryBase = validOr(config.primary, validOr(config.blue, QColor(QStringLiteral("#1677ff"))));
  const QColor successBase = validOr(config.success, validOr(config.green, QColor(QStringLiteral("#52c41a"))));
  ...
  colors->colorErrorBgFilledHover = mixColor(colors->colorErrorBg, colors->colorErrorBgActive, 50.0);
...
#undef ADQT_EQ_CONFIG_ACCENT
         ADQT_EQ_FIELD(primary)
         ADQT_EQ_FIELD(success)
         ADQT_EQ_FIELD(warning)
         ADQT_EQ_FIELD(error)
         ADQT_EQ_FIELD(info)
         ADQT_EQ_FIELD(link)
         ADQT_EQ_FIELD(appFont)
         ADQT_EQ_FIELD(codeFont)
         ADQT_EQ_FIELD(fontSize)
         ADQT_EQ_FIELD(lineWidth)
         ADQT_EQ_FIELD(borderRadius)
         ADQT_EQ_FIELD(sizeUnit)
         ADQT_EQ_FIELD(sizeStep)
         ADQT_EQ_FIELD(sizePopupArrow)
         ADQT_EQ_FIELD(controlHeight)
...
bool operator==(const ThemeValues& lhs, const ThemeValues& rhs) {
  return lhs.scheme == rhs.scheme && lhs.density == rhs.density && lhs.motion == rhs.motion
