...
QString sliderAccessibleDescription(const AdMultiSlider* slider) {
  ...
  const QString explicitDescription = slider->accessibleDescription().trimmed();
  ...
  const QList<double> values = slider->handleValues();
  if (values.size() > 1) {
    const int activeIndex = std::clamp(slider->activeHandleIndex(), 0, static_cast<int>(values.size()) - 1);
...
int adjustmentDirectionForKey(const AdMultiSlider* slider, int key) {
  ...
  if (slider && slider->orientation() == Qt::Horizontal && slider->layoutDirection() == Qt::RightToLeft &&
...
class AdMultiSlider::TooltipHost final {
 public:
  ...
  void setAnchorRect(const QRect& rawAnchorRect, const QRect& boundsRect, QWidget* coordinateWidget) {
    if (!tooltip_) {
      return;
    }
...
void AdMultiSlider::setHandleValues(const QList<double>& values) {
  setHandlesInternal(values, true, false);
}
...
AdMultiSlider::TooltipVisibleMode AdMultiSlider::tooltipVisibleMode() const { return tooltipVisibleMode_; }
...
AdMultiSlider::TooltipPlacement AdMultiSlider::tooltipPlacement() const { return tooltipPlacement_; }
...
AdMultiSlider::TooltipFormatter AdMultiSlider::tooltipFormatter() const { return tooltipFormatter_; }
...
QSize AdMultiSlider::sizeHint() const {
  ...
  if (orientation_ == Qt::Horizontal) {
    ...
    return QSize(std::max(260, sliderLength * 6), height);
  }
  const int width = std::max(
      std::max(52, sliderThickness),
      layout.style.metrics.controlSize + layout.style.metrics.marginCross * 2 +
          (hasMarks ? layout.style.metrics.markGap + markLabelWidth : 0));
  return QSize(width, std::max(260, sliderLength * 6));
}
...
QSize AdMultiSlider::minimumSizeHint() const {
  ...
  const int sliderLength = style()->pixelMetric(QStyle::PM_SliderLength, &option, this);
  if (orientation_ == Qt::Horizontal) {
    return QSize(std::max(120, sliderLength * 2), hint.height());
  }
  return QSize(hint.width(), std::max(120, sliderLength * 2));
}
...
void AdMultiSlider::setPendingChangeStateFromVisuals() {
  ...
  const double committedPrimary = committedHandles_.isEmpty() ? minimum_ : committedHandles_.constFirst();
...
QList<double> AdMultiSlider::normalizedValues(const QList<double>& values, bool forceRangeMode) const {
...
void AdMultiSlider::setHandlesInternal(const QList<double>& handles,
  ...
  if (!rangeMode) {
    ...
    if (normalized.isEmpty()) {
      normalized = {HandleEntry{minimum_, nextHandleId_++, 0}, HandleEntry{minimum_, nextHandleId_++, 1}};
    }
    ...
    if (normalized.size() < requiredMinCount) {
      ...
      while (normalized.size() < requiredMinCount) {
        normalized.append(HandleEntry{fillValue, nextHandleId_++, static_cast<int>(normalized.size())});
      }
    }
    ...
    std::stable_sort(normalized.begin(), normalized.end(), [](const HandleEntry& lhs, const HandleEntry& rhs) {
  ...
  const bool primaryValueChanged =
      !fuzzyEq(previousCommittedValue, committedHandles_.isEmpty() ? minimum_ : committedHandles_.constFirst());
...
double AdMultiSlider::effectiveSingleStep() const {
  return effectiveSingleStepForSlider(this);
}

double AdMultiSlider::effectivePageStep() const {
  return effectivePageStepForSlider(this);
}
...
void AdMultiSlider::notifyAccessibleDescriptionChange() const {
  detail::notifyAccessibilityEvent(const_cast<AdMultiSlider*>(this), QAccessible::DescriptionChanged);
}
...
void AdMultiSlider::initStyleOption(QStyleOptionSlider* option, int handleIndex) const {
  ...
  const double sliderValue =
      resolvedIndex >= 0 && resolvedIndex < committedHandles_.size() ? committedHandles_.at(resolvedIndex)
                                                                     : minimum_;
  ...
  option->upsideDown =
      invertedAppearance_ ^ (orientation_ == Qt::Horizontal && layoutDirection() == Qt::RightToLeft);
...
AdMultiSlider::LayoutInfo AdMultiSlider::buildLayout() const {
  if (!layoutCacheDirty_ && layoutCache_ && layoutCacheSize_ == size()) {
    return *layoutCache_;
  }
  ...
  layout.style = detail::resolveSliderVisualStyle(input, resolvedTheme);
...
double AdMultiSlider::valueFromPosition(const QPoint& pos, const LayoutInfo& layout) const {
  if (maximum_ <= minimum_) {
    return minimum_;
  }

  const int span = std::max(1, qRound(layout.axisLength));
  int position = 0;
  int sliderUnits = 0;
  if (!layout.vertical) {
    position = std::clamp(qRound(pos.x() - layout.axisStart), 0, span);
    const bool upsideDown = (layoutDirection() == Qt::RightToLeft) ^ invertedAppearance_;
    sliderUnits = QStyle::sliderValueFromPosition(0, 1000000, position, span, upsideDown);
  } else {
    position = std::clamp(qRound(pos.y() - layout.axisStart), 0, span);
    const int lowEndPosition = span - position;
    sliderUnits = QStyle::sliderValueFromPosition(0, 1000000, lowEndPosition, span, invertedAppearance_);
  }
  const double raw = minimum_ + (static_cast<double>(sliderUnits) / 1000000.0) * (maximum_ - minimum_);
  return normalizeValue(raw);
}

int AdMultiSlider::positionFromValue(double value, const LayoutInfo& layout) const {
  if (maximum_ <= minimum_) {
  ...
  const int sliderUnits = std::clamp(qRound(ratio * 1000000.0), 0, 1000000);
  const int span = std::max(1, qRound(layout.axisLength));
  if (!layout.vertical) {
    const bool upsideDown = (layoutDirection() == Qt::RightToLeft) ^ invertedAppearance_;
    const int position = QStyle::sliderPositionFromValue(0, 1000000, sliderUnits, span, upsideDown);
    return qRound(layout.axisStart + position);
  }

  const int lowEndPosition = QStyle::sliderPositionFromValue(0, 1000000, sliderUnits, span,
                                                             invertedAppearance_);
  return qRound(layout.axisStart + (span - lowEndPosition));
}
...
int AdMultiSlider::handleRailAction(const QPoint& pos, const LayoutInfo& layout, bool emitLiveSignals) {
  ...
  const int activeIndex = targetHandleId >= 0 ? indexForHandleId(targetHandleId) : nearestHandleIndex(nextValue);
...
void AdMultiSlider::wheelEvent(QWheelEvent* event) {
  ...
  QList<double> next = handles_;
  next[index] = normalizeValue(next.at(index) + (angleDelta > 0 ? effectiveSingleStep() : -effectiveSingleStep()));
  emitHandleActionTriggered(angleDelta > 0 ? QAbstractSlider::SliderSingleStepAdd
...
void AdMultiSlider::keyPressEvent(QKeyEvent* event) {
  ...
  if ((key == Qt::Key_Tab || key == Qt::Key_Backtab) && handles_.size() > 1) {
    int index = focusHandleIndex_;
    if (index < 0 || index >= handles_.size()) {
      index = 0;
    } else if (key == Qt::Key_Tab) {
      index = (index + 1) % handles_.size();
    } else {
      index = (index - 1 + handles_.size()) % handles_.size();
    }
    setFocusHandleIndex(index);
    event->accept();
    return;
  ...
  const QList<double> before = handles_;
  setHandlesInternal(next, tracking_, true, tracking_);
  const int activeIndex = focusedHandleId >= 0 ? indexForHandleId(focusedHandleId) : nearestHandleIndex(nextValue);
  setFocusHandleIndex(activeIndex);
  if (!listFuzzyEquals(before, handles_)) {
    dragChanged_ = true;
  }
...
void AdSlider::setRange(double minimum, double maximum) { AdMultiSlider::setRange(minimum, maximum); }
...
AdRangeSlider::AdRangeSlider(QWidget* parent) : AdMultiSlider(parent) {
  ...
  connect(this, &AdMultiSlider::handlePressed, this, [this](int index) { emit sliderPressed(index); });
...
void AdRangeSlider::setRange(double minimum, double maximum) { AdMultiSlider::setRange(minimum, maximum); }
