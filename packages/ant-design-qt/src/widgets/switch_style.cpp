...
QColor multiplyAlpha(const QColor& color, qreal opacity) {
  ...
  QColor out = color;
  out.setAlphaF(std::clamp(color.alphaF() * opacity, 0.0, 1.0));
  return out;
}
...
void recomputeDerivedMetrics(SwitchMetrics* metrics) {
  ...
  metrics->contentInsetFar = std::max(metrics->contentInsetNear, metrics->thumbSize + metrics->trackPadding * 3);
...
SwitchAppearance resolveSwitchAppearance(const SwitchAppearanceInput& input,
  ...
  const QColor primaryHoverColor = toColor(colors.colorPrimaryHover, QColor(QStringLiteral("#4096ff")));
  const QColor contentColor = toColor(semantic.textOnAccent, toColor(colors.colorWhite, QColor(QStringLiteral("#ffffff"))));
  const QColor uncheckedIndicatorBase =
      toColor(semantic.textSecondary, toColor(colors.colorTextSecondary, QColor(QStringLiteral("#8c8c8c"))));
  const QColor checkedIndicatorBase = toColor(semantic.accent, primaryColor);
  const QColor thumbShadowBase = toColor(semantic.text, toColor(colors.colorText, QColor(QStringLiteral("#141414"))));

  SwitchAppearance appearance;
  appearance.uncheckedTrackColor = toColor(colors.colorTextQuaternary, QColor(QStringLiteral("#bfbfbf")));
  appearance.uncheckedTrackHoverColor = toColor(colors.colorTextTertiary, QColor(QStringLiteral("#8c8c8c")));
  ...
  if (input.disabled || input.loading) {
    ...
    appearance.uncheckedTrackHoverColor = multiplyAlpha(appearance.uncheckedTrackHoverColor, opacity);
    ...
    appearance.checkedLoadingIndicatorColor = multiplyAlpha(appearance.checkedLoadingIndicatorColor, opacity);
    appearance.waveColor = multiplyAlpha(appearance.waveColor, opacity);
    appearance.metrics.thumbShadowColor = multiplyAlpha(appearance.metrics.thumbShadowColor, opacity);
  }

  if (!input.componentTokens.colors.wave.has_value()) {
    appearance.waveColor = input.checked ? appearance.checkedTrackColor : appearance.uncheckedTrackColor;
  }
...
SwitchGeometry buildSwitchGeometry(const QRectF& bounds,
                                   Qt::LayoutDirection direction,
                                   const SwitchAppearance& appearance,
                                   AdSwitch::ControlSize controlSize,
                                   qreal thumbProgress,
                                   qreal pressProgress,
                                   qreal pressDirection,
  ...
  geometry.trackRect = snapRectToDevicePixels(geometry.trackRect.adjusted(0.5, 0.5, -0.5, -0.5), devicePixelRatio);
  ...
  const qreal checkedThumbLeft = uncheckedOnLeft
                                     ? geometry.trackRect.right() - trackPadding - thumbSize
                                     : geometry.trackRect.left() + trackPadding;
  const qreal thumbLeft = uncheckedThumbLeft + (checkedThumbLeft - uncheckedThumbLeft) * thumbProgress;

  geometry.thumbRect = QRectF(thumbLeft,
                              geometry.trackRect.top() + (geometry.trackRect.height() - thumbSize) / 2.0,
                              thumbSize,
                              thumbSize);
  geometry.thumbRect = snapRectToDevicePixels(geometry.thumbRect.adjusted(0.5, 0.5, -0.5, -0.5), devicePixelRatio);
  geometry.thumbVisualRect = geometry.thumbRect;
  if (pressProgress > 0.0 && appearance.metrics.thumbActiveStretchRatio > 0.0) {
    const qreal activeInset = thumbSize * appearance.metrics.thumbActiveStretchRatio * pressProgress;
...
int switchContentWidth(const QString& text,
                       bool hasIcon,
                       const SwitchAppearance& appearance,
