...
QString scientificToDecimalString(QString text) {
  text = text.trimmed();
  const int expIndex = text.indexOf(QRegularExpression(QStringLiteral("[eE]")));
  if (expIndex < 0) {
    return text;
  }
  ...
  const int dotIndex = mantissa.indexOf(QLatin1Char('.'));
  ...
  int decimalPoint = integerPart.size() + exponent;
  if (decimalPoint <= 0) {
...
QString normalizeLocaleNumberText(QString text, const QLocale& locale) {
  ...
  const QChar decimalPoint =
      decimalPointText.isEmpty() ? QLatin1Char('.') : decimalPointText.at(0);
  const QChar groupSeparator =
      groupSeparatorText.isEmpty() ? QChar() : groupSeparatorText.at(0);
  const QString positiveSignText = locale.positiveSign();
  const QString negativeSignText = locale.negativeSign();
  const QChar plusSign =
      positiveSignText.isEmpty() ? QLatin1Char('+') : positiveSignText.at(0);
  const QChar minusSign =
      negativeSignText.isEmpty() ? QLatin1Char('-') : negativeSignText.at(0);
...
QString addPositiveDigits(const QString& lhs, const QString& rhs) {
  ...
  int leftIndex = lhs.size() - 1;
  int rightIndex = rhs.size() - 1;
  int carry = 0;
...
QString scaledDigits(const DecimalValue& value, int scale) {
...
QString scaledDigitsToCanonicalString(bool negative, QString digits, int scale) {
...
QString scaledDigitsToFixedString(bool negative, QString digits, int scale) {
  digits = trimLeadingZeros(digits);
  if (scale > 0) {
    if (digits.size() <= scale) {
      digits.prepend(QString(scale - digits.size() + 1, QLatin1Char('0')));
...
int compareDecimalValues(const DecimalValue& lhs, const DecimalValue& rhs) {
  ...
  const int scale = std::max(lhs.decimal.size(), rhs.decimal.size());
...
QString addDecimalStrings(const QString& lhs, const QString& rhs) {
  ...
  const int scale = std::max(left.decimal.size(), right.decimal.size());
...
QString fixedDecimalString(const QString& text, int decimals, bool cutOnly = false) {
  ...
  const int nextDigit = value.decimal.size() > decimals ? value.decimal.at(decimals).digitValue() : 0;
...
int decimalsFromString(QString text) {
  ...
  return std::max(0, static_cast<int>(text.size()) - dotIndex - 1);
}
...
int mergedPrecisionForText(const QString& text,
                           const QVariant& singleStep,
...
QString decimalPointToken(const QLocale& locale) {
...
const InputNumberValueModel::Config& InputNumberValueModel::config() const {
  return config_;
}

void InputNumberValueModel::setConfig(const Config& config) {
  config_ = config;
}
...
QVariant InputNumberValueModel::parseInputText(
    const QString& text,
...
QVariant InputNumberValueModel::steppedValue(const QVariant& currentValue, int steps) const {
  ...
  if (currentText.isEmpty()) {
    currentText =
        steps > 0 ? normalizedDecimalString(config_.minimum, config_.locale)
...
QString InputNumberValueModel::defaultDisplayText(const QVariant& value, bool userTyping) const {
  ...
  const int displayPrecision =
      userTyping ? -1
                 : mergedPrecisionForText(text, config_.singleStep, config_.decimals, config_.locale,
                                          false);
...
bool InputNumberValueModel::lessThan(const QVariant& lhs, const QVariant& rhs) const {
  ...
  return compareDecimalValues(parseCanonicalDecimalString(left), parseCanonicalDecimalString(right)) <
         0;
}
...
bool InputNumberValueModel::isIntermediateText(const QString& text, const QLocale& locale) {
  ...
  const QString signPattern = QStringLiteral("(?:%1|%2)?")
  ...
  const QRegularExpression leadingDecimal(
      QStringLiteral("^%1%2[0-9]*$").arg(signPattern, decimal));
