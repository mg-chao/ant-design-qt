...
QColor resolveTokenColorChain(const std::optional<QColor>& primaryToken,
                             const std::optional<QColor>& secondaryToken,
...
const AdNavigationMenu::SharedColorTokens& sharedColors(const AdNavigationMenu::ComponentTokens& tokens) {
  return tokens.colors.shared;
}

const AdNavigationMenu::SchemeColorTokens& lightColors(const AdNavigationMenu::ComponentTokens& tokens) {
  return tokens.colors.light;
}

const AdNavigationMenu::SchemeColorTokens& darkColors(const AdNavigationMenu::ComponentTokens& tokens) {
  return tokens.colors.dark;
}

QColor resolveSchemeColor(const std::optional<QColor>& schemeToken,
                         const std::optional<QColor>& sharedToken,
                         const QColor& fallback) {
...
void applyTokenOverrides(MenuMetrics& metrics, const AdNavigationMenu::ComponentTokens& tokens) {
  ...
  if (metricTokens.horizontalItemBorderRadius.has_value()) {
    metrics.horizontalItemBorderRadius = std::max(0, metricTokens.horizontalItemBorderRadius.value());
  }
...
MenuVisualStyle makeLightStyle(const ThemeMapToken& map,
  ...
  style.menuBackground = resolveSchemeColor(
      light.itemBackground, shared.itemBackground, toColor(map.colorBgContainer, QColor("#ffffff")));
  style.borderColor = toColor(map.colorBorderSecondary, QColor("#f0f0f0"));
  style.dividerColor = style.borderColor;
  style.groupTitleColor = resolveSchemeColor(
      light.groupTitleText, shared.groupTitleText, toColor(map.colorTextTertiary, QColor("#8c8c8c")));
  style.subMenuBackground = resolveSchemeColor(
      light.subMenuItemBackground,
      shared.subMenuItemBackground,
      toColor(map.colorFillQuaternary, QColor("#fafafa")));
  style.popupBackground = resolveSchemeColor(
      light.popupBackground, shared.popupBackground, toColor(map.colorBgElevated, QColor("#ffffff")));
  style.popupBorderColor = style.borderColor;

  style.normal.text =
      resolveSchemeColor(light.itemText, shared.itemText, toColor(map.colorText, QColor("#141414")));
  style.normal.background = transparent;

  style.hover.text = resolveSchemeColor(
      light.itemHoverText, shared.itemHoverText, toColor(map.colorText, QColor("#141414")));
  style.hover.background = resolveSchemeColor(
      light.itemHoverBackground,
      shared.itemHoverBackground,
      toColor(map.colorFillSecondary, QColor("#f5f5f5")));

  style.active.text = toColor(map.colorText, QColor("#141414"));
  style.active.background = resolveSchemeColor(
      light.itemActiveBackground,
      shared.itemActiveBackground,
      toColor(map.colorPrimaryBg, QColor("#e6f4ff")));

  style.selected.text = resolveSchemeColor(
      light.itemSelectedText, shared.itemSelectedText, toColor(map.colorPrimary, QColor("#1677ff")));
  style.selected.background = resolveSchemeColor(
      light.itemSelectedBackground,
      shared.itemSelectedBackground,
      toColor(map.colorPrimaryBg, QColor("#e6f4ff")));
  ...
  style.disabled.text = resolveSchemeColor(light.itemDisabledText,
                                           shared.itemDisabledText,
                                           toColor(map.colorTextQuaternary, QColor("#bfbfbf")));
  style.disabled.background = transparent;

  style.danger.text = resolveSchemeColor(
      light.dangerItemText, shared.dangerItemText, toColor(map.colorError, QColor("#ff4d4f")));
  ...
  style.dangerActive.background = resolveSchemeColor(
      light.dangerItemActiveBackground,
      shared.dangerItemActiveBackground,
      toColor(map.colorErrorBg, QColor("#fff2f0")));

  style.dangerSelected.text =
      resolveSchemeColor(light.dangerItemSelectedText, shared.dangerItemSelectedText, style.danger.text);
  style.dangerSelected.background = resolveSchemeColor(
      light.dangerItemSelectedBackground,
      shared.dangerItemSelectedBackground,
      toColor(map.colorErrorBg, QColor("#fff2f0")));

  style.horizontalNormal = style.normal;
  style.horizontalHover = style.hover;
  style.horizontalHover.text =
      resolveSchemeColor(light.horizontalItemHoverText, shared.horizontalItemHoverText, style.hover.text);
  ...
  style.horizontalSelected.background = resolveSchemeColor(
      light.horizontalItemSelectedBackground,
      shared.horizontalItemSelectedBackground,
      transparent);

  if (light.horizontalItemHoverBackground.has_value() || shared.horizontalItemHoverBackground.has_value()) {
    style.horizontalHover.background = resolveSchemeColor(
        light.horizontalItemHoverBackground,
        shared.horizontalItemHoverBackground,
...
MenuVisualStyle makeDarkStyle(const ThemeMapToken& map,
  ...
  style.subMenuBackground = resolveSchemeColor(
      dark.subMenuItemBackground, shared.subMenuItemBackground, QColor("#000c17"));
  ...
  style.active.background =
      resolveSchemeColor(dark.itemActiveBackground,
                         shared.itemActiveBackground,
  ...
  style.disabled.text = resolveSchemeColor(
      dark.itemDisabledText, shared.itemDisabledText, withAlpha(textLight, 0.25));
  style.disabled.background = transparent;
  ...
  style.dangerActive.text = resolveSchemeColor(
      dark.dangerItemSelectedText, shared.dangerItemSelectedText, textLight);
  style.dangerActive.background = resolveSchemeColor(
  ...
  style.horizontalHover = style.hover;
  style.horizontalHover.text =
      resolveSchemeColor(dark.horizontalItemHoverText, shared.horizontalItemHoverText, style.hover.text);
  style.horizontalActive = style.horizontalHover;
  style.horizontalSelected.text = resolveSchemeColor(
      dark.horizontalItemSelectedText, shared.horizontalItemSelectedText, style.selected.text);
  style.horizontalSelected.background = resolveSchemeColor(
      dark.horizontalItemSelectedBackground,
      shared.horizontalItemSelectedBackground,
      style.selected.background);

  if (dark.horizontalItemHoverBackground.has_value() || shared.horizontalItemHoverBackground.has_value()) {
    style.horizontalHover.background = resolveSchemeColor(
        dark.horizontalItemHoverBackground,
        shared.horizontalItemHoverBackground,
        style.horizontalHover.background);
  }
...
MenuVisualStyle resolveMenuVisualStyle(const MenuStyleInput& input) {
  const adqt::theme::ResolvedTheme resolved = adqt::theme::makeResolvedTheme(adqt::theme::makeTheme());
