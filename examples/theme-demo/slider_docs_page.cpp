...
QLabel* makeIconLabel(const adqt::icons::IconRef& token, const QColor& color, QWidget* parent = nullptr) {
...
QWidget* SliderDocsPage::buildInputNumberDemo() {
  ...
  connect(integerInput, &AdInputNumber::valueChanged, integerSlider, [integerSlider](double value) {
  ...
  connect(decimalInput, &AdInputNumber::valueChanged, decimalSlider, [decimalSlider](double value) {
    decimalSlider->setValue(value);
  });
...
QWidget* SliderDocsPage::buildEventDemo() {
  ...
  connect(range, &AdMultiSlider::handleValuesChanged, rangeHint, [rangeHint](const QList<double>& values) {
    QStringList parts;
