...
class PopoverContentView final : public QWidget {
 ...
 private:
  ...
  void updateSpacing() {
    ...
    const bool showGap = titleHost_ && titleHost_->isVisible() && contentHost_ && contentHost_->isVisible();
...
bool isVisualRefreshEvent(QEvent::Type type) {
  return type == QEvent::FontChange || type == QEvent::PaletteChange || type == QEvent::StyleChange ||
         type == QEvent::ApplicationFontChange || type == QEvent::ApplicationPaletteChange;
}
...
detail::PopoverStyleOverrides makePopoverStyleOverrides(
    const std::optional<QFont>& titleFontOverride,
    const std::optional<QFont>& textFontOverride,
    const std::optional<QColor>& backgroundColorOverride,
    const std::optional<QColor>& borderColorOverride,
    const std::optional<QColor>& titleColorOverride,
    const std::optional<QColor>& textColorOverride,
    const std::optional<QMargins>& contentMarginsOverride,
    const std::optional<int>& titleMinimumWidthOverride,
    const std::optional<int>& maximumWidthOverride,
    const std::optional<int>& zIndexOverride,
    const std::optional<int>& cornerRadiusOverride,
    const std::optional<int>& borderWidthOverride,
    const std::optional<int>& arrowSizeOverride,
    const std::optional<int>& popupOffsetOverride) {
...
detail::PopoverVisualStyle resolvePopoverStyleSnapshot(AdPopover::Placement placement,
                                                       bool visible,
                                                       bool disabled,
                                                       bool arrowVisible,
                                                       const QFont& baseFont,
                                                       const detail::PopoverStyleOverrides& overrides,
...
QFont AdPopover::titleFont() const {
  ...
  const detail::PopoverVisualStyle style =
      resolvePopoverStyleSnapshot(placement_, isVisible(), effectiveDisabled(), arrowVisible_, effectiveBaseFont(),
                                  makePopoverStyleOverrides(titleFontOverride_, textFontOverride_,
                                                            backgroundColorOverride_, borderColorOverride_,
                                                            titleColorOverride_, textColorOverride_,
                                                            contentMarginsOverride_, titleMinimumWidthOverride_,
                                                            maximumWidthOverride_, zIndexOverride_,
                                                            cornerRadiusOverride_, borderWidthOverride_,
                                                            arrowSizeOverride_, popupOffsetOverride_),
                                  effectiveAnchorWidget() ? effectiveAnchorWidget()
...
void AdPopover::deleteOwnedWidget(QPointer<QWidget>& widget, QMetaObject::Connection& destroyedConnection) {
...
QWidget* AdPopover::takeOwnedWidget(QPointer<QWidget>& widget, QMetaObject::Connection& destroyedConnection) {
  QObject::disconnect(destroyedConnection);
  destroyedConnection = QMetaObject::Connection();
  if (!widget) {
    return nullptr;
...
void AdPopover::ensurePopupSurface() {
  ...
  auto* surface = new detail::OverlayPopupSurface(scopeWindow);
...
void AdPopover::releasePopupSurface() {
  const auto reparentIfOwned = [this](QPointer<QWidget>& widget) {
    if (!widget) {
      return;
    }
    const bool insideSurface =
        popupSurface_ && (widget.data() == popupSurface_ || popupSurface_->isAncestorOf(widget));
    if (!insideSurface) {
      return;
    }
    widget->hide();
...
void AdPopover::refreshObservedWidgets() {
  ...
  for (auto it = observedWidgetDestroyedConnections_.begin(); it != observedWidgetDestroyedConnections_.end();) {
  ...
  for (QObject* object : nextWidgets) {
    ...
    observedWidgetDestroyedConnections_.insert(
        object, connect(object, &QObject::destroyed, this, &AdPopover::handleObservedWidgetDestroyed));
  }
}
...
void AdPopover::applyDefaultVisibleIfNeeded() {
  if (!controller_ || defaultVisibleApplied_ || explicitVisibleSet_ || !defaultVisible_ ||
      visibilityPolicy_ == VisibilityPolicy::Manual || !popupHasContent() || !effectiveAnchorWidget()) {
    return;
  }
...
void AdPopover::handleControllerPopupVisibleChanged(bool value) {
  if (popupSurface_) {
    detail::notifyAccessibilityEvent(popupSurface_, value ? QAccessible::ObjectShow : QAccessible::ObjectHide);
  }
...
detail::OverlayPopupPlacement AdPopover::popupPlacement() const { return toOverlayPlacement(placement_); }
...
void AdPopover::popupApplyResolvedPlacement(detail::OverlayPopupPlacement placement, qreal arrowCenterCoord) {
  auto* surface = static_cast<detail::OverlayPopupSurface*>(popupSurface_.data());
  if (!surface) {
    return;
  }
...
bool AdPopover::popupReleaseOnHide() const { return popupLifetime_ == PopupLifetime::RecreateOnOpen; }
