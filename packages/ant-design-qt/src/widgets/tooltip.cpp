...
void mergeSemanticSlotStyle(AdTooltip::SemanticSlotStyle* target, const AdTooltip::SemanticSlotStyle& extra) {
  if (!target) {
    return;
  }
...
detail::OverlayPopupController::VisibilityMode toControllerActivationMode(AdTooltip::ActivationMode value) {
  return value == AdTooltip::ActivationMode::Manual ? detail::OverlayPopupController::VisibilityMode::External
...
class AdTooltipPrivate final : public QObject, private detail::OverlayPopupControllerDelegate {
 ...
 public:
  ...
  void popupApplyResolvedPlacement(detail::OverlayPopupPlacement placement, qreal arrowCenterCoord) override;
  ...
  int hoverOpenDelayMs = 100;
  int hoverCloseDelayMs = 100;
  bool enabled = true;
  QString text;
...
void TooltipPopupView::ensureSurface(AdTooltipPrivate* owner, QWidget* scopeWindow) {
  ...
  auto* surfaceWidget = new detail::OverlayPopupSurface(scopeWindow);
...
bool AdTooltipPrivate::actualVisible() const { return popupView.surface && popupView.surface->isVisible(); }
...
void AdTooltipPrivate::updateStyle() {
  ...
  const int centerAlignedMinWidth = std::max(1, styleState.visual.metrics.borderRadius * 2 + arrowWidth);
  const int edgeOffsetHorizontal =
      (styleState.visual.metrics.borderRadius > 12) ? (styleState.visual.metrics.borderRadius + 2) : 12;
  const int edgeAlignedMinWidth = std::max(centerAlignedMinWidth,
  ...
  const int naturalContentWidth = measureTooltipNaturalTextWidth(text, styleState.visual.metrics.textFont);
...
void AdTooltipPrivate::refreshObservedWidgets() {
  ...
  for (auto it = observedWidgetDestroyedConnections.begin(); it != observedWidgetDestroyedConnections.end();) {
  ...
  for (QObject* object : nextWidgets) {
    ...
    observedWidgetDestroyedConnections.insert(
        object, connect(object, &QObject::destroyed, this, &AdTooltipPrivate::handleObservedWidgetDestroyed));
  }
}

void AdTooltipPrivate::clearObservedWidgets() {
  for (auto it = observedWidgetDestroyedConnections.begin(); it != observedWidgetDestroyedConnections.end(); ++it) {
...
void AdTooltipPrivate::handleControllerPopupVisibleChanged(bool value) {
  ...
  if (popupView.surface) {
    detail::notifyAccessibilityEvent(popupView.surface, value ? QAccessible::ObjectShow : QAccessible::ObjectHide);
  }
  if (!value) {
    syncActualVisible();
  }
...
std::optional<QRect> AdTooltipPrivate::popupAnchorGlobalRect() const {
  ...
  return QRect(coordinateWidget->mapToGlobal(anchorRect.value().topLeft()), anchorRect.value().size());
}

detail::OverlayPopupPlacement AdTooltipPrivate::popupPlacement() const { return toOverlayPlacement(placement); }
...
void AdTooltip::setTargetWidget(QWidget* widget) {
  ...
  if (!d->anchorWidgetOverride && previousResolvedAnchor != d->resolvedAnchorWidget() && d->anchorRect.has_value()) {
    d->anchorRect.reset();
