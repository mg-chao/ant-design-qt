...
struct AdColorSelection {
  ...
  bool operator==(const AdColorSelection& other) const {
    return kind == other.kind && solidColor == other.solidColor && gradientStops == other.gradientStops;
  }
...
struct AdColorValue {
  ...
  static AdColorValue gradient(const QGradientStops& stops, const QColor& editableColor = QColor()) {
    AdColorValue value;
...
inline AdColorValue toColorValue(const AdColorSelection& selection) {
  ...
  if (selection.isGradient()) {
    ...
    for (const AdColorGradientStop& stop : selection.gradientStops) {
      stops.append(QGradientStop(qBound(0.0, static_cast<double>(stop.percent) / 100.0, 1.0), stop.color));
    }
...
inline AdColorSelection toColorSelection(const AdColorValue& value) {
  ...
  if (value.isGradient()) {
    ...
    for (const QGradientStop& stop : value.gradientStops) {
      stops.append(
          AdColorGradientStop{stop.second, qBound(0, qRound(stop.first * 100.0), 100)});
    }
