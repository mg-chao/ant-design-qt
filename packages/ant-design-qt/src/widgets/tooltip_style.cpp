...
void applySemanticSlot(const AdTooltip::SemanticSlotStyle& slot,
                       QColor* textColor,
...
TooltipVisualStyle resolveTooltipVisualStyle(const TooltipStyleInput& input,
  ...
  style.metrics.padding = QMargins(std::max(0, qRound(map.sizeXS)), std::max(0, qRound(map.sizeSM / 2.0)),
                                   std::max(0, qRound(map.sizeXS)),
                                   std::max(0, qRound(map.sizeSM / 2.0)));
  ...
  style.contentColor = resolveTokenColor(tokens.textColor,
                                         explicitBackground ? textColorForBackground(style.surfaceBackground)
                                                            : style.contentColor);
  style.arrowBackground = style.surfaceBackground;
  style.arrowBorderColor = style.surfaceBorderColor;
