...
QRect textAreaFrameRect(const QRect& bounds, const InputVisualStyle& style, bool countVisible) {
  const int reservedBottom = countVisible ? style.metrics.countTopMargin + style.metrics.countHeight : 0;
...
AdTextEdit::AdTextEdit(QWidget* parent) : QTextEdit(parent) {
  ...
  setStyleSheet(QStringLiteral("QTextEdit { border: 0px; padding: 0px; background: transparent; }"));
...
QSize AdTextEdit::sizeHint() const {
  ...
  const int rows = std::clamp(minimumVisibleRows_, 1, std::max(minimumVisibleRows_, maximumVisibleRows_));
  const int contentHeight = rows * lineHeight + detail::input_internal::textAreaRowPaddingExtra(style);
  const int countHeight =
      countVisible_ ? (style.metrics.countTopMargin + style.metrics.countHeight) : 0;
  const int textWidth = std::max(horizontalTextWidth(fm, placeholderText()),
                                 horizontalTextWidth(fm, toPlainText()));
  const int widthHint =
      std::max(240, contentInsets.left() + contentInsets.right() + textWidth + widthAccessoryHint(style) +
                         style.metrics.height * 2);
...
void AdTextEdit::paintEvent(QPaintEvent* event) {
...
void AdTextEdit::updateClearButton() {
  ...
  const QPixmap normal =
      detail::input_internal::renderTintedIcon(filled_icons::CloseCircle(), style.clearColor, iconSide,
                                               devicePixelRatioF());
  const QPixmap hover =
      detail::input_internal::renderTintedIcon(filled_icons::CloseCircle(), style.clearHoverColor,
                                               iconSide, devicePixelRatioF());
  const QPixmap pressed =
      detail::input_internal::renderTintedIcon(filled_icons::CloseCircle(), style.clearActiveColor,
                                               iconSide, devicePixelRatioF());
...
void AdTextEdit::applyScrollBarStyle() {
  ...
  const int collapsedInset =
      std::max(0, baseThickness - detail::input_internal::kTextAreaScrollBarCollapsedVisualThickness);

  QScrollBar* styledVerticalBar = overlayVerticalScrollBar_ ? overlayVerticalScrollBar_ : verticalScrollBar();
...
void AdTextEdit::updateLayoutMetrics() {
  ...
  const int clearWidth = (clearButton_ && clearButton_->isVisible()) ? clearButton_->width() + clearGap : 0;
  const int countHeight =
      countVisible_ ? style.metrics.countTopMargin + style.metrics.countHeight : 0;

  setViewportMargins(contentInsets.left(),
                     contentInsets.top(),
                     contentInsets.right() + clearWidth,
  ...
  if (countLabel_ && countVisible_) {
    ...
    const int y = countAreaTop + std::max(0, (style.metrics.countHeight - countLabel_->height()) / 2);
...
void AdTextEdit::updateHeightConstraints() {
  ...
  const int desiredRows = std::clamp((contentHeight + documentLineHeight - 1) /
                                         std::max(1, documentLineHeight),
                                     minRows,
                                     maxRows);
...
void AdTextEdit::applyEditorPalette() {
  const InputVisualStyle style = resolvedStyle();
  const QColor background = resolvedBackgroundColor(style, focused_, hovered_);
...
int AdTextEdit::widthAccessoryHint(const InputVisualStyle& style) const {
  ...
  if (countVisible_) {
    widthHint += horizontalTextWidth(fm, countLabelText(toPlainText(),
                                                        effectiveCount(toPlainText()),
                                                        effectiveCountMax()));
  }
  return widthHint;
}
