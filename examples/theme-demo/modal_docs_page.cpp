...
ModalDocsPage::ModalDocsPage(QWidget* parent) : QWidget(parent) {
  ...
  addSection(root, "Controlled open state", "Repository demo: controlled-open.cpp", buildControlledDemo());
...
QWidget* ModalDocsPage::buildControlledDemo() {
  ...
  connect(modal, &AdModal::closeRequested, modal,
          [modal, blockClose, status, reasonText](AdModal::CloseReason reason) {
            ...
            if (!blockClose->isChecked()) {
              const AdModal::DialogCode result =
                  reason == AdModal::CloseReason::OkAction ? AdModal::DialogCode::Accepted
  ...
  layout->addWidget(makeHintLabel("When the checkbox is checked, the modal stays open after closeRequested."));
  return box;
}
...
QWidget* ModalDocsPage::buildFooterDemo() {
  ...
  connect(searchButton, &QAbstractButton::clicked, status, [status]() {
...
QWidget* ModalDocsPage::buildFooterRenderDemo() {
  ...
  connect(customButton, &QAbstractButton::clicked, modal, [modal]() {
    modal->setText("Custom footer button clicked.");
  });
...
QWidget* ModalDocsPage::buildMaskDemo() {
  ...
  connect(notClosable, &QAbstractButton::clicked, this, [openConfirm]() { openConfirm(true, false); });
...
QWidget* ModalDocsPage::buildWidthDemo() {
  ...
  connect(fixedButton, &QAbstractButton::clicked, fixedModal, [fixedModal]() { fixedModal->open(); });
  ...
  connect(openButton, &QAbstractButton::clicked, this, [this, status]() {
    ...
    connect(timer, &QTimer::timeout, modal, [modal, timer, secondsToGo, status]() {
      *secondsToGo -= 1;
      ...
      modal->setText(QStringLiteral("This modal will be destroyed after %1 seconds.")
                                .arg(*secondsToGo));
    });
    timer->start(1000);
  });
