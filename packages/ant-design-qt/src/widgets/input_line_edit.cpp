...
int affixPadding(const InputVisualStyle& style) {
...
InputVisualStyle applyDynamicMetricOverrides(InputVisualStyle style, const QObject* object) {
...
void setLabelIcon(QLabel* label,
...
void setButtonIcon(QToolButton* button,
                   const adqt::icons::IconRef& token,
  ...
  icon.addPixmap(pressedPixmap.isNull() ? activePixmap : pressedPixmap, QIcon::Selected, QIcon::Off);
...
AdLineEdit::AdLineEdit(QWidget* parent) : QLineEdit(parent) {
  ...
  setStyleSheet(QStringLiteral("QLineEdit { border: 0px; padding: 0px; background: transparent; }"));
...
QSize AdLineEdit::sizeHint() const {
  ...
  const int contentWidth = std::max(horizontalTextWidth(fm, placeholderText()),
                                    horizontalTextWidth(fm, text()));
  const int baseWidth = std::max(160,
...
void AdLineEdit::focusEditor(FocusSelection selection, bool preventScroll) {
  ...
  if (selection == FocusSelection::Start) {
    ...
    setCursorPosition(currentText.size());
...
bool AdLineEdit::eventFilter(QObject* watched, QEvent* event) {
  ...
  if (watched == prefixIconLabel_ || watched == prefixLabel_ || watched == suffixLabel_ ||
      watched == suffixIconLabel_ || watched == countLabel_) {
...
bool AdLineEdit::event(QEvent* event) {
  ...
  if (propertyName == kCompactHeightProperty || propertyName == kCompactHorizontalPaddingProperty ||
      propertyName == kCompactFontPixelSizeProperty || propertyName == kCompactIconSizeProperty) {
    refreshVisualState(true);
  }
...
void AdLineEdit::updateAccessoryVisibility() {
  if (!prefixLabel_ || !prefixIconLabel_ || !suffixLabel_ || !suffixIconLabel_ || !clearButton_ ||
      !suffixActionButton_ || !countLabel_) {
    return;
  }
  ...
  const bool showClear = allowClear_ && !isReadOnly() && isEnabled() && !text().isEmpty();
...
void AdLineEdit::updateAccessoryGeometry() {
  ...
  const auto placeTrailing = [&centeredRect, &right, &hasTrailingAffix](QWidget* widget,
                                                                         const QSize& size,
                                                                         int gapBefore) {
  ...
  const auto placeSuffixAction = [this, &placeTrailing]() {
    ...
    const QSize size = suffixActionButton_->sizeHint().expandedTo(suffixActionButton_->minimumSize());
  ...
  setTextMargins(std::max(0, left),
                 std::max(0, contentInsets.top()),
                 std::max(0, width() - right),
...
void AdLineEdit::updateClearButton() {
  ...
  setButtonIcon(clearButton_,
                filled_icons::CloseCircle(),
                style.clearColor,
                style.clearHoverColor,
                style.clearActiveColor,
                iconSide,
                devicePixelRatioF());
...
void AdLineEdit::updatePrefixVisual() {
  ...
  setLabelIcon(prefixIconLabel_,
               prefixIconRef_,
               isEnabled() ? style.prefixColor : style.disabledTextColor,
               iconSide,
               devicePixelRatioF());
}
...
void AdLineEdit::updateSuffixVisual() {
  ...
  const InputVisualStyle style = resolvedStyle();
  const QColor suffixColor = isEnabled() ? style.suffixColor : style.disabledTextColor;
  const ActionIconColors actionColors = suffixActionColors(style);
  ...
  setButtonIcon(suffixActionButton_,
                trailingActionIconRef_,
                actionColors.normal,
                actionColors.hover,
                actionColors.pressed,
                iconSide,
                devicePixelRatioF());
  suffixActionButton_->setEnabled(isEnabled());
}
...
InputVisualStyle AdLineEdit::resolvedStyle() const {
  ...
  input.baseFont = font();
  return applyDynamicMetricOverrides(adqt::widgets::detail::resolveInputVisualStyle(
...
void AdLineEdit::updateCursorForRole() {
  ...
  if (suffixActionButton_) {
    suffixActionButton_->setCursor((suffixActionButton_->isVisible() && suffixActionButton_->isEnabled())
                                       ? Qt::PointingHandCursor
                                       : passiveCursor);
  }
}
...
int AdLineEdit::accessoryWidthHint(const InputVisualStyle& style) const {
  ...
  const int passiveSuffixWidth =
      (adqt::icons::isValid(suffixIconRef_) ? iconSide : 0) +
      (!suffixText_.isEmpty() ? horizontalTextWidth(fm, suffixText_) : 0) +
      (adqt::icons::isValid(suffixIconRef_) && !suffixText_.isEmpty() ? itemGap : 0);
