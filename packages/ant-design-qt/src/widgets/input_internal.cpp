...
void InputIconButton::paintEvent(QPaintEvent* event) {
  ...
  const QIcon::Mode mode = !isEnabled() ? QIcon::Disabled
                                        : (isDown() ? QIcon::Selected
                                                    : (hovered_ ? QIcon::Active : QIcon::Normal));
...
QMargins textControlContentMargins(const adqt::widgets::detail::InputVisualStyle& style) {
  ...
  return QMargins(borderInset + horizontalPadding,
                  borderInset + verticalPadding,
                  borderInset + horizontalPadding,
                  borderInset + verticalPadding);
}
...
void paintInputFrame(QPainter* painter, const QRect& bounds, const InputFramePaintStyle& style) {
  ...
  if (style.underlined) {
    ...
    if (hasVisibleBorder) {
      ...
      painter->drawLine(QPointF(borderRect.left(), underlineY), QPointF(borderRect.right(), underlineY));
    }
    painter->restore();
    return;
  }
  ...
  const QPainterPath fillPath =
      roundedRectPath(fillRect, style.topLeftRadius, style.topRightRadius, style.bottomRightRadius,
  ...
  if (hasVisibleBorder) {
    const QPainterPath borderPath =
        roundedRectPath(borderRect, style.topLeftRadius, style.topRightRadius, style.bottomRightRadius,
                        style.bottomLeftRadius);
...
void updateInputFocusOverlay(const QWidget* owner,
  ...
  QRectF baseRect =
      joinedBorderRect(QRect(origin, frameRect.size()), style.metrics.borderWidth, joinedLeft, joinedRight);
