...
ManagedRadioGroup createManagedRadioGroup(Qt::Orientation orientation = Qt::Horizontal) {
  ...
  managed.layout = orientation == Qt::Vertical ? static_cast<QBoxLayout*>(new QVBoxLayout(managed.container))
...
QWidget* RadioDocsPage::buildQtNativeGroupingDemo() {
  ...
  layout->addWidget(makeHintLabel(
      "These radios share the same parent widget, so Qt handles exclusivity without a controller."));
  return box;
}
...
QWidget* RadioDocsPage::buildRadioGroupDemo() {
  ...
  connect(group.controller, &AdRadioButtonGroup::checkedIdChanged, output, [output, options](int id) {
...
QWidget* RadioDocsPage::buildVerticalGroupDemo() {
  ...
  connect(group.controller, &AdRadioButtonGroup::checkedIdChanged, moreInput, [moreInput](int id) {
    moreInput->setVisible(id == 4);
  });
...
QWidget* RadioDocsPage::buildStyleClassDemo() {
  ...
  functionStyle->setComponentTokenResolver([functionStyle](const AdRadio::ComponentTokenContext& state) {
