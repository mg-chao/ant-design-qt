...
bool isVisualRefreshEvent(QEvent::Type type) {
  return type == QEvent::EnabledChange || type == QEvent::FontChange || type == QEvent::ApplicationFontChange ||
         type == QEvent::PaletteChange || type == QEvent::ApplicationPaletteChange ||
         type == QEvent::StyleChange || type == QEvent::LayoutDirectionChange;
}
...
int measureIntrinsicTextWidth(const QString& value, const QFont& font, Qt::TextFormat format) {
  ...
  const bool richText = format == Qt::RichText || (format != Qt::PlainText && Qt::mightBeRichText(value));
  if (richText) {
    QTextDocument document;
    document.setDocumentMargin(0.0);
...
int measureIntrinsicTextHeight(const QString& value, const QFont& font, Qt::TextFormat format, int width) {
  if (value.isEmpty()) {
    return 0;
  }
...
StandardButtons allSupportedStandardButtons() {
  return StandardButton::Ok | StandardButton::Save | StandardButton::SaveAll | StandardButton::Open |
         StandardButton::Yes | StandardButton::YesToAll | StandardButton::No | StandardButton::NoToAll |
         StandardButton::Abort | StandardButton::Retry | StandardButton::Ignore | StandardButton::Close |
         StandardButton::Cancel | StandardButton::Discard | StandardButton::Help | StandardButton::Apply |
         StandardButton::Reset | StandardButton::RestoreDefaults;
}
...
bool isHelpRole(QDialogButtonBox::ButtonRole role) {
  return role == QDialogButtonBox::HelpRole;
}
...
int buttonKey(StandardButton button) {
...
QList<StandardButton> platformOrderedButtons(StandardButtons buttons, Qt::LayoutDirection direction) {
 ...
 public:
  ...
  adqt::icons::IconRef resolvedIconRef(const DerivedVisualStyle& style) const {
    ...
    if (!token.colors.hasPrimary) {
      token.colors.primary = style.iconColor;
  ...
  DerivedVisualStyle deriveVisualStyle() const {
    ...
    context.visibilityMode = popover ? popover->visibilityPolicy() : AdPopconfirm::VisibilityMode::Automatic;
    ...
    applySemanticSlot(effectiveSemantic.root, nullptr, &style.popoverBackgroundColor, &style.popoverBorderColor);
    applySemanticSlot(effectiveSemantic.container, nullptr, &style.popoverBackgroundColor,
                      &style.popoverBorderColor);
    applySemanticSlot(effectiveSemantic.arrow, nullptr, &style.popoverBackgroundColor, &style.popoverBorderColor);
  ...
  bool watchesPopupObject(const QObject* object) const {
    ...
    for (const QPointer<QObject>& watchedObject : watchedPopupObjects) {
      if (watchedObject == object) {
        return true;
      }
    }
    return false;
  ...
  void refreshAccessibility() {
    ...
    const StandardButton fallbackButton =
        resolvedDefaultButton() != StandardButton::NoButton ? resolvedDefaultButton() : resolvedEscapeButton();
    const QString fallbackButtonText = plainTextForAccessibility(resolvedButtonText(fallbackButton));
    const QString accessibleName = firstNonEmpty(
        {accessibleTitle, accessibleDescriptionText, fallbackButtonText, AdPopconfirm::tr("Confirmation")});
    const QString accessibleDescription = firstNonEmpty({accessibleDescriptionText, accessibleTitle});
    ...
    for (StandardButton buttonType : orderedButtons) {
      if (AdButton* control = panel->button(buttonType)) {
        const QString buttonAccessibleText = plainTextForAccessibility(resolvedButtonText(buttonType));
  ...
  void ensurePanel() {
    ...
    QObject::connect(contentPanel->buttonBox(), &QDialogButtonBox::clicked, q, [this](QAbstractButton* button) {
      if (!panel) {
        return;
      }
  ...
  void syncButtons(const DerivedVisualStyle& style) {
    ...
    const bool rebuildButtons = renderedButtons != visibleButtons ||
                                std::any_of(visibleButtons.cbegin(), visibleButtons.cend(), [this](StandardButton buttonType) {
                                  return !panel->button(buttonType);
                                });
    ...
    for (StandardButton buttonType : renderedButtons) {
      ...
      control->setButtonStyle(buttonConfig(buttonType).style.value_or(defaultStyleForStandardButton(buttonType)));
      control->setAccentRole(
          buttonConfig(buttonType).accentRole.value_or(defaultAccentRoleForStandardButton(buttonType)));
  ...
  void refreshVisualStyle() {
    ...
    panel->textLayout()->setSpacing(hasTitleText && hasDescriptionText ? std::max(0, style.descriptionGap) : 0);
    ...
    if (showIcon) {
      ...
      const qreal dpr = styleOwnerWidget() ? std::max(1.0, styleOwnerWidget()->devicePixelRatioF()) : 1.0;
    ...
    syncButtons(style);
    panel->rootLayout()->setSpacing(panel->buttonBox()->isVisible() ? std::max(0, style.messageBottom) : 0);
    ...
    const int preferredTextWidth = std::max(intrinsicTextWidth(panel->titleLabel(), text),
                                            intrinsicTextWidth(panel->descriptionLabel(), informativeText));
    int preferredMessageWidth = preferredTextWidth;
    int iconWidth = 0;
    if (showIcon) {
      iconWidth =
          panel->iconLabel()->width() > 0 ? panel->iconLabel()->width() : panel->iconLabel()->sizeHint().width();
    ...
    const auto measuredLabelHeight = [&](QLabel* label, const QString& value) {
      ...
      return measureIntrinsicTextHeight(value, label->font(), label->textFormat(), availableTextWidth + 1) +
    ...
    const int textHeight =
        titleHeight + (hasTitleText && hasDescriptionText ? std::max(0, style.descriptionGap) : 0) + descriptionHeight;
    ...
    const int buttonHeight = panel->buttonBox()->isVisible()
                                 ? panel->buttonBox()->sizeHint().expandedTo(panel->buttonBox()->minimumSizeHint()).height()
                                 : 0;
    panel->setFixedHeight(
        messageHeight + (buttonHeight > 0 ? std::max(0, style.messageBottom) + buttonHeight : 0));
  ...
  StandardButtons autoCloseButtons = StandardButton::Ok | StandardButton::Save | StandardButton::SaveAll |
                                     StandardButton::Open | StandardButton::Yes | StandardButton::YesToAll |
                                     StandardButton::No | StandardButton::NoToAll | StandardButton::Abort |
                                     StandardButton::Retry | StandardButton::Ignore | StandardButton::Close |
                                     StandardButton::Cancel | StandardButton::Discard;
...
AdPopconfirm::AdPopconfirm(QObject* parent) : QObject(parent), d_(std::make_unique<AdPopconfirmPrivate>(this)) {
  if (qApp) {
  ...
  connect(d_->popover, &AdPopover::visibilityPolicyChanged, this, [this](AdPopover::VisibilityPolicy value) {
    emit visibilityModeChanged(value);
  });
  connect(d_->popover, &AdPopover::popupLifetimeChanged, this, &AdPopconfirm::popupLifetimeChanged);
  connect(d_->popover, &AdPopover::defaultVisibleChanged, this, &AdPopconfirm::defaultVisibleChanged);
  ...
  connect(d_->popover, &AdPopover::arrowPointAtCenterChanged, this, &AdPopconfirm::arrowPointAtCenterChanged);
  connect(d_->popover, &AdPopover::hoverOpenDelayMsChanged, this, &AdPopconfirm::hoverOpenDelayMsChanged);
  connect(d_->popover, &AdPopover::hoverCloseDelayMsChanged, this, &AdPopconfirm::hoverCloseDelayMsChanged);
...
bool AdPopconfirm::isVisible() const {
  return d_->popover && d_->popover->isVisible();
}
...
void AdPopconfirm::show() {
  setVisible(true);
}

void AdPopconfirm::hide() {
  setVisible(false);
}

void AdPopconfirm::toggle() {
...
bool AdPopconfirm::defaultVisible() const {
  return d_->popover && d_->popover->defaultVisible();
}
...
bool AdPopconfirm::arrowVisible() const {
  return d_->popover && d_->popover->arrowVisible();
}
...
bool AdPopconfirm::isEnabled() const {
  return d_->enabled;
}
...
QString AdPopconfirm::text() const {
  return d_->text;
}
...
QString AdPopconfirm::informativeText() const {
  return d_->informativeText;
}
...
AdPopconfirm::StandardButtons AdPopconfirm::standardButtons() const {
  return d_->standardButtons;
}
...
AdPopconfirm::StandardButton AdPopconfirm::defaultButton() const {
  return d_->defaultButton;
}

void AdPopconfirm::setDefaultButton(StandardButton value) {
  const StandardButton normalized = isSupportedStandardButton(value) ? value : StandardButton::NoButton;
...
AdPopconfirm::StandardButton AdPopconfirm::escapeButton() const {
  return d_->escapeButton;
}
...
AdPopconfirm::Icon AdPopconfirm::icon() const {
  return d_->icon;
}
...
QWidget* AdPopconfirm::sourceWidget() const {
  return d_->sourceWidget;
}
...
AdPopconfirm::ComponentTokens AdPopconfirm::componentTokens() const {
...
AdPopconfirm::SemanticStyles AdPopconfirm::semanticStyles() const {
