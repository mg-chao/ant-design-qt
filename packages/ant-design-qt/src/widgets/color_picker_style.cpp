...
ColorPickerVisualStyle resolveColorPickerVisualStyle(const ColorPickerStyleInput& input,
  ...
  const int requiredCrossPadding = std::max(
      0, static_cast<int>(std::ceil(sliderOuterRadius + kSliderAntialiasPadding - sliderHalfHeight)));
  ...
  const qreal sliderHandleBorder = std::max(
      static_cast<qreal>(style.metrics.sliderHandleLineWidth),
      static_cast<qreal>(style.metrics.sliderHandleLineWidthHover));
  const int requiredMainPadding = std::max(
      0, static_cast<int>(std::ceil(sliderHandleRadius + sliderHandleBorder +
