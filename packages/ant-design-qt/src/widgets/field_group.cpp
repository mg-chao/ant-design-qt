...
bool isVisibleControl(const QWidget* control) {
  return control && !control->isHidden();
}
...
int AdFieldGroup::controlCount() const {
  return std::count_if(controls_.cbegin(), controls_.cend(),
...
bool AdFieldGroup::eventFilter(QObject* watched, QEvent* event) {
  const bool isTrackedControl =
      std::any_of(controls_.cbegin(), controls_.cend(), [watched](const ControlEntry& entry) {
        return entry.control.data() == watched;
      });
...
void AdFieldGroup::refreshJoinedEdges() {
  ...
  const int visibleCount = std::count_if(controls_.cbegin(), controls_.cend(),
                                         [](const ControlEntry& entry) {
                                           return isVisibleControl(entry.control.data());
                                         });
  int visibleIndex = 0;
  ...
  for (const ControlEntry& entry : controls_) {
    ...
    const bool appliedJoinedProperties =
        writeJoinedProperty(control, "joinedLeft", joinedLeft) |
    ...
    if (!appliedJoinedProperties) {
      if (auto* button = qobject_cast<AdButton*>(control)) {
        detail::setButtonSegmentPosition(button, segmentPositionForJoinState(joinedLeft, joinedRight));
      }
    }
