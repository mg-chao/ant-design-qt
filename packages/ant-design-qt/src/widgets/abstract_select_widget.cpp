...
class AdAbstractSelectWidgetAccessible final : public QAccessibleWidget {
 public:
  ...
  QString text(QAccessible::Text t) const override {
    ...
    if (QAccessibleInterface* iface = QAccessible::queryAccessibleInterface(embeddedSelect(widget))) {
      return iface->text(t);
    }
    return QAccessibleWidget::text(t);
  }
...
QAccessibleInterface* abstractSelectWidgetAccessibleFactory(const QString& className, QObject* object) {
  Q_UNUSED(className)
...
AdAbstractSelectWidget::AdAbstractSelectWidget(QWidget* parent) : QWidget(parent) {
  ...
  connect(control_, &AdSelect::modelColumnChanged, this, &AdAbstractSelectWidget::modelColumnChanged);
  ...
  connect(control_, &AdSelect::placeholderChanged, this, &AdAbstractSelectWidget::placeholderChanged);
  ...
  connect(control_, &AdSelect::variantChanged, this, [this](AdSelect::Variant value) {
    emit variantChanged(fromInternalVariant(value));
  });
  connect(control_, &AdSelect::statusChanged, this, [this](AdSelect::Status value) {
    emit statusChanged(fromInternalStatus(value));
  });
...
void AdAbstractSelectWidget::setPlaceholder(const QString& value) { control_->setPlaceholder(value); }
...
void AdAbstractSelectWidget::setVariant(Variant value) { control_->setVariant(toInternalVariant(value)); }

AdAbstractSelectWidget::Status AdAbstractSelectWidget::status() const {
  return fromInternalStatus(control_->status());
}

void AdAbstractSelectWidget::setStatus(Status value) { control_->setStatus(toInternalStatus(value)); }
...
void AdAbstractSelectWidget::setOptions(const QVector<Option>& options) { control_->setOptions(options); }
...
void AdAbstractSelectWidget::setRoleConfig(const RoleConfig& config) { control_->setRoleConfig(config); }

QList<int> AdAbstractSelectWidget::searchRoles() const { return control_->searchRoles(); }

void AdAbstractSelectWidget::setSearchRoles(const QList<int>& roles) { control_->setSearchRoles(roles); }
...
QAbstractItemDelegate* AdAbstractSelectWidget::itemDelegate() const { return control_->itemDelegate(); }
...
void AdAbstractSelectWidget::setSemanticStyleResolver(SemanticStyleResolver resolver) {
  ...
  control_->setSemanticStyleResolver([resolver = std::move(resolver)](const AdSelect::StyleContext& ctx) {
