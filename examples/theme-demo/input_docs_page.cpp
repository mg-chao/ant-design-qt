...
class CompactAddon final : public QWidget {
 ...
 protected:
  ...
  void paintEvent(QPaintEvent* event) override {
    ...
    int cursorX = startX;
    if (hasIcon) {
      adqt::icons::IconRef icon = iconRef_;
      icon.colors.primary = textColor;
...
InputDocsPage::InputDocsPage(QWidget* parent) : QWidget(parent) {
  ...
  addSection(root, "Qt-native customization hooks", "Demo: native-hooks.tsx", buildStyleClassDemo());
...
QWidget* InputDocsPage::buildSearchInputDemo() {
  ...
  auto makeSearch = [info](bool allowClear, bool searchButtonVisible, const QString& buttonText,
    ...
    search->setSearchButtonText(searchButtonVisible ? (buttonText.isEmpty() ? QStringLiteral("Search") : buttonText)
                                            : QString());
    search->setControlSize(size);
    search->setFixedWidth(320);
    QObject::connect(search, &AdSearchEdit::searchRequested, info,
  ...
  layout->addWidget(makeSearch(true, true, QStringLiteral("Search"), AdLineEdit::ControlSize::Large), 0, Qt::AlignLeft);

  auto* suffixSearch = makeSearch(false, true, QStringLiteral("Search"), AdLineEdit::ControlSize::Large);
...
QWidget* InputDocsPage::buildClearButtonDemo() {
  ...
  connect(input, &AdLineEdit::textChanged, output,
          [output](const QString& text) { output->setText(QStringLiteral("Input changed: %1").arg(text)); });
  connect(textArea, &AdTextEdit::plainTextChanged, output,
...
QWidget* InputDocsPage::buildFocusDemo() {
  ...
  auto focusCurrent = [stack, input, textArea](AdLineEdit::FocusSelection cursor, bool preventScroll) {
