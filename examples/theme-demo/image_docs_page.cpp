...
const QUrl kWebpC(QStringLiteral(
    "https://gw.alipayobjects.com/zos/antfincdn/x43I27A55%26/photo-1438109491414-7198515b166b.webp"));
const QUrl kBlurPng(QStringLiteral(
...
ImageDocsPage::ImageDocsPage(QWidget* parent) : QWidget(parent) {
  ...
  addSection(root, QStringLiteral("Basic Usage"), QStringLiteral("Simple image loading."), buildBasicDemo());
  ...
  addSection(root, QStringLiteral("Progressive Loading"),
             QStringLiteral("Placeholder source while the main image is loading."), buildPlaceholderDemo());
  addSection(root, QStringLiteral("Preview Group"),
             QStringLiteral("Multiple thumbnails share one preview dialog."), buildPreviewGroupDemo());
  addSection(root, QStringLiteral("Preview Items"),
             QStringLiteral("One thumbnail can open a full preview album."), buildPreviewItemsDemo());
  addSection(root, QStringLiteral("Preview Text And Mask"),
             QStringLiteral("Preview text and mask styling stay within the supported API."), buildPreviewMaskDemo());
  addSection(root, QStringLiteral("Semantic Styling"),
             QStringLiteral("Use semantic slots and component tokens for theming."), buildStyleDemo());
  addSection(root, QStringLiteral("Preview Info"),
             QStringLiteral("Read current preview item info from the signal."), buildPreviewInfoDemo());
  addSection(root, QStringLiteral("Nested Dialog"),
             QStringLiteral("Preview still works when the image lives in a dialog."), buildNestedDemo());
...
AdImage* ImageDocsPage::createImage(const QUrl& src,
...
QWidget* ImageDocsPage::buildPreviewItemsDemo() {
  ...
  auto* info = makeInfoLabel(QStringLiteral("Click the thumbnail to open three preview items."), box);
...
QWidget* ImageDocsPage::buildNestedDemo() {
  ...
  connect(button, &QAbstractButton::clicked, box, [box]() {
    ...
    root->addWidget(new QLabel(QStringLiteral("The preview overlay is attached to this dialog window."), dialog));
    root->addWidget(ImageDocsPage::createImage(kWebpC,
                                               QSize(260, 160),
                                               QStringLiteral("nested preview"),
                                               dialog),
                    0,
                    Qt::AlignLeft);
