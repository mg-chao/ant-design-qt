...
AlertVisualStyle resolveAlertVisualStyle(const AlertStyleInput& input,
  ...
  style.closeButtonPressedBackground = toColor(colors.colorFillSecondary, style.closeButtonHoverBackground);
  ...
  style.metrics.paddingWithInformativeTextInline = std::max(style.metrics.paddingInline, qRound(metrics.sizeLG));
  style.metrics.paddingWithInformativeTextBlock = std::max(style.metrics.paddingBlock, qRound(metrics.sizeMD));
  ...
  if (input.hasPaletteOverride) {
    ...
    style.informativeTextColor = paletteColor(input.palette, QPalette::Text, group, style.informativeTextColor);
    style.iconColor = paletteColor(input.palette, QPalette::WindowText, group, style.iconColor);
    style.closeIconColor = paletteColor(input.palette, QPalette::ButtonText, group, style.closeIconColor);
    style.closeIconHoverColor = paletteColor(input.palette, QPalette::Text, group, style.closeIconHoverColor);
    style.closeButtonHoverBackground = paletteColor(input.palette,
                                                    QPalette::AlternateBase,
                                                    group,
                                                    style.closeButtonHoverBackground);
    style.closeButtonPressedBackground = paletteColor(input.palette,
                                                      QPalette::Button,
                                                      group,
                                                      style.closeButtonPressedBackground);
    style.closeButtonFocusOutlineColor = paletteColor(input.palette,
                                                      QPalette::Highlight,
                                                      group,
