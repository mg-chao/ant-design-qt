...
void applyAppearanceRole(const AdInputNumber::AppearanceRole& role,
                         QColor* textColor,
...
InputNumberVisualStyle resolveInputNumberVisualStyle(const InputNumberStyleInput& input,
  ...
  if (metrics.handleVisibleWidth.has_value()) {
    style.metrics.handleVisibleWidth =
        input.stepButtonsVisible
            ? std::max(0, metrics.handleVisibleWidth.value())
            : 0;
  ...
  if (input.controlSize == AdInputNumber::ControlSize::Small && metrics.inputFontSizeSM.has_value()) {
    style.metrics.inputFontSize = std::max(8, metrics.inputFontSizeSM.value());
    style.metrics.font.setPixelSize(style.metrics.inputFontSize);
  }
  if (input.controlSize == AdInputNumber::ControlSize::Large && metrics.inputFontSizeLG.has_value()) {
  ...
  applyAppearanceRole(appearance.input,
                      &style.selectorTextColor,
                      &style.selectorBg,
  ...
  applyAppearanceRole(appearance.actions,
                      &style.handleIconColor,
                      &style.handleBg,
