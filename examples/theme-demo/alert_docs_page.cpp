...
AlertDocsPage::AlertDocsPage(QWidget* parent) : QWidget(parent) {
  ...
  addSection(root, "Closable", "Close button path emits closeRequested() and closed().", buildClosableDemo());
  addSection(root, "Informative Text", "Two-line alerts with title and supporting content.", buildDescriptionDemo());
  addSection(root, "Icon", "Inline alerts can opt into a visible severity icon.", buildIconDemo());
  addSection(root, "Banner", "Banner mode stretches edge-to-edge and defaults to a visible icon.", buildBannerDemo());
  addSection(root, "Loop Banner", "Custom text widget content in banner mode.", buildLoopBannerDemo());
  addSection(root, "Leading Widget", "Custom leading content replaces token-based custom icons.", buildLeadingWidgetDemo());
  addSection(root, "Qt Close Lifecycle", "Programmatic close() and reopen behavior.", buildLifecycleDemo());
  addSection(root, "Custom action", "Hosted action widgets compose with close and content.", buildActionDemo());
  addSection(root, "Palette Override", "Explicit QPalette and QFont customize the component.", buildPaletteDemo());
...
AdAlert* AlertDocsPage::makeAlert(const QString& text,
                                  std::optional<AdAlert::Severity> severity,
                                  std::optional<AdAlert::IconMode> iconMode,
                                  bool closable,
...
QWidget* AlertDocsPage::buildClosableDemo() {
  ...
  for (const auto& item : items) {
    ...
    connect(alert, &AdAlert::closeRequested, status, [status, title = item.first](AdAlert::CloseReason) {
...
QWidget* AlertDocsPage::buildDescriptionDemo() {
  ...
  layout->addWidget(makeAlert(
      "Success Text", AdAlert::Severity::Success, AdAlert::IconMode::Hidden, false,
      "Success Description Success Description Success Description"));
  layout->addWidget(makeAlert(
      "Info Text", AdAlert::Severity::Info, AdAlert::IconMode::Hidden, false,
      "Info Description Info Description Info Description Info Description"));
  layout->addWidget(makeAlert(
      "Warning Text", AdAlert::Severity::Warning, AdAlert::IconMode::Hidden, false,
      "Warning Description Warning Description Warning Description Warning Description"));
  layout->addWidget(makeAlert(
      "Error Text", AdAlert::Severity::Error, AdAlert::IconMode::Hidden, false,
...
QWidget* AlertDocsPage::buildIconDemo() {
  ...
  layout->addWidget(makeAlert("Success Tips", AdAlert::Severity::Success, AdAlert::IconMode::Visible));
  layout->addWidget(makeAlert("Informational Notes", AdAlert::Severity::Info, AdAlert::IconMode::Visible));
  layout->addWidget(makeAlert("Warning", AdAlert::Severity::Warning, AdAlert::IconMode::Visible, true));
  layout->addWidget(makeAlert("Error", AdAlert::Severity::Error, AdAlert::IconMode::Visible));
  layout->addWidget(makeAlert("Success Tips",
                              AdAlert::Severity::Success,
                              AdAlert::IconMode::Visible,
                              false,
                              "Detailed description and advice about successful copywriting."));
  layout->addWidget(makeAlert("Informational Notes",
                              AdAlert::Severity::Info,
                              AdAlert::IconMode::Visible,
                              false,
...
QWidget* AlertDocsPage::buildBannerDemo() {
  ...
  configureAlert(first, [](AdAlert* alert) { alert->setDisplayMode(AdAlert::DisplayMode::Banner); });
  ...
  configureAlert(fourth, [](AdAlert* alert) { alert->setDisplayMode(AdAlert::DisplayMode::Banner); });
...
QWidget* AlertDocsPage::buildLeadingWidgetDemo() {
  ...
  auto* second = makeAlert("Deployment paused",
                           AdAlert::Severity::Warning,
                           AdAlert::IconMode::Hidden,
                           false,
  ...
  layout->addWidget(makeHintLabel(
      "Use setLeadingWidget() for custom iconography or compact semantic badges instead of token-based icon overrides."));
  return box;
}
...
QWidget* AlertDocsPage::buildLifecycleDemo() {
  ...
  auto* alert = makeAlert("Alert Message Text",
                          AdAlert::Severity::Success,
                          AdAlert::IconMode::Hidden,
                          true);
  ...
  connect(alert, &AdAlert::closeRequested, this, [status](AdAlert::CloseReason reason) {
    const QString reasonText =
        reason == AdAlert::CloseReason::CloseButton ? QStringLiteral("close button")
...
QWidget* AlertDocsPage::buildActionDemo() {
  ...
  auto* first = makeAlert("Success Tips",
                          AdAlert::Severity::Success,
                          AdAlert::IconMode::Visible,
                          true);
  ...
  auto* third = makeAlert("Info Text",
                          AdAlert::Severity::Info,
                          AdAlert::IconMode::Hidden,
                          true,
                          "Info Description Info Description Info Description Info Description");
...
QWidget* AlertDocsPage::buildPaletteDemo() {
  ...
  auto* alert = makeAlert("Palette Override",
                          AdAlert::Severity::Info,
                          AdAlert::IconMode::Visible,
                          true,
  ...
  layout->addWidget(alert);
  layout->addWidget(makeHintLabel(
      "Use theme defaults for consistency; apply palette/font overrides when a host view needs semantic emphasis."));
  return box;
}
